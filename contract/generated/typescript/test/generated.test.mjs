...
test("generated current contract surface is stable", () => {
  ...
  assert.deepEqual(METHODS.slice(27, 31), contextBankMethods);
...
test("skill packages retain binary resources and are full-control only", () => {
  const skillPackage = { name: "aspire", files: [
    { path: "SKILL.md", contentBase64: "c2tpbGw=", executable: false },
    { path: "assets/image.bin", contentBase64: "AP8=", executable: false },
  ] };
  assert.equal(validateMethodResult("skill.packageGet", skillPackage), true);
  assert.equal(validateMethodResult("skill.packageGet", { ...skillPackage, files: [] }), false);
  assert.equal(validateMethodResult("skill.packageGet", { ...skillPackage, files: [{ path: "SKILL.md" }] }), false);
  for (const method of ["skill.packageGet", "skill.packageCreate"]) {
    assert.ok(!READ_ONLY_METHODS.includes(method));
    assert.ok(!COMPANION_METHODS.includes(method));
  }
});

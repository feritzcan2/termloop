...
- Treat GitHub Actions as native verification, not as a lint or type-check
  iteration loop. Before dispatching manual release CI after a code change, run
  the complete required local checks and all installed affected Rust
  cross-target clippy checks. Cross-target success is only a preflight signal;
  it never replaces the native jobs required for release evidence.
- Preserve the cost gate in `.github/workflows/ci.yml`: the self-hosted macOS
  preflight runs static validation plus Linux and Windows cross-target clippy,
  and hosted/native verification jobs must depend on its success. Do not move
  those static checks back into every native matrix job or let hosted jobs start
  before preflight succeeds merely to shorten wall-clock time.
- Isolate native-only failures with the selectable
  `.github/workflows/native-diagnostic.yml` workflow. Run only the affected
  Windows, Ubuntu/Linux, or macOS jobs while iterating. Use its default `tests`
  scope for test failures; select `full` only when diagnosing acceptance, smoke,
  packaging, or bundle assembly. A partial diagnostic success is never release
  evidence. After the affected native diagnostics pass, dispatch one full
  `ci.yml` release gate. Full-gate native siblings use `fail-fast: false` so an
  independent failure does not discard already-paid cross-platform evidence.
  Fix and exhaust local verification before starting another exact-SHA run.
- If GitHub reports that an Actions budget prevents a hosted job from starting,
  do not retry or dispatch another hosted workflow until the budget resets or is
  increased. A budget-rejected job consumes no native evidence and cannot be
  repaired by rerunning the same SHA.
- Use `gh run rerun --failed` only for a transient failure when the candidate
  SHA and code are unchanged. A code fix requires a new immutable commit and a
  new exact-SHA manual CI run; never rewrite `main` to reuse old CI evidence.
...
- Keep test and build output bounded without hiding failures. Prefer quiet or
  concise reporters that preserve the exit code, final summary, and complete
  failing assertion/diagnostic. Do not stream lists of every passing test,
  routine compiler progress, or repeated successful output into the
  conversation. Set a proportionate tool-output limit; when a command fails,
  inspect only the failing scope or a bounded relevant excerpt instead of
  dumping the entire log. Report the command, pass/fail result, and test counts

...
- [x] Remote daemon bootstrap/upload/start path with `termloopd-remote serve --stdio`
...
- [ ] Extend `termloopd-remote` RPC beyond `hello/ping` with proxy stream methods (`proxy.open|close`)
...
- [x] Expand end-user skill docs with deep-linkable cmux-browser references/templates plus a new core `skills/termloop/` topology skill.
...
- [ ] Add "Install Claude Code integration" menu item in menubar
  ...
  - Implement as part of `termloop` CLI, menubar just triggers the CLI command
...
- [ ] Cmd+click should open links in termloop (browser panel) instead of external browser

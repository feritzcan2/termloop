...
const defaultFeedbackRecipient = "feritzcan93@gmail.com";
...
export async function POST(request: Request) {
  ...
  const feedbackRecipient =
    env.TERMLOOP_FEEDBACK_RECIPIENT_EMAIL ?? defaultFeedbackRecipient;

  const { error } = await resend.emails.send({
    from: `Termloop <${feedbackConfig.fromEmail}>`,
...
function resolveFeedbackConfig() {
  ...
  const fromEmail = env.TERMLOOP_FEEDBACK_FROM_EMAIL;
  const rateLimitId = env.TERMLOOP_FEEDBACK_RATE_LIMIT_ID;

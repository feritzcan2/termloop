      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>{t("release")}</td>
            <td>
              <code>/tmp/termloop.sock</code>
            </td>
          </tr>
          <tr>
            <td>{t("debug")}</td>
            <td>
              <code>/tmp/termloop-debug.sock</code>
            </td>
          </tr>
          <tr>
            <td>{t("taggedDebug")}</td>
            <td>
              <code>/tmp/termloop-debug-&lt;tag&gt;.sock</code>
            </td>
          </tr>
        </tbody>
      </table>
...
{"id":"req-1","ok":true,"result":{"workspaces":[...]}}`}</CodeBlock>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td>
              <strong>TermLoop processes only</strong>
            </td>
...
termloop send "ls -la\\n"`}
      ...
      <Cmd
        ...
        cli={`termloop send-key-surface --surface <id> enter`}
...
termloop list-notifications --json`}
      ...
      <Cmd
        ...
        cli={`termloop set-status build "compiling" --icon hammer --color "#ff9500"
...
termloop set-progress 1.0 --label "Done"`}
      ...
      <Cmd
        ...
        cli={`termloop clear-progress`}
        socket={`clear_progress --tab=<workspace-uuid>`}
      />
      <Cmd
        name="log"
        desc={t("logDesc")}
        cli={`termloop log "Build started"
...
termloop log --level success -- "All 42 tests passed"`}
      ...
      <Cmd
        name="clear-log"
        desc={t("clearLogDesc")}
        cli={`termloop clear-log`}
        socket={`clear_log --tab=<workspace-uuid>`}
      />
      <Cmd
        name="list-log"
        desc={t("listLogDesc")}
        cli={`termloop list-log
...
// Response: {"id":"ping","ok":true,"result":{"pong":true}}`}
      ...
      <Cmd
        ...
        cli={`termloop capabilities
termloop capabilities --json`}
        socket={`{"id":"caps","method":"system.capabilities","params":{}}`}
      />
      <Cmd
        name="identify"
        desc={t("identifyDesc")}
        cli={`termloop identify
termloop identify --json`}
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              <code>TERMLOOP_SOCKET_PATH</code>
            </td>
            <td>{t("socketPathDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>TERMLOOP_SOCKET_ENABLE</code>
            </td>
            <td>{t("socketEnableDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>TERMLOOP_SOCKET_MODE</code>
            </td>
            <td>{t("socketModeDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>TERMLOOP_WORKSPACE_ID</code>
            </td>
            <td>{t("workspaceIdDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>TERMLOOP_SURFACE_ID</code>
            </td>
...
SOCK="\${TERMLOOP_SOCKET_PATH:-/tmp/termloop.sock}"
[ -S "$SOCK" ] && echo "Socket available"

# Check for the CLI
command -v termloop &>/dev/null && echo "termloop available"

# In TermLoop-managed terminals these are auto-set
[ -n "\${TERMLOOP_WORKSPACE_ID:-}" ] && [ -n "\${TERMLOOP_SURFACE_ID:-}" ] && echo "Inside termloop surface"

# Distinguish from regular Ghostty
[ "$TERM_PROGRAM" = "ghostty" ] && [ -n "\${TERMLOOP_WORKSPACE_ID:-}" ] && echo "In TermLoop"`}</CodeBlock>
...
SOCKET_PATH = os.environ.get("TERMLOOP_SOCKET_PATH", "/tmp/termloop.sock")
...
npm run build
if [ $? -eq 0 ]; then
    termloop notify --title "✓ Build Success" --body "Ready to deploy"
else
    termloop notify --title "✗ Build Failed" --body "Check the logs"
fi`}</CodeBlock>
    </>
  );
}

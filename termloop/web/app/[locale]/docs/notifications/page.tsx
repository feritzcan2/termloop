  ...
  return (
    <>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td><code>TERMLOOP_NOTIFICATION_TITLE</code></td>
            <td>{t("envTitle")}</td>
          </tr>
          <tr>
            <td><code>TERMLOOP_NOTIFICATION_SUBTITLE</code></td>
            <td>{t("envSubtitle")}</td>
          </tr>
          <tr>
            <td><code>TERMLOOP_NOTIFICATION_BODY</code></td>
            <td>{t("envBody")}</td>
          </tr>
        </tbody>
      </table>
      <CodeBlock title="Examples" lang="bash">{`# Text-to-speech
say "$TERMLOOP_NOTIFICATION_TITLE"

# Custom sound file
afplay /path/to/sound.aiff

# Log to file
echo "$TERMLOOP_NOTIFICATION_TITLE: $TERMLOOP_NOTIFICATION_BODY" >> ~/notifications.log`}</CodeBlock>
...
case "$EVENT_TYPE" in
    "Stop")
        termloop notify --title "Claude Code" --body "Session complete"
        ;;
    "PostToolUse")
        [ "$TOOL" = "Task" ] && termloop notify --title "Claude Code" --body "Agent finished"
        ;;
esac`}</CodeBlock>
  ...
  "hooks": {
    ...
    "agentStop": [
      {
        "type": "command",
        "bash": "if command -v termloop &>/dev/null; then termloop notify --title 'Copilot CLI' --body 'Done'; termloop set-status copilot_cli Idle; fi",
        "timeoutSec": 5
      }
    ],
    "errorOccurred": [
      {
        "type": "command",
        "bash": "if command -v termloop &>/dev/null; then termloop notify --title 'Copilot CLI' --subtitle 'Error' --body 'An error occurred'; termloop set-status copilot_cli Error; fi",
        "timeoutSec": 5
      }
    ],
    "sessionEnd": [
...
notify-after() {
  ...
  if [ $exit_code -eq 0 ]; then
    termloop notify --title "✓ Command Complete" --body "$1"
  else
    termloop notify --title "✗ Command Failed" --body "$1 (exit $exit_code)"
  fi
  return $exit_code
}

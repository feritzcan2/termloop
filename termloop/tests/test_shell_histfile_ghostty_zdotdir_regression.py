...
Resources/shell-integration. If termloop incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    if not (cmux_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing termloop wrapper .zshenv at {cmux_wrapper_dir}")
        return 0
    ...
    try:
        ...
        # Simulate the buggy situation: termloop stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["TERMLOOP_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)

...
for asset in \
  ...
  termloopd-remote-manifest.json
do
...
python3 - <<'PY' "$OUTPUT_DIR/termloopd-remote-manifest.json" "$OUTPUT_DIR/termloopd-remote-checksums.txt"
import json
import sys
...
if not manifest["checksumsURL"].endswith("/termloopd-remote-checksums.txt"):
...
for asset in \
  termloopd-remote-darwin-arm64-123456 \
  termloopd-remote-darwin-amd64-123456 \
  termloopd-remote-linux-arm64-123456 \
  termloopd-remote-linux-amd64-123456 \
  termloopd-remote-checksums-123456.txt \
  termloopd-remote-manifest-123456.json
do
...
python3 - <<'PY' "$SUFFIX_DIR/termloopd-remote-manifest-123456.json"
import json
import sys
from pathlib import Path

#!/usr/bin/env python3
"""
Regression test: `termloop claude-teams` supports Claude's tmux teammate flow.
"""
    ...
    def handle(self, method: str, params: dict[str, object]) -> dict[str, object]:
        with self.lock:
            ...
            if method == "surface.send_text":
                return {"ok": True}
            raise RuntimeError(f"Unsupported fake termloop method: {method}")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cmux-claude-teams-seq-") as td:
        ...
        socket_path = tmp / "fake-termloop.sock"
        state = FakeCmuxState()
...
printf '%s\\n' "${TERMLOOP_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_LOG"
...
""",
        ...
        try:
            ...
            print("FAIL: `termloop claude-teams --version` timed out")
        ...
        if socket_value != str(socket_path):
            print(f"FAIL: expected TERMLOOP_SOCKET_PATH={socket_path}, got {socket_value!r}")
            return 1
    ...
    print("PASS: termloop claude-teams supports Claude's tmux teammate flow")
    return 0

...
typeset -g TERMLOOP_TOP='%F{4}%~%f'
typeset -g TERMLOOP_LAST_PROMPT=''
typeset -gi TERMLOOP_ASYNC_DONE=0
typeset -g TERMLOOP_ASYNC_FD=''
...
cmux_render_prompt() {
  ...
  PROMPT="${TERMLOOP_TOP}${prompt_newline}${cleaned_ps1:-%F{5}❯%f }"

  local expanded_prompt="${(S%%)PROMPT}"
  if [[ ${1:-} == precmd ]]; then
    print
  elif [[ $TERMLOOP_LAST_PROMPT != $expanded_prompt ]]; then
    zle && zle .reset-prompt
  fi
  typeset -g TERMLOOP_LAST_PROMPT=$expanded_prompt
}

cmux_async_ready() {
  emulate -L zsh
  local fd="${1:-$TERMLOOP_ASYNC_FD}"
  if [[ -n $fd ]]; then
    zle -F "$fd"
    exec {fd}<&-
  fi
  ...
  (( TERMLOOP_ASYNC_DONE )) && return
  TERMLOOP_ASYNC_DONE=1
  TERMLOOP_TOP='%F{4}%~%f %F{242}main%f%F{218}*%f %F{6}⇣⇡%f'
  cmux_render_prompt async
}

precmd() {
  TERMLOOP_ASYNC_DONE=0
  cmux_render_prompt precmd
}

cmux_line_init() {
  if (( !TERMLOOP_ASYNC_DONE )) && [[ -z $TERMLOOP_ASYNC_FD ]]; then
    exec {TERMLOOP_ASYNC_FD}< <(
      sleep 0.05
      printf 'ready\n'
    )
    zle -F "$TERMLOOP_ASYNC_FD" cmux_async_ready
  fi
}

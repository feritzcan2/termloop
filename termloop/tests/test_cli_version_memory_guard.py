#!/usr/bin/env python3
"""
Regression test: `termloop --version` must not scan huge sibling app lists just to
...
EXPECTED_STDOUT = "termloop 9.9.9 (999)"
...
def build_fixture(root: str, cli_path: str) -> str:
    app_path = os.path.join(root, "termloop.app")
    contents_path = os.path.join(app_path, "Contents")
    resources_path = os.path.join(contents_path, "Resources")
    bin_path = os.path.join(resources_path, "bin")
    os.makedirs(bin_path, exist_ok=True)

    fixture_cli = os.path.join(bin_path, "termloop")
    shutil.copy2(cli_path, fixture_cli)
    copy_runtime_frameworks(cli_path, contents_path)

    info = {
        "CFBundleExecutable": "termloop",
        "CFBundleIdentifier": "test.termloop.version-memory-guard",
...
def run_with_limits(cli_path: str, *args: str) -> dict[str, object]:
    env = dict(os.environ)
    env.pop("TERMLOOP_UPSTREAM_COMMIT", None)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="termloop-version-memory-guard-") as root:
        fixture_cli = build_fixture(root, cli_path)
        result = run_with_limits(fixture_cli, "--version")

    if result["failure_reason"]:
        print("FAIL: `termloop --version` exceeded runtime guard")
    ...
    print(
        "PASS: `termloop --version` exits within memory/time limits "

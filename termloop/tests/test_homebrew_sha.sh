...
# Catches issues like https://github.com/feritzcan2/termloop/issues/110 where a race
# condition caused the cask to contain the SHA of a 404 page instead of the DMG.
set -euo pipefail

CASK_FILE="$(dirname "$0")/../homebrew-cmux/Casks/termloop.rb"

if [ ! -f "$CASK_FILE" ]; then
  echo "SKIP: vendored homebrew-cmux checkout not present"
  exit 0
fi

        ...
        try:
            ...
            with conn:
                ...
                if not self.commands:
                    raise RuntimeError("termloop CLI never sent a command to the teardown test socket")
...
def main() -> int:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="termloop-claude-hook-stop-")
    try:
        root = Path(temp_dir.name)
        socket_path = str(root / "termloop.sock")
        ...
        env["TERMLOOP_WORKSPACE_ID"] = str(uuid.uuid4())
        env["TERMLOOP_SURFACE_ID"] = str(uuid.uuid4())

#!/usr/bin/env python3
"""
Regression test: `termloop claude-teams` injects the tmux-style auto-mode env.
"""
...
def run_claude_teams(
    ...
    with tempfile.TemporaryDirectory(prefix="termloop-claude-teams-env-") as td:
        tmp = Path(td)
...
printf '%s\\n' "${TERMLOOP_CLAUDE_TEAMS_TERMLOOP_BIN-__UNSET__}" > "$FAKE_TERMLOOP_BIN_LOG"
printf '%s\\n' "$@" > "$FAKE_ARGV_LOG"
...
printf '%s\\n' "${TERMLOOP_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_PATH_LOG"
printf '%s\\n' "${TERMLOOP_SOCKET_PASSWORD-__UNSET__}" > "$FAKE_SOCKET_PASSWORD_LOG"
...
""",
        ...
        env["FAKE_TERMLOOP_BIN_LOG"] = str(cmux_bin_log)
        ...
        if tmpdir is not None:
            env["TMPDIR"] = tmpdir
        explicit_socket_path = str(tmp / "explicit-termloop.sock")
        ...
        if tmux_path.startswith(str(real_bin)):
            print(f"FAIL: expected termloop tmux shim to shadow PATH, got {tmux_path!r}")
            raise SystemExit(1)

        cmux_bin_value = read_text(cmux_bin_log)
        if not cmux_bin_value or cmux_bin_value == "__UNSET__":
            print("FAIL: missing TERMLOOP_CLAUDE_TEAMS_TERMLOOP_BIN")
            raise SystemExit(1)

        if not os.path.exists(cmux_bin_value):
            print(f"FAIL: TERMLOOP_CLAUDE_TEAMS_TERMLOOP_BIN does not exist: {cmux_bin_value!r}")
            raise SystemExit(1)
        ...
        if socket_path_value != explicit_socket_path:
            print(f"FAIL: expected TERMLOOP_SOCKET_PATH={explicit_socket_path!r}, got {socket_path_value!r}")
            raise SystemExit(1)

        socket_password_value = read_text(socket_password_log)
        if socket_password_value != explicit_socket_password:
            print(
                "FAIL: expected TERMLOOP_SOCKET_PASSWORD to preserve the explicit CLI override, "
...
def main() -> int:
    ...
    if proc.returncode != 0:
        print("FAIL: `termloop claude-teams --version` exited non-zero")
    ...
    if proc.returncode != 0:
        print("FAIL: `termloop claude-teams --version` with existing heap flag exited non-zero")
        print(f"exit={proc.returncode}")
        print(f"stdout={proc.stdout.strip()}")
        print(f"stderr={proc.stderr.strip()}")
        return 1
    ...
    if proc.returncode != 0:
        print("FAIL: `termloop claude-teams --version` should still succeed when TMPDIR is unusable")
        print(f"exit={proc.returncode}")
        print(f"stdout={proc.stdout.strip()}")
        print(f"stderr={proc.stderr.strip()}")
        return 1
    ...
    print("PASS: termloop claude-teams restores child NODE_OPTIONS while injecting the auto-mode tmux env")
    return 0

...
def _git_stub() -> str:
    return textwrap.dedent(
        ...
        if [ "$1" = "remote" ] && [ "$2" = "get-url" ] && [ "$3" = "origin" ]; then
          printf 'https://github.com/feritzcan2/termloop.git\\n'
          exit 0
        fi
...
def _gh_stub() -> str:
    return textwrap.dedent(
        """\
        #!/bin/sh
        args_log="${TERMLOOP_TEST_GH_ARGS_LOG:?}"
        printf '%s\\n' "$*" >> "$args_log"
        exit 0
        """
    )
...
def _shell_command(kind: str, scenario: str) -> str:
    ...
    if kind == "zsh":
        return textwrap.dedent(
            f"""\
            source "$TERMLOOP_TEST_SCRIPT"
            _cmux_send() {{ print -r -- "$1" >> "$TERMLOOP_TEST_SEND_LOG"; }}
...
def _run_case(base: Path, *, shell: str, shell_args: list[str], script: Path, scenario: str) -> tuple[int, str]:
    ...
    repo = base / "repo"
    repo_git = repo / ".git"
    socket_path = base / "termloop.sock"
    ...
    env["TERMLOOP_TEST_SEND_LOG"] = str(send_log)
    env["TERMLOOP_TEST_GH_ARGS_LOG"] = str(gh_args_log)
    env["TERMLOOP_TEST_HEAD_FILE"] = str(head_file)
...
def main() -> int:
    ...
    cases = [
        ("zsh", ["-f", "-c"], root / "Resources" / "shell-integration" / "termloop-zsh-integration.zsh"),
        ("bash", ["--noprofile", "--norc", "-c"], root / "Resources" / "shell-integration" / "termloop-bash-integration.bash"),
    ]
    scenarios = [
        "prompt_does_not_call_gh",
        "merge_action",

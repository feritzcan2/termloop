#!/bin/bash
# Test script that sends keystrokes to termloop via AppleScript
# This tests the actual keyboard input path through the app

set -e

echo "=== termloop Keystroke Test ==="
echo ""

# Check if termloop is running
if ! pgrep -x "cmux" > /dev/null; then
    echo "Error: termloop is not running"
    echo "Please start termloop first"
    exit 1
fi
...
echo "Please check the termloop window to verify:"

...
def _preferred_worktree_slug():
    env_slug = os.environ.get("TERMLOOP_TAG") or os.environ.get("TERMLOOP_UPSTREAM_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/TermLoop DEV.app")
...
def _find_app():
    explicit = os.environ.get("TERMLOOP_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/termloop-{preferred_slug}/Build/Products/Debug/TermLoop DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/termloop-{preferred_slug}/Build/Products/Debug/TermLoop DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/TermLoop DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/termloop-*/Build/Products/Debug/TermLoop DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/termloop-*/Build/Products/Debug/TermLoop DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/termloop"
    )))
    candidates.extend(glob.glob("/tmp/termloop-*/Build/Products/Debug/termloop"))
    candidates.extend(glob.glob("/private/tmp/termloop-*/Build/Products/Debug/termloop"))
...
def _kill_cmux(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/TermLoop DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "TermLoop DEV"], capture_output=True)
    time.sleep(1.5)
...
def _launch_cmux(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"TERMLOOP_SOCKET_MODE={mode}"]
    launch_env = {
        "TERMLOOP_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    ...
    Inject a test via the shell rc file, then launch termloop in cmuxOnly mode.
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify TERMLOOP_SOCKET_MODE=allowAll bypasses ancestry check."""
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        _launch_cmux(
            ...
            socket_path,
            mode="password",
            extra_env={"TERMLOOP_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find termloop CLI binary")
            return result
...
def run_tests():
    ...
    app_path = _find_app()
    if not app_path:
        print("Error: Could not find TermLoop DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/termloop-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # Ensure termloop is running in cmuxOnly mode
    _kill_cmux(app_path)
    print("  Launching termloop in cmuxOnly mode...")
    _launch_cmux(app_path, socket_path, mode="cmuxOnly")
    ...
    # ── Cleanup: leave termloop in cmuxOnly mode ──
    _kill_cmux(app_path)
    _launch_cmux(app_path, socket_path, mode="cmuxOnly")

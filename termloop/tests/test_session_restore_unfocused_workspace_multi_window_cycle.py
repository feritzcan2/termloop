...
def _snapshot_path(bundle_id: str) -> Path:
    ...
    return Path.home() / "Library/Application Support/TermLoop" / f"session-{safe_bundle}.json"
...
def _socket_candidates(app_path: Path, preferred: Path) -> list[Path]:
    ...
    if app_name.startswith(prefix):
        ...
        candidates.append(Path(f"/tmp/termloop-debug-{slug}.sock"))
    deduped: list[Path] = []
    seen: set[str] = set()
...
def _kill_existing(app_path: Path) -> None:
    exe = app_path / "Contents" / "MacOS" / "TermLoop DEV"
...
def _launch(app_path: Path, preferred_socket_path: Path) -> Path:
    ...
    subprocess.run(
        [
            ...
            str(app_path),
            "--env",
            f"TERMLOOP_SOCKET_PATH={preferred_socket_path}",
            "--env",
...
def main() -> int:
    app_path_str = os.environ.get("TERMLOOP_APP_PATH", "").strip()
    if not app_path_str:
        print("SKIP: set TERMLOOP_APP_PATH to a built TermLoop DEV .app path")
        return 0
    app_path = Path(app_path_str)
    if not app_path.exists():
        print(f"SKIP: TERMLOOP_APP_PATH does not exist: {app_path}")
        return 0
    ...
    socket_path = Path(f"/tmp/termloop-mw-restore-{bundle_suffix}.sock")

    markers = {
        "w1_ws0": "TERMLOOP_MW_RESTORE_W1_WS0",
        "w1_ws1": "TERMLOOP_MW_RESTORE_W1_WS1",
        "w2_ws0": "TERMLOOP_MW_RESTORE_W2_WS0",
        "w2_ws1": "TERMLOOP_MW_RESTORE_W2_WS1",
    }
    failures: list[str] = []

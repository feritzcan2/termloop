...
A client library for programmatically controlling termloop via Unix socket.

Usage:
    from termloop import cmux
...
_DEFAULT_DEBUG_BUNDLE_ID = "com.termloop.app.debug"
...
def _default_bundle_id() -> str:
    override = os.environ.get("TERMLOOP_BUNDLE_ID")
    if override:
        return override

    tag = os.environ.get("TERMLOOP_TAG")
    if tag:
...
def _default_socket_path() -> str:
    ...
    if tag:
        ...
        tagged_candidates = [
            f"/tmp/termloop-debug-{slug}.sock",
            f"/tmp/termloop-{slug}.sock",
        ]
    ...
    override = os.environ.get("TERMLOOP_SOCKET_PATH")
    if override:
    ...
    tagged = glob.glob("/tmp/termloop-debug-*.sock")
...
class cmux:
    """Client for controlling termloop via Unix socket"""
    ...
    def connect(self) -> None:
        """Connect to the termloop socket"""

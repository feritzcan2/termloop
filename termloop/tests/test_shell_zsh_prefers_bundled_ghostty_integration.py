#!/usr/bin/env python3
"""
Regression: the termloop zsh wrapper should prefer a bundled Ghostty zsh
integration file in TERMLOOP_SHELL_INTEGRATION_DIR over the fallback integration
under GHOSTTY_RESOURCES_DIR.

Without this, tagged termloop builds can silently load Ghostty's installed app
...
def main() -> int:
    ...
    try:
        ...
        (bundled / "ghostty-integration.zsh").write_text(
            'echo "bundled" >> "$TERMLOOP_TEST_OUT"\n',
            encoding="utf-8",
        )
        (fallback / "shell-integration" / "zsh" / "ghostty-integration").write_text(
            'echo "fallback" >> "$TERMLOOP_TEST_OUT"\n',
            encoding="utf-8",
        )
        ...
        env["TERMLOOP_SHELL_INTEGRATION_DIR"] = str(bundled)
        env["GHOSTTY_RESOURCES_DIR"] = str(fallback)
        env["TERMLOOP_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        env["TERMLOOP_SHELL_INTEGRATION"] = "0"
        env["TERMLOOP_TEST_OUT"] = str(marker)

...
Run with a tagged instance to avoid unix socket conflicts:
  TERMLOOP_TAG=<tag> python3 tests/test_sidebar_ports.py
"""
...
from termloop import cmux, cmuxError  # noqa: E402
...
def _start_external_server(base: Path, port: int) -> subprocess.Popen:
    """
    Start an http.server outside termloop and ensure it is actually listening.
...
def main() -> int:
    tag = os.environ.get("TERMLOOP_TAG") or ""
    if not tag:
        print("Tip: set TERMLOOP_TAG=<tag> when running this test to avoid socket conflicts.")

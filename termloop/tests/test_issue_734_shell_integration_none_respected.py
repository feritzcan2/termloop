...
def _run_case(
    *,
    ...
    env["TERMLOOP_TEST_OUT"] = str(out_path)

    # Keep input deterministic and local to this test.
    for key in (
        "GHOSTTY_ZSH_ZDOTDIR",
        "TERMLOOP_ZSH_ZDOTDIR",
        "TERMLOOP_ORIGINAL_ZDOTDIR",
    ...
    if ghostty_enabled:
        ...
        env["TERMLOOP_ZSH_ZDOTDIR"] = str(orig_zdotdir)
...
def main() -> int:
    ...
    try:
        ...
        (resources / "shell-integration" / "zsh" / "ghostty-integration").write_text(
            'echo "sourced" >> "$TERMLOOP_TEST_OUT"\n',
            encoding="utf-8",
        )

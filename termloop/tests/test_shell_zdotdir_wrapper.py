...
The termloop zsh integration sets ZDOTDIR to the app's wrapper directory so zsh
...
def main() -> int:
    ...
    try:
        ...
        (orig / ".zshenv").write_text(
            'echo "$ZDOTDIR" > "$TERMLOOP_ZDOTDIR_TEST_OUTPUT"\n',
            encoding="utf-8",
        )
        ...
        env["TERMLOOP_ZDOTDIR_TEST_OUTPUT"] = str(seen_path)

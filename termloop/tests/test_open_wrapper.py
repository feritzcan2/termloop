...
if [[ -n "${FAKE_TERMLOOP_FAIL_URLS:-}" ]]; then
  IFS=',' read -r -a failures <<< "$FAKE_TERMLOOP_FAIL_URLS"
...
""",
        ...
        env = os.environ.copy()
        env["TERMLOOP_SOCKET_PATH"] = "/tmp/termloop-open-wrapper-test.sock"
        env["TERMLOOP_BUNDLE_ID"] = "com.termloop.app.debug.test"
        env["TERMLOOP_OPEN_WRAPPER_SYSTEM_OPEN"] = str(system_open)
        env["TERMLOOP_OPEN_WRAPPER_DEFAULTS"] = str(defaults)
        env["FAKE_OPEN_LOG"] = str(open_log)
        env["FAKE_TERMLOOP_LOG"] = str(cmux_log)
        if python_bin is None:
            env.pop("TERMLOOP_OPEN_WRAPPER_PYTHON3", None)
        else:
            env["TERMLOOP_OPEN_WRAPPER_PYTHON3"] = python_bin
        ...
        if fail_urls:
            env["FAKE_TERMLOOP_FAIL_URLS"] = ",".join(fail_urls)
        else:
            env.pop("FAKE_TERMLOOP_FAIL_URLS", None)
...
def test_toggle_disabled_passthrough(failures: list[str]) -> None:
    ...
    expect(cmux_log == [], f"toggle off: termloop should not be called, got {cmux_log}", failures)
...
def test_toggle_disabled_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        cmux_log == [],
        f"toggle off (case-insensitive): termloop should not be called, got {cmux_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_whitelist_miss_passthrough(failures: list[str]) -> None:
    ...
    expect(cmux_log == [], f"whitelist miss: termloop should not be called, got {cmux_log}", failures)
...
def test_whitelist_match_routes_to_cmux(failures: list[str]) -> None:
    ...
    expect(cmux_log == [f"browser open {url}"], f"whitelist match: unexpected termloop log {cmux_log}", failures)
...
def test_external_invalid_regex_is_ignored_silently(failures: list[str]) -> None:
    ...
    expect(
        cmux_log == [f"browser open {url}"],
        f"external invalid regex: expected termloop open for {url}, got {cmux_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_partial_failures_only_fallback_failed_urls(failures: list[str]) -> None:
    ...
    expect(
        ...
        f"partial failure: termloop log mismatch {cmux_log}",
        failures,
    )
    expect(
...
def test_legacy_toggle_fallback_passthrough(failures: list[str]) -> None:
    ...
    expect(cmux_log == [], f"legacy fallback: termloop should not be called, got {cmux_log}", failures)
...
def test_legacy_toggle_fallback_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        cmux_log == [],
        f"legacy fallback (case-insensitive): termloop should not be called, got {cmux_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_uppercase_scheme_routes_to_cmux(failures: list[str]) -> None:
    ...
    expect(cmux_log == [f"browser open {url}"], f"uppercase scheme: unexpected termloop log {cmux_log}", failures)
...
def test_local_html_file_routes_to_cmux(failures: list[str]) -> None:
    ...
    expect(len(cmux_log) == 1, f"local html file: expected exactly one termloop call, got {cmux_log}", failures)
    if cmux_log:
        expect(
        ...
        expect(
            ...
            f"local html file: expected URL-encoded filename in termloop target, got {cmux_log[0]}",
            failures,
        )


def test_file_url_html_routes_to_cmux(failures: list[str]) -> None:
    url = "file:///tmp/termloop-open-wrapper-fixture.html"
    open_log, cmux_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
        whitelist="",
    ...
    expect(cmux_log == [f"browser open {url}"], f"file url html: unexpected termloop log {cmux_log}", failures)
...
def test_file_url_html_routes_to_cmux_without_python_binary(failures: list[str]) -> None:
    ...
    expect(
        cmux_log == [f"browser open {url}"],
        f"file url html no-python fallback: unexpected termloop log {cmux_log}",
        failures,
    )
...
def test_local_html_file_routes_to_cmux_without_python_binary(failures: list[str]) -> None:
    ...
    expect(
        len(cmux_log) == 1,
        f"local html no-python fallback: expected exactly one termloop call, got {cmux_log}",
        failures,
    )
    if cmux_log:
        expect(
...
def test_domain_like_html_argument_passthrough(failures: list[str]) -> None:
    ...
    expect(
        cmux_log == [],
        f"domain-like html argument: termloop should not be called, got {cmux_log}",
        failures,
    )
    expect(
        open_log == [arg],
...
def test_non_file_scheme_html_passthrough(failures: list[str]) -> None:
    ...
    expect(cmux_log == [], f"non-file scheme html: termloop should not be called, got {cmux_log}", failures)
...
def test_mailto_html_passthrough(failures: list[str]) -> None:
    ...
    expect(cmux_log == [], f"mailto html: termloop should not be called, got {cmux_log}", failures)
...
def test_local_non_html_file_passthrough(failures: list[str]) -> None:
    ...
    expect(cmux_log == [], f"local non-html file: termloop should not be called, got {cmux_log}", failures)
...
def test_unicode_whitelist_matches_punycode_url(failures: list[str]) -> None:
    ...
    expect(cmux_log == [f"browser open {url}"], f"unicode whitelist: unexpected termloop log {cmux_log}", failures)
...
def test_punycode_whitelist_matches_unicode_url(failures: list[str]) -> None:
    ...
    expect(cmux_log == [f"browser open {url}"], f"punycode whitelist: unexpected termloop log {cmux_log}", failures)

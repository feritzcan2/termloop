#!/usr/bin/env python3
"""Regression test: CLI should auto-discover tagged debug sockets from TERMLOOP_TAG."""
...
def main() -> int:
    ...
    socket_path = f"/tmp/termloop-debug-{tag}.sock"
    ...
    env = os.environ.copy()
    env["TERMLOOP_SOCKET_PATH"] = "/tmp/termloop.sock"
    env["TERMLOOP_TAG"] = tag
    env["TERMLOOP_CLI_SENTRY_DISABLED"] = "1"
    env["TERMLOOP_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    try:
        ...
        print(f"FAIL: invoking termloop ping failed: {exc}")
        return 1
    finally:
    ...
    if proc.returncode != 0:
        print("FAIL: termloop ping returned non-zero status")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return 1

    if proc.stdout.strip() != "PONG":
        print("FAIL: termloop ping did not use auto-discovered socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return 1

    print("PASS: termloop ping auto-discovers tagged socket from TERMLOOP_TAG")
    return 0

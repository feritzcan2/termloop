...
if ! grep -Fq 'com.termloop.app.nightly.universal' "$WORKFLOW_FILE"; then
  echo "FAIL: nightly workflow must set a distinct .universal bundle ID"
  exit 1
fi

if ! grep -Fq 'https://github.com/feritzcan2/termloop/releases/download/nightly/appcast-universal.xml' "$WORKFLOW_FILE"; then

...
def main() -> int:
    ...
    integration_script = root / "Resources" / "shell-integration" / "termloop-zsh-integration.zsh"
    ...
    try:
        ...
        env["TERMLOOP_RESTORE_SCROLLBACK_FILE"] = str(replay_file)
        env["TERMLOOP_TEST_INTEGRATION_SCRIPT"] = str(integration_script)

        result = subprocess.run(
            ["/bin/zsh", "-f", "-c", 'source "$TERMLOOP_TEST_INTEGRATION_SCRIPT"'],
            env=env,
            capture_output=True,
            text=True,
            timeout=5,

    ...
    env.update(
        {
            ...
            "TERMLOOP_TEST_BIN_DIR": str(bin_dir),
        }
    )
    command = f"""
source "{integration_path}"
PATH="$TERMLOOP_TEST_BIN_DIR:$PATH"
hash -r 2>/dev/null || true
: > "{log_path}"
rm -f "{state_path}"
_TERMLOOP_TTY_NAME={tty_name}
_TERMLOOP_TTY_REPORTED=0
_cmux_report_tty_once || true
first="${{_TERMLOOP_TTY_REPORTED}}:$(wc -l < "{log_path}" | tr -d ' ')"
_cmux_report_tty_once || true
second="${{_TERMLOOP_TTY_REPORTED}}:$(wc -l < "{log_path}" | tr -d ' ')"
printf '%s\\n' "$first|$second"
""".strip()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cmux-ssh-relay-report-tty-retry-") as td:
        ...
        _write_executable(
            ...
            """#!/bin/sh
count=0
if [ -r "$TERMLOOP_TEST_STATE" ]; then
    count=$(cat "$TERMLOOP_TEST_STATE")
fi
count=$((count + 1))
printf '%s' "$count" > "$TERMLOOP_TEST_STATE"
printf '%s\n' "$*" >> "$TERMLOOP_TEST_LOG"
if [ "$count" -eq 1 ]; then
...
""",
        ...
        cases = [
            (
                "zsh",
                ["-f", "-c"],
                SHELL_DIR / "termloop-zsh-integration.zsh",
                "ttys777",
            ),
            (
                "bash",
                ["--noprofile", "--norc", "-c"],
                SHELL_DIR / "termloop-bash-integration.bash",
                "ttys888",
            ),
        ]

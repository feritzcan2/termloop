...
Validates:
1) `termloop codex-hook session-start` records the inferred agent root PID.
...
from claude_teams_test_utils import resolve_termloop_cli
...
host = os.environ["TERMLOOP_TEST_BIND_HOST"]
port = int(os.environ["TERMLOOP_TEST_PORT"])
cli_path = os.environ["TERMLOOP_TEST_CLI_PATH"]
socket_path = os.environ["TERMLOOP_TEST_SOCKET_PATH"]
workspace_id = os.environ["TERMLOOP_WORKSPACE_ID"]
surface_id = os.environ["TERMLOOP_SURFACE_ID"]
state_path = os.environ["TERMLOOP_CLAUDE_HOOK_STATE_PATH"]
session_id = os.environ["TERMLOOP_TEST_SESSION_ID"]
cwd = os.environ["TERMLOOP_TEST_CWD"]
ready_file = Path(os.environ["TERMLOOP_TEST_READY_FILE"])
start_file = Path(os.environ["TERMLOOP_TEST_START_FILE"])
server_pid_file = Path(os.environ["TERMLOOP_TEST_SERVER_PID_FILE"])
server_log_file = Path(os.environ["TERMLOOP_TEST_SERVER_LOG_FILE"])

hook_env = os.environ.copy()
hook_env["TERMLOOP_SOCKET_PATH"] = socket_path
hook_env["TERMLOOP_WORKSPACE_ID"] = workspace_id
hook_env["TERMLOOP_SURFACE_ID"] = surface_id
hook_env["TERMLOOP_CLAUDE_HOOK_STATE_PATH"] = state_path
...
""",
    ...
    env = os.environ.copy()
    env["TERMLOOP_TEST_BIND_HOST"] = _PREFERRED_BIND_HOST
    env["TERMLOOP_TEST_PORT"] = str(port)
    env["TERMLOOP_TEST_CLI_PATH"] = cli_path
    env["TERMLOOP_TEST_SOCKET_PATH"] = socket_path
    env["TERMLOOP_WORKSPACE_ID"] = workspace_id
    env["TERMLOOP_SURFACE_ID"] = surface_id
    env["TERMLOOP_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
    env["TERMLOOP_TEST_SESSION_ID"] = session_id
    env["TERMLOOP_TEST_CWD"] = str(cwd)
    env["TERMLOOP_TEST_READY_FILE"] = str(ready_file)
    env["TERMLOOP_TEST_START_FILE"] = str(start_file)
    env["TERMLOOP_TEST_SERVER_PID_FILE"] = str(server_pid_file)
    env["TERMLOOP_TEST_SERVER_LOG_FILE"] = str(server_log_file)

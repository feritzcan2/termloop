...
Requires termloop to be running with socket control enabled.
"""
...
def get_cmux_pid() -> Optional[int]:
    """Get the PID of the running termloop process."""
    socket_path = os.environ.get("TERMLOOP_SOCKET_PATH")
    if not socket_path:
        # Ask cmux.py to resolve default socket path (supports TERMLOOP_TAG and last-socket file).
        try:
    ...
    if result.returncode != 0:
        # Try DEV build
        result = subprocess.run(
            ["pgrep", "-f", r"TermLoop DEV\.app/Contents/MacOS/cmux"],
            capture_output=True,
            text=True,
        )
    if result.returncode != 0:
...
def test_cpu_after_popover_close(client: cmux, pid: int) -> tuple[bool, str]:
    ...
    # Ensure the correct termloop instance is frontmost (tag-safe).
...
def main():
    ...
    pid = get_cmux_pid()
    if pid is None:
        print("\n❌ SKIP: termloop is not running")
        return 0

    print(f"\nFound termloop process: PID {pid}")
    ...
    try:
        ...
        print("\n❌ SKIP: Could not connect to termloop socket")
        print("Tip: set TERMLOOP_TAG=<tag> or TERMLOOP_SOCKET_PATH=<path> to target a tagged instance.")
        return 0

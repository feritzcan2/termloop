...
<p align="center">
  <a href="https://github.com/feritzcan2/termloop/releases/latest/download/termloop-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="Завантажити TermLoop для macOS" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="Скріншот TermLoop" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ Демо-відео</a> · <a href="https://termloop.ai/blog/zen-of-cmux">Філософія TermLoop</a>
</p>
...
- **Користувацькі команди** — Визначте дії для конкретного проєкту у [`cmux.json`](https://termloop.ai/docs/custom-commands), які запускаються з палітри команд
...
Відкрийте `.dmg` та перетягніть TermLoop до папки Applications. TermLoop автоматично оновлюється через Sparkle, тому завантажити потрібно лише один раз.
...
Я спробував кілька оркестраторів програмування, але більшість з них були додатками на Electron/Tauri, і продуктивність мене турбувала. Я також просто віддаю перевагу терміналу, оскільки GUI-оркестратори прив'язують вас до свого робочого процесу. Тому я створив TermLoop як нативний додаток macOS на Swift/AppKit. Він використовує libghostty для рендерингу терміналу та читає вашу існуючу конфігурацію Ghostty для тем, шрифтів та кольорів.
...
TermLoop не нав'язує розробникам, як використовувати їхні інструменти. Це термінал і браузер із CLI, а решта — за вами.

TermLoop — це примітив, а не рішення. Він дає вам термінал, браузер, сповіщення, робочі простори, розділення, вкладки та CLI для керування всім цим. TermLoop не змушує вас дотримуватися нав'язаного способу використання агентів програмування. Те, що ви створите з цих примітивів — ваше.
...
Для додаткової інформації про налаштування TermLoop [перейдіть до нашої документації](https://termloop.ai/docs/getting-started?utm_source=readme).
...
[Завантажити TermLoop NIGHTLY](https://github.com/feritzcan2/termloop/releases/download/nightly/termloop-nightly-macos.dmg)

TermLoop NIGHTLY — це окремий додаток з власним bundle ID, тому він працює поруч зі стабільною версією. Збирається автоматично з останнього коміту `main` та автоматично оновлюється через власний канал Sparkle.

Повідомляйте про помилки нічних збірок на [GitHub Issues](https://github.com/feritzcan2/termloop/issues) або в [#nightly-bugs у Discord](https://github.com/feritzcan2/termloop/discussions).

## Відновлення сесії (поточна поведінка)

При перезапуску TermLoop наразі відновлює лише макет та метадані додатку:
- Макет вікон/робочих просторів/панелей
- Робочі директорії
- Буфер прокрутки терміналу (наскільки можливо)
- URL браузера та історію навігації

TermLoop **не** відновлює стан активних процесів усередині термінальних додатків. Наприклад, активні сесії Claude Code/tmux/vim поки що не відновлюються після перезапуску.
...
<a href="https://star-history.com/#feritzcan2/termloop&Date">
 <picture>
   ...
   <img alt="Діаграма історії зірок" src="https://api.star-history.com/svg?repos=feritzcan2/termloop&type=Date" width="600" />
 </picture>
</a>
...
- Приєднуйтесь до обговорень у [Discord](https://github.com/feritzcan2/termloop/discussions)
- Створюйте та беріть участь у [GitHub issues](https://github.com/feritzcan2/termloop/issues) та [обговореннях](https://github.com/feritzcan2/termloop/discussions)
- Розкажіть нам, що ви створюєте з TermLoop
...
TermLoop є безкоштовним, з відкритим кодом і завжди буде таким. Якщо ви хочете підтримати розробку та отримати ранній доступ до того, що буде далі:

**[Отримати Founder's Edition](https://buy.stripe.com/3cI00j2Ld0it5OU33r5EY0q)**

- **Пріоритетні запити на функції/виправлення помилок**
- **Ранній доступ: TermLoop AI, що надає контекст для кожного робочого простору, вкладки та панелі**
...
TermLoop є відкритим програмним забезпеченням під ліцензією [GPL-3.0-or-later](LICENSE).

Якщо ваша організація не може дотримуватися GPL, доступна комерційна ліцензія. Зв'яжіться з [feritzcan93@gmail.com](mailto:feritzcan93@gmail.com) для деталей.

...
private enum TabShortcutHintWidthCache {
    ...
    static func width(for label: String, fontSize: CGFloat) -> CGFloat {
        let key = Key(label: label, fontSize: Int((fontSize * 100).rounded()))
        lock.lock()
...
struct TabItemView: View, Equatable {
    static func == (lhs: TabItemView, rhs: TabItemView) -> Bool {
        lhs.tab.id == rhs.tab.id &&
            ...
            lhs.tab.hasCustomTitle == rhs.tab.hasCustomTitle &&
            lhs.tab.icon == rhs.tab.icon &&
            lhs.tab.iconImageData == rhs.tab.iconImageData &&
            lhs.tab.kind == rhs.tab.kind &&
            lhs.tab.isDirty == rhs.tab.isDirty &&
            lhs.tab.showsNotificationBadge == rhs.tab.showsNotificationBadge &&
            lhs.tab.isLoading == rhs.tab.isLoading &&
            lhs.tab.isPinned == rhs.tab.isPinned &&
            lhs.isSelected == rhs.isSelected &&
            lhs.showsZoomIndicator == rhs.showsZoomIndicator &&
            lhs.appearance == rhs.appearance &&
            lhs.saturation == rhs.saturation &&
            lhs.controlShortcutDigit == rhs.controlShortcutDigit &&
            lhs.showsControlShortcutHint == rhs.showsControlShortcutHint &&
            lhs.shortcutModifierSymbol == rhs.shortcutModifierSymbol &&
            lhs.contextMenuState == rhs.contextMenuState
    }
    ...
    private func shortcutHintWidth(for label: String) -> CGFloat {
        TabShortcutHintWidthCache.width(for: label, fontSize: accessoryFontSize)
    }
...
private struct TabItemContextMenuContent: View {
    let state: TabContextMenuState
    let onAction: (TabContextAction) -> Void
    ...
    var body: some View {
        ...
        contextButton("Close Tabs to Left", action: .closeToLeft)
            .disabled(!state.canCloseToLeft)

        contextButton("Close Tabs to Right", action: .closeToRight)
            .disabled(!state.canCloseToRight)
        ...
        if state.isTerminal {
            localizedContextButton(
                ...
                .disabled(!state.canMoveToLeftPane)
            ...
            localizedContextButton(
                ...
                .disabled(!state.canMoveToRightPane)
        }
        ...
        if state.hasSplits {
            contextButton(
                state.isZoomed ? "Exit Zoom" : "Zoom Pane",
                action: .toggleZoom
            )
        }
        ...
        if state.isUnread {
            contextButton("Mark Tab as Read", action: .markAsRead)
                .disabled(!state.canMarkAsRead)
        } else {
            contextButton("Mark Tab as Unread", action: .markAsUnread)
                .disabled(!state.canMarkAsUnread)
        }
    }

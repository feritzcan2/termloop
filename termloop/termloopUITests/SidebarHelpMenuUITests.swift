...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        let app = XCUIApplication()
        app.launchEnvironment["TERMLOOP_UI_TEST_MODE"] = "1"
        app.launchEnvironment["TERMLOOP_UI_TEST_FEED_URL"] = "https://cmux.test/appcast.xml"
        app.launchEnvironment["TERMLOOP_UI_TEST_FEED_MODE"] = "available"
        app.launchEnvironment["TERMLOOP_UI_TEST_UPDATE_VERSION"] = "9.9.9"
        app.launchEnvironment["TERMLOOP_UI_TEST_AUTO_ALLOW_PERMISSION"] = "1"
        launchAndActivate(app)
    ...
    func testHelpMenuSendFeedbackOpensComposerSheet() {
        ...
        XCTAssertTrue(
            app.staticTexts[
                "A human will read this! You can also reach us at feritzcan93@gmail.com."
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["TERMLOOP_UI_TEST_MODE"] = "1"
        app.launchEnvironment["TERMLOOP_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/termloop-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/termloop-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "termloop.settings" && data["settingsWindowIsKey"] == "1"
        }

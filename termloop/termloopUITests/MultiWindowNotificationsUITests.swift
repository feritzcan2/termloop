...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/termloop-ui-test-multi-window-notifs-\(UUID().uuidString).json"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["TERMLOOP_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["TERMLOOP_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["TERMLOOP_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["TERMLOOP_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["TERMLOOP_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["TERMLOOP_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["TERMLOOP_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["TERMLOOP_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["TERMLOOP_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["TERMLOOP_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["TERMLOOP_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["TERMLOOP_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        guard let bundledCLIPath = resolveCmuxCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled termloop CLI for notify regression test")
            return
        }
        ...
        XCTAssertTrue(
            ...
            "Expected termloop to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected termloop to remain in background after bundled `cmux notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
    ...
    private func runCmuxCommand(
        ...
        environment["TERMLOOP_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)
        ...
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled termloop CLI"
            )
        }
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled termloop CLI command failed without an executable path"
            )
        }
    ...
    private func resolveCmuxCLIPaths(strategy: CmuxCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["TERMLOOP_UI_TEST_CLI_PATH", "TERMLOOP_CLI"] {
        ...
        candidates.append("/tmp/termloop-\(launchTag)/Build/Products/Debug/TermLoop DEV.app/Contents/Resources/bin/termloop")
        candidates.append("/tmp/termloop-\(launchTag)/Build/Products/Debug/termloop.app/Contents/Resources/bin/termloop")
        if strategy == .any {
            candidates.append("/tmp/termloop-\(launchTag)/Build/Products/Debug/termloop")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        candidates.append("\(productsDir)/TermLoop DEV.app/Contents/Resources/bin/termloop")
        candidates.append("\(productsDir)/TermLoop.app/Contents/Resources/bin/termloop")
        if strategy == .any {
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/termloop")
                .path
    ...
    private func executeCmuxCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run termloop command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested TERMLOOP_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/termloop-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/termloop-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/termloop.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("termloop.sock", isDirectory: false)
            .path ?? "/tmp/termloop.sock"
    }

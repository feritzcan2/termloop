...
final class DisplayResolutionRegressionUITests: XCTestCase {
    private let defaultDisplayHarnessManifestPath = "/tmp/termloop-ui-test-display-harness.json"
    ...
    private let prelaunchManifestPath = "/tmp/termloop-ui-test-prelaunch.json"
    ...
    private func loadPrebuiltHelperBinaryPath(_ env: [String: String]) -> String? {
        guard let helperBinaryPath = env["TERMLOOP_UI_TEST_DISPLAY_HELPER_BINARY_PATH"],
    ...
    private func loadExternalHarnessFromEnvironment(_ env: [String: String]) -> ExternalDisplayHarness? {
        guard let readyPath = env["TERMLOOP_UI_TEST_DISPLAY_READY_PATH"], !readyPath.isEmpty,
              let displayIDPath = env["TERMLOOP_UI_TEST_DISPLAY_ID_PATH"], !displayIDPath.isEmpty,
              let donePath = env["TERMLOOP_UI_TEST_DISPLAY_DONE_PATH"], !donePath.isEmpty else {
            return nil
        }
        ...
        return ExternalDisplayHarness(
            ...
            startPath: env["TERMLOOP_UI_TEST_DISPLAY_START_PATH"],
            donePath: donePath,
            logPath: env["TERMLOOP_UI_TEST_DISPLAY_LOG_PATH"],
            helperBinaryPath: nil
        )
    }

    private func loadExternalHarnessFromManifest(_ env: [String: String]) -> ExternalDisplayHarness? {
        let manifestPath = env["TERMLOOP_UI_TEST_DISPLAY_HARNESS_MANIFEST_PATH"] ?? defaultDisplayHarnessManifestPath
    ...
    private func resolveAppBinaryPath() throws -> String {
        ...
        //   .../Build/Products/Debug/termloopUITests-Runner.app/Contents/PlugIns/termloopUITests.xctest
        // The app binary is at:
        //   .../Build/Products/Debug/TermLoop DEV.app/Contents/MacOS/TermLoop DEV
        let testBundle = Bundle(for: Self.self)
        let productsDir = testBundle.bundleURL
            .deletingLastPathComponent()  // -> .../Contents/PlugIns
            .deletingLastPathComponent()  // -> .../Contents
            .deletingLastPathComponent()  // -> .../termloopUITests-Runner.app
            .deletingLastPathComponent()  // -> .../Debug
        let binaryPath = productsDir
            .appendingPathComponent("TermLoop DEV.app")
            .appendingPathComponent("Contents/MacOS/TermLoop DEV")
            .path
    ...
    private func launchEnvironment(targetDisplayID: String) -> [String: String] {
        [
            "TERMLOOP_UI_TEST_MODE": "1",
            "TERMLOOP_UI_TEST_DIAGNOSTICS_PATH": diagnosticsPath,
            "TERMLOOP_UI_TEST_DISPLAY_RENDER_STATS": "1",
            "TERMLOOP_UI_TEST_TARGET_DISPLAY_ID": targetDisplayID,
            "TERMLOOP_TAG": launchTag,
        ]
    }

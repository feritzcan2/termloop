...
<p align="center">
  <a href="https://github.com/feritzcan2/termloop/releases/latest/download/termloop-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="Скачать TermLoop для macOS" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="Скриншот TermLoop" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ Демо-видео</a> · <a href="https://termloop.ai/blog/zen-of-cmux">The Zen of TermLoop</a>
</p>
...
- **Пользовательские команды** — Определяйте команды для конкретного проекта в [`cmux.json`](https://termloop.ai/docs/custom-commands), которые запускаются из палитры команд
...
Откройте файл `.dmg` и перетащите TermLoop в папку «Программы». TermLoop автоматически обновляется через Sparkle, поэтому скачивание требуется только один раз.
...
TermLoop не навязывает разработчикам, как использовать свои инструменты. Это терминал и браузер с CLI, а остальное зависит от вас.

TermLoop — это примитив, а не решение. Он даёт вам терминал, браузер, уведомления, рабочие пространства, разделения, вкладки и CLI для управления всем этим. TermLoop не заставляет вас использовать агентов для кодирования определённым образом. То, что вы построите из этих примитивов, принадлежит вам.
...
Подробнее о настройке TermLoop читайте в [нашей документации](https://termloop.ai/docs/getting-started?utm_source=readme).
...
[Скачать TermLoop NIGHTLY](https://github.com/feritzcan2/termloop/releases/download/nightly/termloop-nightly-macos.dmg)

TermLoop NIGHTLY — это отдельное приложение с собственным идентификатором пакета, поэтому оно работает параллельно со стабильной версией. Собирается автоматически из последнего коммита `main` и обновляется через собственный канал Sparkle.

## Восстановление сессии (текущее поведение)

При перезапуске TermLoop в настоящее время восстанавливает только макет приложения и метаданные:
- Макет окон/рабочих пространств/панелей
- Рабочие каталоги
- Scrollback терминала (по возможности)
- URL браузера и история навигации

TermLoop **не** восстанавливает состояние живых процессов внутри терминальных приложений. Например, активные сессии Claude Code/tmux/vim пока не возобновляются после перезапуска.
...
- Присоединяйтесь к обсуждению в [Discord](https://github.com/feritzcan2/termloop/discussions)
- Создавайте и участвуйте в [GitHub issues](https://github.com/feritzcan2/termloop/issues) и [обсуждениях](https://github.com/feritzcan2/termloop/discussions)
- Расскажите нам, что вы создаёте с помощью TermLoop
...
TermLoop бесплатен, с открытым исходным кодом и всегда будет таким. Если вы хотите поддержать разработку и получить ранний доступ к будущим возможностям:
...
- **Ранний доступ: TermLoop AI, который даёт контекст по каждому рабочему пространству, вкладке и панели**
...
TermLoop распространяется с открытым исходным кодом по лицензии [GPL-3.0-or-later](LICENSE).

Если ваша организация не может соблюдать условия GPL, доступна коммерческая лицензия. Свяжитесь с [feritzcan93@gmail.com](mailto:feritzcan93@gmail.com) для получения подробностей.

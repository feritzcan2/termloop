    ...
    with cmux(SOCKET_PATH) as c:
        ...
        try:
            ...
            payload = c._call(
                "workspace.create",
                {
                    "initial_env": {"TERMLOOP_INITIAL_ENV_TOKEN": token},
                },
            ) or {}
            ...
            c.send("printf 'TERMLOOP_ENV_CHECK=%s\\n' \"$TERMLOOP_INITIAL_ENV_TOKEN\"\\n")
            text = _wait_for_text(c, created_workspace, f"TERMLOOP_ENV_CHECK={token}")
            _must(
                f"TERMLOOP_ENV_CHECK={token}" in text,

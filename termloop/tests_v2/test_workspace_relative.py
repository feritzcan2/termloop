#!/usr/bin/env python3
"""Regression: CLI commands are workspace-relative via TERMLOOP_WORKSPACE_ID.

Tests that when TERMLOOP_WORKSPACE_ID is set, CLI commands target that workspace
...
def test_list_panels_workspace_relative(c: cmux, cli: str) -> None:
    ...
    payload_env = _run_cli_json(
        cli, ["list-panels"],
        env_overrides={"TERMLOOP_WORKSPACE_ID": ws_a["id"]}
    )
...
def test_send_workspace_relative(c: cmux, cli: str) -> None:
    """send with TERMLOOP_WORKSPACE_ID env var targets that workspace's surface."""
    ws_result = c._call("workspace.list")
    workspaces = ws_result.get("workspaces", [])
    _must(len(workspaces) >= 1, "Need at least 1 workspace")
    ...
    output = _run_cli(
        cli, ["send", " "],
        env_overrides={"TERMLOOP_WORKSPACE_ID": ws["id"]}
    )

...
def _run_cli(cli: str, args: List[str], env_overrides: Optional[Dict[str, str]] = None) -> str:
    env = dict(os.environ)
    # Keep this test deterministic when running from inside another termloop shell.
...
def main() -> int:
    ...
    with cmux(SOCKET_PATH) as c:
        ...
        _run_cli(
            ...
            env_overrides={"TERMLOOP_WORKSPACE_ID": ws_id},
        )
        _must(
            _workspace_title(c, ws_id) == env_title,
            "cmux rename-workspace should default to TERMLOOP_WORKSPACE_ID",
        )

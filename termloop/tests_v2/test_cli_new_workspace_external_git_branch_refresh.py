...
def _resolve_socket_path() -> str:
    socket_path = os.environ.get("TERMLOOP_SOCKET", "").strip()
    if not socket_path:
        raise cmuxError("TERMLOOP_SOCKET is required (expected /tmp/termloop-debug-<tag>.sock)")
    if not re.fullmatch(r"/tmp/termloop-debug-[^/]+\.sock", socket_path):
        raise cmuxError(f"TERMLOOP_SOCKET must be a tagged debug socket, got: {socket_path!r}")
    return socket_path
...
def _find_cli_binary() -> str:
    ...
    candidates = glob.glob(
        os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/termloop"),
        recursive=True,
    )

...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set TERMLOOP_SSH_TEST_HOST to run remote browser move/proxy regression")
        return 0
    ...
    marker_file = f"TERMLOOP_REMOTE_PROXY_MOVE_{stamp}.txt"
    marker_body = f"TERMLOOP_REMOTE_PROXY_BODY_{stamp}"
    ready_token = f"TERMLOOP_HTTP_READY_{stamp}"
    default_web_port = 20000 + (os.getpid() % 5000)
    ssh_web_port = int(os.environ.get("TERMLOOP_SSH_TEST_WEB_PORT", str(default_web_port)))
    ...
    try:
        with cmux(SOCKET_PATH) as client:
            ...
            server_script = (
                ...
                f"python3 -m http.server {ssh_web_port} --directory /tmp >/tmp/termloop-remote-browser-proxy-{stamp}.log 2>&1 & "

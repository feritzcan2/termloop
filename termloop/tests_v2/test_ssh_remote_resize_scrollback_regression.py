...
SOCKET_PATH = os.environ.get("TERMLOOP_SOCKET", "/tmp/termloop.sock")
SSH_HOST = os.environ.get("TERMLOOP_SSH_TEST_HOST", "").strip()
SSH_PORT = os.environ.get("TERMLOOP_SSH_TEST_PORT", "").strip()
SSH_IDENTITY = os.environ.get("TERMLOOP_SSH_TEST_IDENTITY", "").strip()
SSH_OPTIONS_RAW = os.environ.get("TERMLOOP_SSH_TEST_OPTIONS", "").strip()
LS_ENTRY_COUNT = int(os.environ.get("TERMLOOP_SSH_TEST_LS_COUNT", "320"))
RESIZE_ITERATIONS = int(os.environ.get("TERMLOOP_SSH_TEST_RESIZE_ITERATIONS", "48"))
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set TERMLOOP_SSH_TEST_HOST to run remote resize scrollback regression")
        return 0
    if LS_ENTRY_COUNT < 64:
        print("SKIP: TERMLOOP_SSH_TEST_LS_COUNT must be >= 64 for meaningful scrollback coverage")
        return 0
    ...
    try:
        with cmux(SOCKET_PATH) as client:
            ...
            ls_entries = [f"TERMLOOP_REMOTE_RESIZE_LS_{stamp}_{index:04d}.txt" for index in range(1, LS_ENTRY_COUNT + 1)]
            ls_start = f"TERMLOOP_REMOTE_RESIZE_LS_START_{stamp}"
            ls_end = f"TERMLOOP_REMOTE_RESIZE_LS_END_{stamp}"

            ls_prefix = f"TERMLOOP_REMOTE_RESIZE_LS_{stamp}_"
            ls_script = (
                "tmpdir=$(mktemp -d); "
                f"echo {ls_start}; "
            ...
            post_token = f"TERMLOOP_REMOTE_RESIZE_POST_{secrets.token_hex(6)}"
        ...
        print(
            "PASS: termloop ssh split+resize churn preserved large pre-resize scrollback "

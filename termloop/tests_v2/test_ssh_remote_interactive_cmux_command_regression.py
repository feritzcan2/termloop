#!/usr/bin/env python3
"""Regression: interactive `cmux ssh` shells must resolve `termloop` to the relay wrapper."""
...
def _wait_shell_ready(client: cmux, surface_id: str, timeout: float = 20.0) -> None:
    token = f"__TERMLOOP_SHELL_READY_{secrets.token_hex(6)}__"
...
def _assert_no_login_profile_noise(text: str) -> None:
    _must(
        "/Users/termloop/.profile:" not in text,
...
def _run_remote_shell_command(client: cmux, surface_id: str, command: str, timeout: float = 12.0) -> tuple[int, str, str]:
    token = f"__TERMLOOP_REMOTE_CMD_{secrets.token_hex(6)}__"
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set TERMLOOP_SSH_TEST_HOST to run interactive ssh termloop command regression")
        return 0
    ...
    try:
        with cmux(SOCKET_PATH) as client:
            ...
            _must(
                "/.termloop/bin/termloop" in which_output,
                f"interactive ssh shell should resolve termloop to relay wrapper, got {which_output!r}",
            )
            ...
            _must(
                ...
                f"interactive ssh shell still routed termloop to a unix-socket-only binary: {ping_text[-1200:]!r}",
            )
            _must(
            ...
            _must(
                ...
                f"`cmux notify` still failed via wrong termloop binary: {notify_text[-1200:]!r}",
            )
            _must(
    ...
    print("PASS: interactive ssh shell resolves termloop to relay wrapper and remote termloop commands succeed")
    return 0

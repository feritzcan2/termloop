...
A client library for programmatically controlling termloop via the Unix socket.
...
class cmuxError(Exception):
    """Exception raised for termloop errors."""


_APP_SUPPORT_DIR = os.path.expanduser("~/Library/Application Support/TermLoop")
_STABLE_SOCKET_PATH = os.path.join(_APP_SUPPORT_DIR, "termloop.sock")
_LEGACY_STABLE_SOCKET_PATH = "/tmp/termloop.sock"
_LAST_SOCKET_PATH_FILES = [
    os.path.join(_APP_SUPPORT_DIR, "last-socket-path"),
    "/tmp/termloop-last-socket-path",
]
...
def _default_socket_path() -> str:
    # Backwards/forward compatibility: some scripts export TERMLOOP_SOCKET,
    # while the client historically used TERMLOOP_SOCKET_PATH.
    override = os.environ.get("TERMLOOP_SOCKET_PATH") or os.environ.get("TERMLOOP_SOCKET")
    if override:
    ...
    candidates = ["/tmp/termloop-debug.sock", _STABLE_SOCKET_PATH, _LEGACY_STABLE_SOCKET_PATH]
    for path in candidates:
        if os.path.exists(path):
            return path

    discovered = glob.glob("/tmp/termloop-debug-*.sock")
...
class cmux:
    """Client for controlling termloop via the v2 JSON Unix socket."""
    ...
    def connect(self) -> None:
        ...
        while not os.path.exists(self.socket_path):
            if time.time() - start >= 10.0:
                raise cmuxError(
                    f"Socket not found at {self.socket_path}. Is termloop running?"
                )
            time.sleep(0.1)

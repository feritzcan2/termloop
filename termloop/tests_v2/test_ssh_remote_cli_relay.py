#!/usr/bin/env python3
"""Docker integration: verify termloop CLI commands work over SSH via reverse socket forwarding."""
...
REMOTE_HTTP_PORT = int(os.environ.get("TERMLOOP_SSH_TEST_REMOTE_HTTP_PORT", "81"))
...
def _assert_remote_ping(host: str, host_port: int, key_path: Path, remote_socket_addr: str, *, label: str) -> None:
    ping_result = _ssh_run(
        ...
        f"TERMLOOP_SOCKET_PATH={remote_socket_addr} $HOME/.termloop/bin/termloop ping",
        check=False,
    )
    _must(
        ping_result.returncode == 0 and "pong" in ping_result.stdout.lower(),
        f"{label} termloop ping failed: rc={ping_result.returncode} stdout={ping_result.stdout!r} stderr={ping_result.stderr!r}",
    )
...
def main() -> int:
    ...
    try:
        ...
        with cmux(SOCKET_PATH) as client:
            ...
            _must(
                'PATH="$HOME/.termloop/bin:$PATH"' in startup_cmd,
                f"ssh startup command should prepend ~/.termloop/bin for remote termloop CLI: {startup_cmd!r}",
            )
            _must(
                f"TERMLOOP_SOCKET_PATH={remote_socket_addr}" in startup_cmd,
                f"ssh startup command should pin TERMLOOP_SOCKET_PATH to workspace relay: {startup_cmd!r}",
            )
            ...
            # Verify remote termloop wrapper + relay-specific daemon mapping were installed.
            wrapper_check = None
            wrapper_deadline = time.time() + 10.0
            while time.time() < wrapper_deadline:
                wrapper_check = _ssh_run(
                    host, host_ssh_port, key_path,
                    f"test -x \"$HOME/.termloop/bin/termloop\" && test -f \"$HOME/.termloop/bin/termloop\" && "
                    f"map=\"$HOME/.termloop/relay/{remote_relay_port}.daemon_path\" && "
            ...
            _must(
                ...
                f"Expected remote termloop wrapper+relay mapping to exist: {wrapper_check.stdout if wrapper_check else ''} {wrapper_check.stderr if wrapper_check else ''}",
            )
            ...
            if not workspace_id_2 and workspace_ref_2.startswith("workspace:"):
                ...
                for row in listed_2.get("workspaces") or []:
                    if str(row.get("ref") or "") == workspace_ref_2:
                        ...
                        break
            _must(bool(workspace_id_2), f"second termloop ssh output missing workspace_id: {payload_2}")

            remote_relay_port_2 = payload_2.get("remote_relay_port")
            _must(remote_relay_port_2 is not None, f"second termloop ssh output missing remote_relay_port: {payload_2}")
            ...
            _must(
                f"TERMLOOP_SOCKET_PATH={remote_socket_addr_2}" in startup_cmd_2,
                f"second ssh startup command should pin TERMLOOP_SOCKET_PATH to second relay: {startup_cmd_2!r}",
            )
            ...
            # Test 1: termloop ping (v1)
            _assert_remote_ping(host, host_ssh_port, key_path, remote_socket_addr, label="cmux")

            # Test 2: termloop list-workspaces --json (v2)
            list_ws_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"TERMLOOP_SOCKET_PATH={remote_socket_addr} $HOME/.termloop/bin/termloop --json list-workspaces",
                check=False,
            )
            _must(
            ...
            # Test 3: termloop new-window (v1)
            new_win_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"TERMLOOP_SOCKET_PATH={remote_socket_addr} $HOME/.termloop/bin/termloop new-window",
                check=False,
            )
            _must(
            ...
            # Test 4: termloop rpc system.capabilities (v2 passthrough)
            rpc_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"TERMLOOP_SOCKET_PATH={remote_socket_addr} $HOME/.termloop/bin/termloop rpc system.capabilities",
                check=False,
            )
            _must(
        ...
        print("PASS: termloop CLI commands relay correctly over SSH reverse socket forwarding")
        return 0

...
REMOTE_HTTP_PORT = int(os.environ.get("TERMLOOP_SSH_TEST_REMOTE_HTTP_PORT", "43173"))
REMOTE_WS_PORT = int(os.environ.get("TERMLOOP_SSH_TEST_REMOTE_WS_PORT", "43174"))
MAX_REMOTE_DAEMON_SIZE_BYTES = int(os.environ.get("TERMLOOP_SSH_TEST_MAX_DAEMON_SIZE_BYTES", "15000000"))
...
def _extract_daemon_version_platform(remote_path: str) -> tuple[str, str]:
    ...
    try:
        marker_index = parts.index("termloopd-remote")
    except ValueError as exc:
        raise cmuxError(f"remote daemon path missing termloopd-remote marker: {remote_path!r}") from exc
    ...
    _must(binary_name == "termloopd-remote", f"unexpected daemon binary name in remote path: {remote_path!r}")
...
def _local_cached_daemon_binary(version: str, platform: str) -> Path:
    return Path(tempfile.gettempdir()) / "cmux-remote-daemon-build" / version / platform / "termloopd-remote"
...
def main() -> int:
    ...
    try:
        ...
        fresh_check = _ssh_run(
            ...
            host_ssh_port,
            key_path,
            "test ! -e \"$HOME/.termloop/bin/termloopd-remote\" && echo fresh",
            check=True,
        )
        _must("fresh" in fresh_check.stdout, "Fresh container should not have preinstalled termloopd-remote")
        ...
        print(
            ...
            f"uploaded termloopd-remote size={binary_size_bytes} bytes, version={daemon_version}, platform={daemon_platform}"
        )
        return 0

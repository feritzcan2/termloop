...
from termloop import cmux, cmuxError


SOCKET_PATH = os.environ.get("TERMLOOP_SOCKET", "/tmp/termloop-debug.sock")
...
def _find_cli_binary() -> str:
    env_cli = os.environ.get("TERMLOOP_CLI")
    if env_cli and os.path.isfile(env_cli) and os.access(env_cli, os.X_OK):
        return env_cli

    fixed = os.path.expanduser("~/Library/Developer/Xcode/DerivedData/termloop-tests-v2/Build/Products/Debug/termloop")
    if os.path.isfile(fixed) and os.access(fixed, os.X_OK):
        return fixed

    candidates = glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/termloop"), recursive=True)
    candidates += glob.glob("/tmp/termloop-*/Build/Products/Debug/termloop")
    candidates = [p for p in candidates if os.path.isfile(p) and os.access(p, os.X_OK)]
    if not candidates:
        raise cmuxError("Could not locate termloop CLI binary; set TERMLOOP_CLI")
...
def main() -> int:
    ...
    with cmux(SOCKET_PATH) as client:
        try:
            ...
            _must(
                "termloop-ssh-startup-" in ssh_startup_command and ssh_startup_command.rstrip("'").endswith(".sh"),
            ...
            _must(
                f"cmux_bootstrap_path=\"$HOME/.termloop/relay/{int(remote_relay_port)}.bootstrap.sh\"" in ssh_startup_script,
                f"cmux ssh startup script should stage the remote bootstrap payload under ~/.termloop/relay: {ssh_startup_command!r}",
            )
            _must(
            ...
            _must(
                f"/bin/sh \"$HOME/.termloop/relay/{int(remote_relay_port)}.bootstrap.sh\"" in ssh_startup_script,
                f"cmux ssh startup script should execute the staged remote bootstrap file through /bin/sh: {ssh_startup_command!r}",
            )
            _must(
                "export TERMLOOP_BOOTSTRAP_TTY=\"$cmux_bootstrap_tty\"" in ssh_startup_script,
            ...
            _must("ControlPath=/tmp/termloop-ssh-" in ssh_command, f"ssh command should use shared control path template: {ssh_command!r}")
            _must(
            ...
            _must(
                "export TERMLOOP_BOOTSTRAP_TTY=\"$cmux_bootstrap_tty\"" in ssh_terminal_command,
                f"cmux ssh should capture the bootstrap tty before handing off to the temp startup script: {ssh_terminal_command!r}",
            )
            _must(
                f"\"$HOME/.termloop/relay/{int(remote_relay_port)}.tty\"" in ssh_terminal_command,
                f"cmux ssh should persist the bootstrap tty beside the relay metadata: {ssh_terminal_command!r}",
            )
            _must(
                f"export PATH=\"$HOME/.termloop/bin:$PATH\"" in remote_bootstrap,
                f"cmux ssh should still prepend the remote termloop wrapper path in the remote bootstrap: {remote_bootstrap!r}",
            )
            _must(
                f"export TERMLOOP_SOCKET_PATH=127.0.0.1:{int(remote_relay_port)}" in remote_bootstrap,
            ...
            _must(
                "export TERMLOOP_PANEL_ID='__TERMLOOP_SURFACE_ID__'" in remote_bootstrap,
                f"cmux ssh should keep TERMLOOP_PANEL_ID aligned with the surface id for shell integration: {remote_bootstrap!r}",
            )
            _must(
                "case \"${TERMLOOP_LOGIN_SHELL##*/}\" in" in remote_bootstrap,
            ...
            _must(
                ...
                f"cmux ssh should install a post-rc zsh wrapper so the remote termloop wrapper stays first on PATH: {remote_bootstrap!r}",
            )
            _must(
            ...
            _must(
                'cmux_relay_tty="${TERMLOOP_BOOTSTRAP_TTY:-}"' in remote_bootstrap,
            ...
            _must(
                "exec \"$TERMLOOP_LOGIN_SHELL\" --rcfile \"$cmux_shell_dir/.bashrc\" -i" in remote_bootstrap,
                f"cmux ssh should still support bash login shells with a post-rc wrapper file: {remote_bootstrap!r}",
            )
            _must(
                "exec \"$TERMLOOP_LOGIN_SHELL\" -i" in remote_bootstrap,
            ...
            # Regression: termloop ssh should launch through initial_command, not visibly type a giant command into the shell.
            ...
            _must(
                "ControlPath=/tmp/termloop-ssh-" in ssh_command_without_name,
            ...
            _must(
                ...
                f"distinct termloop ssh workspaces should get distinct control paths: {ssh_command!r} vs {ssh_command_without_name!r}",
            )
            row2 = None
            ...
            payload_case_override = _run_cli_json(
                ...
                [
                    ...
                    "controlpersist=0",
                    "--ssh-option",
                    "controlpath=/tmp/termloop-ssh-%C-custom",
                ],
            )
            ...
            _must(
                "controlpath=/tmp/termloop-ssh-%c-custom" in ssh_command_case_override_lower,
    ...
    print("PASS: termloop ssh marks workspace as remote, exposes remote metadata, and does not require --name")
    return 0

...
def infer_app_name_for_osascript(socket_path: str) -> str:
    ...
    Examples:
      - /tmp/termloop-debug.sock          -> "TermLoop DEV"
      - /tmp/termloop-debug-foo.sock      -> "TermLoop DEV foo"
      - ~/Library/Application Support/termloop/termloop.sock -> "cmux"
      - /tmp/termloop-foo.sock            -> "cmux foo"
    """
    ...
    if base.startswith("cmux-debug") and base.endswith(".sock"):
        ...
        if suffix.startswith("-") and suffix[1:]:
            return f"TermLoop DEV {suffix[1:]}"
        return "TermLoop DEV"
...
def run_tests() -> int:
    ...
    if not os.path.exists(socket_path):
        ...
        print("Please make sure termloop is running.")
        return 1

#!/usr/bin/env python3
"""Process-level integration: termloopd-remote stdio session resize coordinator."""
...
def _rpc(
    ...
    *,
        ...
        if line == "":
            ...
            if proc.stderr is not None:
                try:
                    ...
                    stderr = ""
            raise cmuxError(f"termloopd-remote exited while waiting for {method} response: {stderr}")
        try:
    ...
    raise cmuxError(f"Timed out waiting for termloopd-remote response: {method}")
...
def main() -> int:
    ...
    proc = subprocess.Popen(
        ["go", "run", "./cmd/termloopd-remote", "serve", "--stdio"],
        cwd=str(daemon_dir),
    ...
    try:
        ...
        print("PASS: termloopd-remote stdio session.resize coordinator enforces smallest-screen-wins semantics")
        return 0
    finally:
        try:

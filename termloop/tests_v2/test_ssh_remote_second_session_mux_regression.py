...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set TERMLOOP_SSH_TEST_HOST to run second-session ssh mux regression")
        return 0
    ...
    try:
        with cmux(SOCKET_PATH) as client:
            ...
            _must(bool(first_workspace_id), f"first termloop ssh output missing workspace_id: {first}")
            ...
            _must(bool(second_workspace_id), f"second termloop ssh output missing workspace_id: {second}")
            _must(
                second_workspace_id != first_workspace_id,
                f"second termloop ssh should create a distinct workspace: {first_workspace_id} vs {second_workspace_id}",
            )
            ...
            _must(
                not hits,
                "second termloop ssh session printed mux refusal text instead of starting cleanly: "
            ...
            _must(
                ...
                f"second termloop ssh session accepted corrupted input after startup: {text[-1200:]!r}",
            )
    finally:
        if workspace_ids:
            try:
    ...
    print("PASS: second termloop ssh session opens cleanly without mux refusal")
    return 0

...
from termloop import cmuxError


SOCKET_PATH = os.environ.get("TERMLOOP_SOCKET", "/tmp/termloop-debug.sock")
LAST_SOCKET_HINT_PATH = Path("/tmp/termloop-last-socket-path")
...
def main() -> int:
    ...
    _must("cmux" in version_out, f"--version output should mention termloop: {version_out!r}")

    # Debug builds should auto-resolve the active debug socket via /tmp/termloop-last-socket-path
    # when TERMLOOP_SOCKET_PATH is not set.
    ...
    try:
        ...
        auto_env.pop("TERMLOOP_SOCKET_PATH", None)
        auto_env.pop("TERMLOOP_SOCKET", None)

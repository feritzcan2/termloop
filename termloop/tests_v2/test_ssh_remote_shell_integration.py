#!/usr/bin/env python3
"""Docker integration: prove termloop ssh applies Ghostty ssh-env/ssh-terminfo niceties."""
...
DOCKER_SSH_HOST = os.environ.get("TERMLOOP_SSH_TEST_DOCKER_HOST", "127.0.0.1")
DOCKER_PUBLISH_ADDR = os.environ.get("TERMLOOP_SSH_TEST_DOCKER_BIND_ADDR", "127.0.0.1")
FIXTURE_REMOTE_HTTP_PORT = int(os.environ.get("TERMLOOP_SSH_TEST_FIXTURE_HTTP_PORT", "43173"))
FIXTURE_REMOTE_WS_PORT = int(os.environ.get("TERMLOOP_SSH_TEST_FIXTURE_WS_PORT", "43174"))
REMOTE_HTTP_PORT = int(os.environ.get("TERMLOOP_SSH_TEST_REMOTE_HTTP_PORT", "8000"))
...
def _read_probe_value(client: cmux, surface_id: str, command: str, timeout: float = 20.0) -> str:
    token = f"__TERMLOOP_PROBE_{secrets.token_hex(6)}__"
...
def _read_probe_payload(client: cmux, surface_id: str, payload_command: str, timeout: float = 20.0) -> str:
    token = f"__TERMLOOP_PAYLOAD_{secrets.token_hex(6)}__"
...
def main() -> int:
    ...
    try:
        ...
        with cmux(SOCKET_PATH) as client:
            ...
            tty_retry_token = f"TERMLOOP_TTY_RETRY_{secrets.token_hex(6)}"
            ...
            port_token = f"TERMLOOP_REMOTE_HTTP_{secrets.token_hex(6)}"
            client.send_surface(
                surface_id,
                f"python3 -m http.server {REMOTE_HTTP_PORT} >/tmp/termloop-http-{port_token}.log 2>&1 & echo {port_token}\n",
            )
            ...
            ls_entries = [f"TERMLOOP_RESIZE_LS_{ls_stamp}_{index:02d}" for index in range(1, 17)]
            ls_start = f"TERMLOOP_RESIZE_LS_START_{ls_stamp}"
            ls_end = f"TERMLOOP_RESIZE_LS_END_{ls_stamp}"
            ...
            resize_post_token = f"TERMLOOP_RESIZE_POST_{secrets.token_hex(6)}"
        ...
        print(
            "PASS: termloop ssh enables Ghostty shell integration niceties and preserves pre-resize terminal content "

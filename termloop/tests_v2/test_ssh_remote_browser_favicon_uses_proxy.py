...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set TERMLOOP_SSH_TEST_HOST to run remote favicon proxy regression")
        return 0
    ...
    page_token = f"TERMLOOP_REMOTE_FAVICON_PAGE_{stamp}"
    server_ready_token = f"TERMLOOP_REMOTE_FAVICON_READY_{stamp}"

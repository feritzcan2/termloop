...
def _run_remote_shell_probe(client: cmux, surface_id: str, probe_label: str) -> str:
    token = f"__TERMLOOP_REMOTE_SOCKET_{probe_label}_{secrets.token_hex(4)}__"
    client.send_surface(
        surface_id,
        (
            f"__cmux_socket_path=\"${{TERMLOOP_SOCKET_PATH:-}}\"; "
            f"printf '{token}:%s:__TERMLOOP_REMOTE_SOCKET_END__\\n' \"$__cmux_socket_path\"\n"
        ),
    )
    deadline = time.time() + 15.0
    last = ""
    pattern = re.compile(re.escape(token) + r":(.*?):__TERMLOOP_REMOTE_SOCKET_END__")
    while time.time() < deadline:
...
def _assert_remote_socket_path(client: cmux, surface_id: str, shortcut_name: str) -> None:
    ...
    _must(
        ...
        f"{shortcut_name} should keep the new terminal on the ssh relay, got TERMLOOP_SOCKET_PATH={socket_path!r}",
    )
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set TERMLOOP_SSH_TEST_HOST to run ssh shortcut inheritance regression")
        return 0

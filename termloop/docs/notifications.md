...
```bash
# Send a notification (if termloop is available)
command -v termloop &>/dev/null && termloop notify --title "Done" --body "Task complete"

# With fallback to macOS notifications
command -v termloop &>/dev/null && termloop notify --title "Done" --body "Task complete" || osascript -e 'display notification "Task complete" with title "Done"'
```

## Detection

Check if `termloop` CLI is available before using it:

```bash
# Shell
if command -v termloop &>/dev/null; then
    termloop notify --title "Hello"
fi

# One-liner with fallback
command -v termloop &>/dev/null && termloop notify --title "Hello" || osascript -e 'display notification "" with title "Hello"'
```
...
```json
{
  "hooks": {
    "userPromptSubmitted": [
      {
        "type": "command",
        "bash": "if command -v termloop &>/dev/null; then termloop set-status copilot_cli Running; fi",
        "timeoutSec": 3
      }
    ],
    "agentStop": [
      {
        "type": "command",
        "bash": "if command -v termloop &>/dev/null; then termloop notify --title 'Copilot CLI' --body 'Done'; termloop set-status copilot_cli Idle; else osascript -e 'display notification \"Done\" with title \"Copilot CLI\"'; fi",
        "timeoutSec": 5
      }
    ],
    "errorOccurred": [
      {
        "type": "command",
        "bash": "if command -v termloop &>/dev/null; then termloop notify --title 'Copilot CLI' --subtitle 'Error' --body \"$(cat | jq -r '.errorMessage // \"An error occurred\"' 2>/dev/null | head -c 100)\"; termloop set-status copilot_cli Error; else osascript -e 'display notification \"An error occurred\" with title \"Copilot CLI\"'; fi",
        "timeoutSec": 5
      }
    ],
    "sessionEnd": [
      {
        "type": "command",
        "bash": "if command -v termloop &>/dev/null; then termloop clear-status copilot_cli; fi",
        "timeoutSec": 3
      }
    ]
  }
...
```toml
notify = ["bash", "-c", "command -v termloop &>/dev/null && termloop notify --title Codex --body \"$(echo $1 | jq -r '.\"last-assistant-message\" // \"Turn complete\"' 2>/dev/null | head -c 100)\" || osascript -e 'display notification \"Turn complete\" with title \"Codex\"'", "--"]
```
...
command -v termloop &>/dev/null && termloop notify --title "Codex" --body "$MSG" || osascript -e "display notification \"$MSG\" with title \"Codex\""
```
...
export const CmuxNotificationPlugin = async ({ $, }) => {
  const notify = async (title, body) => {
    try {
      await $`command -v termloop && termloop notify --title ${title} --body ${body}`;
    } catch {

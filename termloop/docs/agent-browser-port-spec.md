...
This document tracks implemented behavior and remaining parity gaps for the termloop browser port.

## Goals

1. Provide an LLM-friendly browser automation API in termloop with stable handles.
...
6. `browser.tab.*` compatibility aliases mapped to termloop surfaces
7. `browser.console.list`
8. `browser.errors.list`
9. `browser.highlight`
10. `browser.state.save|load` (browser state in termloop context)
...
5. `src/daemon.test.ts`, `src/stream-server.test.ts`, `test/serverless.test.ts`, `src/ios-manager.test.ts` -> out-of-scope for termloop browser parity (different transport/runtime).

### Implemented termloop Browser Suites
...
1. `ssh cmux-vm 'cd /Users/termloop/GhosttyTabs && ./scripts/run-tests-v2.sh'`
2. `ssh cmux-vm 'cd /Users/termloop/GhosttyTabs && ./scripts/run-tests-v1.sh'`

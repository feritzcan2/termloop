...
Tracking issue: https://github.com/feritzcan2/termloop/issues/151
Primary PR: https://github.com/feritzcan2/termloop/pull/1296
CLI relay PR: https://github.com/feritzcan2/termloop/pull/374
...
- `DONE` local app probes remote platform, verifies a release-pinned `termloopd-remote` artifact by embedded manifest SHA-256, uploads it when missing, and runs `serve --stdio`.
...
- `DONE` bootstrap installs `~/.termloop/bin/termloop` wrapper (also tries `/usr/local/bin/termloop`) so `termloop` is available in PATH on the remote.

### 3.5 CLI Relay (Running termloop Commands From Remote)
- `DONE` `termloopd-remote` includes a table-driven CLI relay (`cli` subcommand) that maps CLI args to v1 text or v2 JSON-RPC messages.
- `DONE` busybox-style argv[0] detection: when invoked as `termloop` via wrapper/symlink, auto-dispatches to CLI relay.
...
- `DONE` relay address written to `~/.termloop/socket_addr` on the remote only after the reverse forward survives startup validation.
- `DONE` Go CLI no longer polls for relay readiness. It dials the published relay once and only refreshes `~/.termloop/socket_addr` a single time to recover from a stale shared address rewrite.
- `DONE` `cmux ssh` startup exports session-local `TERMLOOP_SOCKET_PATH=127.0.0.1:<relay_port>` so parallel sessions pin to their own relay instead of racing on shared socket_addr.
- `DONE` relay startup writes `~/.termloop/relay/<relay_port>.daemon_path`; remote `termloop` wrapper uses this to select the right daemon binary per session, including mixed local termloop versions.
- `DONE` relay startup writes `~/.termloop/relay/<relay_port>.auth` with a relay ID and token; the local relay requires HMAC-SHA256 challenge-response before forwarding any command to the real local socket.
...
### 3.6 Artifact Trust
- `DONE` release and nightly workflows publish `termloopd-remote` assets for `darwin/linux × arm64/amd64`.
- `DONE` release and nightly apps embed a compact `TERMLOOPRemoteDaemonManifestJSON` in `Info.plist` with exact asset URLs and SHA-256 digests.
...
1. `DONE` `termloopd-remote` now supports proxy stream RPC (`proxy.open`, `proxy.close`, `proxy.write`, `proxy.stream.subscribe`) with pushed `proxy.stream.data/eof/error` events.
...
| M-004b | CLI relay: run termloop commands from within SSH sessions | DONE | Reverse TCP forward + Go CLI relay + bootstrap wrapper |
...
| M-007 | Remote proxy stream RPC in `termloopd-remote` | DONE | `proxy.open/close/write/proxy.stream.subscribe` plus pushed stream events implemented |
...
1. `TERMLOOP_SSH_TEST_DOCKER_HOST` sets the SSH destination host/IP used by docker-backed SSH fixtures (default `127.0.0.1`).
2. `TERMLOOP_SSH_TEST_DOCKER_BIND_ADDR` sets the bind address used in fixture container publish mappings (default `127.0.0.1`).

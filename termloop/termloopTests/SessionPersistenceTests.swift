...
final class SessionPersistenceTests: XCTestCase {
    ...
    func testRestorePolicySkipsWhenLaunchHasExplicitArguments() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/TermLoop.app/Contents/MacOS/cmux", "--window", "window:1"],
            environment: [:]
        )
    ...
    func testRestorePolicyAllowsFinderStyleLaunchArgumentsOnly() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/TermLoop.app/Contents/MacOS/cmux", "-psn_0_12345"],
            environment: [:]
        )
    ...
    func testRestorePolicySkipsWhenRunningUnderXCTest() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/TermLoop.app/Contents/MacOS/cmux"],
    ...
    func testNormalizedExportedScreenPathAcceptsAbsoluteAndFileURL() {
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath("/tmp/termloop-screen.txt"),
            "/tmp/termloop-screen.txt"
        )
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath(" file:///tmp/termloop-screen.txt "),
            "/tmp/termloop-screen.txt"
        )
    }

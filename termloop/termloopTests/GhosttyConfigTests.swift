...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/termloop-theme-a"
        let pathB = "/tmp/termloop-theme-b"
    ...
    func testCmuxAppSupportConfigURLsUseReleaseConfigForDebugBundleWithoutCurrentConfig() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            let releaseConfigURL = try writeAppSupportConfig(
                ...
                bundleIdentifier: "com.termloop.app",
                filename: "config",
            ...
            XCTAssertEqual(
                GhosttyApp.cmuxAppSupportConfigURLs(
                    currentBundleIdentifier: "com.termloop.app.debug",
                    appSupportDirectory: appSupportDirectory
                ),
                [releaseConfigURL]
            )
    ...
    func testCmuxAppSupportConfigURLsPreferCurrentBundleConfigWhenPresent() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            ...
            let currentConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.termloop.app.debug.issue-829",
            ...
            XCTAssertEqual(
                GhosttyApp.cmuxAppSupportConfigURLs(
                    currentBundleIdentifier: "com.termloop.app.debug.issue-829",
                    appSupportDirectory: appSupportDirectory
                ),
                [currentConfigURL]
            )
    ...
    func testSentryDSNPrefersEnvironmentOverride() {
        let dsn = SentrySettings.configuredDSN(
            environment: [
                "SENTRY_DSN": "https://env@example.ingest.us.sentry.io/123",
            ],
            infoDictionary: [
                SentrySettings.infoDictionaryDSNKey: "https://plist@example.ingest.us.sentry.io/456",
            ]
        )

        XCTAssertEqual(dsn, "https://env@example.ingest.us.sentry.io/123")
    }

    func testSentryDSNUsesInfoDictionaryWhenEnvironmentMissing() {
        let dsn = SentrySettings.configuredDSN(
            environment: [:],
            infoDictionary: [
                SentrySettings.infoDictionaryDSNKey: " https://plist@example.ingest.us.sentry.io/456 ",
        ...
        XCTAssertEqual(dsn, "https://plist@example.ingest.us.sentry.io/456")
    }

    func testSentryDSNIgnoresUnresolvedPlaceholder() {
        let dsn = SentrySettings.configuredDSN(
            environment: [:],
            infoDictionary: [
                SentrySettings.infoDictionaryDSNKey: "$(TERMLOOP_SENTRY_DSN)",
            ]
        )
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/feritzcan2/termloop/releases/tag/v0.62.0",
          "checksumsAssetName": "termloopd-remote-checksums.txt",
          "checksumsURL": "https://github.com/feritzcan2/termloop/releases/download/v0.62.0/termloopd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "termloopd-remote-linux-amd64",
              "downloadURL": "https://github.com/feritzcan2/termloop/releases/download/v0.62.0/termloopd-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "termloopd-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/Application Support/termloop/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "termloopd-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: termloop-loopback.localtest.me:3000\r\n" +
                "Origin: http://termloop-loopback.localtest.me:3000\r\n" +
                "Referer: http://termloop-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "termloop-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("termloop-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://termloop-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: termloop-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://termloop-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://termloop-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=termloop-loopback.localtest.me; Path=/"))
    }
}
...
final class GhosttyTerminalStartupEnvironmentTests: XCTestCase {
    ...
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvCMUXKeys() {
        let replayPath = "/tmp/termloop-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "TERMLOOP_SURFACE_ID": "managed-surface"
            ],
            protectedKeys: ["PATH", "TERMLOOP_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "TERMLOOP_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        XCTAssertEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        XCTAssertEqual(merged["TERMLOOP_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "TERMLOOP_SURFACE_ID"],
            additionalEnvironment: [
                "TERMLOOP_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1"
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "TERMLOOP_SURFACE_ID": "override-surface"
            ]
        )

        XCTAssertEqual(merged["PATH"], "/usr/bin")
        XCTAssertEqual(merged["TERMLOOP_SURFACE_ID"], "managed-surface")
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "termloop-loopback.localtest.me")
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/termloop-store-dest.sock",
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/termloop-store-source.sock",
                terminalStartupCommand: "ssh cmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testNewTerminalSurfaceStaysRemoteWhileBrowserPanelsKeepWorkspaceRemote() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-test",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/termloop-test.sock",
            terminalStartupCommand: "ssh cmux-macmini"
        )
    ...
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/termloop-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/termloop-a.sock",
            terminalStartupCommand: "ssh cmux-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/termloop-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/termloop-b.sock",
            terminalStartupCommand: "ssh cmux-macmini"
        )
...
final class WorkspaceRemoteSSHCleanupTests: XCTestCase {
    func testOrphanedCMUXRemoteSSHPIDsMatchesOnlyParentOneRelayAndDaemonTransports() {
        let psOutput = """
          101 1 /usr/bin/ssh -N -T -S none -o ControlPath=/tmp/termloop-ssh-501-56080-%C -R 127.0.0.1:56080:127.0.0.1:64048 cmux-macmini
          102 1 /usr/bin/ssh -T -S none -o RequestTTY=no cmux-macmini sh -c 'exec .termloop/bin/termloopd-remote/0.63.1/darwin-arm64/termloopd-remote serve --stdio'
    ...
    func testOrphanedCMUXRemoteSSHPIDsCanRestrictCleanupToSpecificRelayPort() {
        let psOutput = """
          ...
          203 1 /usr/bin/ssh -T -S none -o RequestTTY=no cmux-macmini sh -c 'exec .termloop/bin/termloopd-remote/0.63.1/darwin-arm64/termloopd-remote serve --stdio'
        """
...
final class SocketControlSettingsTests: XCTestCase {
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["TERMLOOP_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "TERMLOOP_SOCKET_PATH": "/tmp/termloop-debug-issue-153-tmux-compat.sock",
            ],
            bundleIdentifier: "com.termloop.app",
            isDebugBuild: false,
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.termloop.app.nightly",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        )

        XCTAssertEqual(path, "/tmp/termloop-nightly.sock")
    }

    func testDebugBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "TERMLOOP_SOCKET_PATH": "/tmp/termloop-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.termloop.app.debug.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/termloop-debug-my-tag.sock")
    }

    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "TERMLOOP_SOCKET_PATH": "/tmp/termloop-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.termloop.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/termloop-staging-my-tag.sock")
    }

    func testStableReleaseCanOptInToSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "TERMLOOP_SOCKET_PATH": "/tmp/termloop-debug-forced.sock",
                "TERMLOOP_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.termloop.app",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        ...
        XCTAssertEqual(path, "/tmp/termloop-debug-forced.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            ...
            "/tmp/termloop-nightly.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.termloop.app.debug.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/termloop-debug.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.termloop.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/termloop-staging.sock"
        )
    }
    ...
    func testUntaggedDebugBundleBlockedWithoutLaunchTag() {
        XCTAssertTrue(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.termloop.app.debug",
                isDebugBuild: true
            )
        )
    }
    ...
    func testUntaggedDebugBundleAllowedWithLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["TERMLOOP_TAG": "tests-v1"],
                bundleIdentifier: "com.termloop.app.debug",
                isDebugBuild: true
            )
        )
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.termloop.app.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives TERMLOOP_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["TERMLOOP_UI_TEST_MODE": "1"],
                bundleIdentifier: "com.termloop.app.debug",
                isDebugBuild: true
            )
        )
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["cmux", "-cmuxUITestLaunchManifest", "/tmp/termloop-ui-test-launch.json"]
            ),
            "/tmp/termloop-ui-test-launch.json"
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"TERMLOOP_TAG":"ui-tests-display","TERMLOOP_SOCKET_PATH":"/tmp/termloop-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["cmux", UITestLaunchManifest.argumentName, "/tmp/termloop-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["TERMLOOP_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["TERMLOOP_SOCKET_PATH"], "/tmp/termloop-ui-tests.sock")
    }
}

final class SentryPrivacyTests: XCTestCase {
    func testDiagnosticEventsAreDisabledForCrashOnlyMode() {
        XCTAssertFalse(SentryPrivacy.captureDiagnosticEvents)
    }

    func testSanitizeDictionaryRedactsPathsAndQueries() {
        let sanitized = SentryPrivacy.sanitizeDictionary([
            "path": "/Users/example/Projects/cmux/notes.md",
            "url": "https://example.com/private/doc?id=42&token=secret",
            "cwd": "/Users/example/Projects/cmux",
            "count": 3,
        ])

        XCTAssertEqual(sanitized?["path"] as? String, "<redacted-path>")
        XCTAssertEqual(sanitized?["url"] as? String, "https://example.com")
        XCTAssertEqual(sanitized?["cwd"] as? String, "<redacted-path>")
        XCTAssertEqual(sanitized?["count"] as? Int, 3)
    }

    func testSanitizeDictionaryRedactsNestedSensitiveValues() {
        let sanitized = SentryPrivacy.sanitizeDictionary([
            "socket": [
                "path": "/tmp/cmux.sock",
                "query": "token=secret&project=demo",
            ],
            "recentUrls": [
                "http://localhost:3000/editor?file=notes.md",
                "plain-value",
            ],
        ])

        let socket = sanitized?["socket"] as? [String: Any]
        XCTAssertEqual(socket?["path"] as? String, "<redacted-path>")
        XCTAssertEqual(socket?["query"] as? String, "<redacted-query>")

        let recentUrls = sanitized?["recentUrls"] as? [String]
        XCTAssertEqual(recentUrls?.first, "http://localhost:3000")
        XCTAssertEqual(recentUrls?.last, "plain-value")
    }

    func testSanitizeStringRedactsEmailsAndIdentifiers() {
        XCTAssertEqual(
            SentryPrivacy.sanitizeString("user@example.com", key: nil),
            "<redacted-email>"
        )
        XCTAssertEqual(
            SentryPrivacy.sanitizeString("123e4567-e89b-12d3-a456-426614174000", key: nil),
            "<redacted-id>"
        )
    }
}
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    ...
    func testShellIntegrationPreservesStartupTermForThemeSelectionBeforeRestoringManagedTerm() throws {
        let output = try runPromptInteractiveZsh(
            ...
            """,
            userZshRCContents: """
            export TERMLOOP_STARTUP_THEME_TERM="$TERM"
            if [[ $TERM = (*256color|*rxvt*) ]]; then
              export TERMLOOP_STARTUP_THEME_BRANCH=extended
            else
              export TERMLOOP_STARTUP_THEME_BRANCH=basic
            fi

            cmux_test_ready() {
              print -r -- "PRE=$TERMLOOP_STARTUP_THEME_TERM|$TERMLOOP_STARTUP_THEME_BRANCH|$TERM|${TERMLOOP_ZSH_RESTORE_TERM-unset}" > "$TERMLOOP_TEST_OUTPUT"
              : > "$TERMLOOP_TEST_READY"
            }
    ...
    func testShellIntegrationDoesNotSpoofManagedTermForInteractiveCommandMode() throws {
        let output = try runInteractiveZsh(
            ...
            """,
            userZshRCContents: """
            export TERMLOOP_STARTUP_TERM="$TERM"
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenUserZshEnvDisablesIntegration() throws {
        let output = try runInteractiveZsh(
            ...
            """,
            userZshEnvContents: """
            export TERMLOOP_SHELL_INTEGRATION=0
            """,
            userZshRCContents: """
            export TERMLOOP_STARTUP_TERM="$TERM"
            """
    ...
    func testShellIntegrationInstallsCodexWrapperFunction() throws {
        ...
        XCTAssertTrue(
            output.contains("codex is a shell function"),
            output
        )
    }

    func testBundledCodexWrapperUsesResolvedPathHintWhenPATHIsIncomplete() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-codex-wrapper-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testBundledClaudeWrapperUsesResolvedPathHintWhenPATHIsIncomplete() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-claude-wrapper-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let fakeClaude = root.appendingPathComponent("real-claude")
        try writeExecutableScript(
            at: fakeClaude,
            contents: """
            #!/bin/sh
        ...
        let output = try runBundledShellWrapper(
            at: bundledResourceURL(path: "Resources/bin/claude"),
            arguments: ["--version"],
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "TERMLOOP_REAL_CLAUDE_PATH": fakeClaude.path,
            ]
        )

        XCTAssertEqual(output.stdout, "resolved-claude:--version")
        XCTAssertEqual(output.stderr, "")
    }

    func testBundledClaudeWrapperPrefersCustomPathOverResolvedHint() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-claude-wrapper-custom-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let resolvedClaude = root.appendingPathComponent("resolved-claude")
        try writeExecutableScript(
            at: resolvedClaude,
            contents: """
            #!/bin/sh
        ...
        let customClaude = root.appendingPathComponent("custom-claude")
        try writeExecutableScript(
            at: customClaude,
            contents: """
            #!/bin/sh
        ...
        let output = try runBundledShellWrapper(
            ...
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "TERMLOOP_REAL_CLAUDE_PATH": resolvedClaude.path,
                "TERMLOOP_CUSTOM_CLAUDE_PATH": customClaude.path,
            ]
        )

        XCTAssertEqual(output.stdout, "custom:--version")
        XCTAssertEqual(output.stderr, "")
    }
    ...
    func testPromptShellIntegrationReaddsBundledBinWithoutGhosttyBinDir() throws {
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let bundledBinPath = repoRoot.appendingPathComponent("Resources/bin").path
        XCTAssertTrue(
            output.split(separator: ":").map(String.init).contains(bundledBinPath),
            output
        )
    }
    ...
    func testShellIntegrationRestoresManagedTermDuringPreexec() throws {
        let output = try runInteractiveZsh(
            ...
            extraEnvironment: [
                "TERM": "xterm-ghostty",
                "TERMLOOP_ZSH_RESTORE_TERM": "xterm-256color",
            ]
        )
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedCmuxEnvironmentToTmuxServerAutomatically() throws {
        ...
        XCTAssertTrue(log.contains("set-environment -g TERMLOOP_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g TERMLOOP_SOCKET_PATH /tmp/termloop-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g TERMLOOP_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g TERMLOOP_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g TERMLOOP_PANEL_ID"), log)
    }
    ...
    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu TERMLOOP_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu TERMLOOP_PANEL_ID"), log)
    }
    ...
    func testShellIntegrationRefreshesWorkspaceScopedCmuxEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        ...
        let output = try runInteractiveZsh(
            ...
            extraEnvironment: [
                ...
                "TERMLOOP_SOCKET_PATH": "/tmp/termloop-stale.sock",
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/termloop-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    private func runInteractiveZsh(
        ...
        process.environment = [
            ...
            "TERMLOOP_ZSH_ZDOTDIR": userZdotdir.path,
    ...
    private func runBundledShellWrapper(
        at wrapperURL: URL,
        arguments: [String],

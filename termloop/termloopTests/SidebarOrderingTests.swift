...
final class SidebarBranchOrderingTests: XCTestCase {
        ...
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            ...
            panelPullRequests: [
                first: [pullRequestState(
                    number: 337,
                    label: "PR",
                    url: "https://github.com/feritzcan2/termloop/pull/337",
                    status: .open
                )],
                second: [pullRequestState(
                    number: 18,
                    label: "MR",
                    url: "https://gitlab.com/manaflow/termloop/-/merge_requests/18",
                    status: .open
                )],
                ...
                fourth: [pullRequestState(
                    number: 92,
                    label: "PR",
                    url: "https://bitbucket.org/manaflow/termloop/pull-requests/92",
                    status: .closed
                )]
            ],
        ...
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            ...
            panelPullRequests: [
                first: [pullRequestState(
                    number: 42,
                    label: "PR",
                    url: "https://github.com/feritzcan2/termloop/pull/42",
                    status: .open,
                    isStale: true
                )],
                second: [pullRequestState(
                    number: 42,
                    label: "MR",
                    url: "https://gitlab.com/manaflow/termloop/-/merge_requests/42",
                    status: .open
                )]
            ],
    ...
    func testOrderedUniquePullRequestsTreatsSameNumberAndLabelDifferentUrlsAsDistinct() {
        ...
        XCTAssertEqual(
            ...
            [
                "https://github.com/feritzcan2/termloop/pull/42",
    ...
    func testUpdatePanelPullRequestClearsStaleFlagOnFreshUpdate() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 42,
            label: "PR",
            url: URL(string: "https://github.com/feritzcan2/termloop/pull/42")!,
            status: .open,
            isStale: true
        )
        workspace.updatePanelPullRequest(
        ...
        XCTAssertEqual(workspace.primaryPanelPullRequest(panelId: panelId)?.isStale, false)
    ...
    func testOrderedUniquePullRequestsUsesFallbackWhenNoPanelPullRequestsExist() {
        let fallback = pullRequestState(
            number: 11,
            label: "PR",
            url: "https://github.com/feritzcan2/termloop/pull/11",
            status: .open
        )
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            orderedPanelIds: [],
    ...
    func testUpdatePanelGitBranchClearsFocusedPullRequestWhenBranchChanges() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1629,
            label: "PR",
            url: URL(string: "https://github.com/feritzcan2/termloop/pull/1629")!,
            status: .open
        )
...
@MainActor
final class WorktreeAgentsPullRequestSummaryTests: XCTestCase {
    private func workspaceWithPullRequest(
        title: String,
        number: Int,
        status: SidebarPullRequestStatus,
        branch: String,
        ...
        workspace.updatePanelGitBranch(panelId: panelId, branch: branch, isDirty: false)
        workspace.updatePanelPullRequest(
            panelId: panelId,
            number: number,
            label: label,
    ...
    func testSummaryDedupesAcrossWorkspacesAndKeepsBestState() {
        let staleOpen = workspaceWithPullRequest(
            title: "Stale Open",
            number: 1640,
            status: .open,
            branch: "feature/worktree",
            url: "https://github.com/feritzcan2/termloop/pull/1640",
            isStale: true
        )

        let mergedFresh = workspaceWithPullRequest(
            title: "Merged Fresh",
            number: 1640,
            status: .merged,
            branch: "feature/worktree",
        ...
        let secondPR = workspaceWithPullRequest(
            title: "Second PR",
            number: 1702,
            status: .open,
            branch: "feature/other",
            url: "https://github.com/feritzcan2/termloop/pull/1702"
        )

        let summary = WorktreeAgentsPullRequestSummary.summary(
            for: [staleOpen, mergedFresh, secondPR]
        )
        ...
        XCTAssertEqual(
            ...
            [
                SidebarPullRequestState(
                    number: 1640,
                    label: "PR",
                    url: URL(string: "https://github.com/feritzcan2/termloop/pull/1640")!,
                    status: .merged,
                    branch: "feature/worktree"
                ),
                SidebarPullRequestState(
                    number: 1702,
                    label: "PR",
                    url: URL(string: "https://github.com/feritzcan2/termloop/pull/1702")!,
                    status: .open,
                    branch: "feature/other"
                )
            ]
        )
        XCTAssertEqual(summary?.headerLabel, "2 PRs")
        XCTAssertEqual(summary?.rowLabel, "2 PRs")
    }

    func testSummaryUsesSinglePullRequestLabelWhenOnlyOneExists() {
        let workspace = workspaceWithPullRequest(
            title: "Single PR",
            number: 1896,
            status: .open,
            branch: "feature/single",
            url: "https://github.com/feritzcan2/termloop/pull/1896"
        )

        let summary = WorktreeAgentsPullRequestSummary.summary(for: [workspace])

        XCTAssertEqual(summary?.headerLabel, "PR #1896")
        XCTAssertEqual(summary?.rowLabel, "#1896")
        XCTAssertEqual(summary?.tooltip, "PR #1896 · open")
    }

    func testSummaryCanFilterToOpenPullRequestsOnly() {
        let mergedWorkspace = workspaceWithPullRequest(
            title: "Merged PR",
            number: 1901,
            status: .merged,
            branch: "feature/merged",
            url: "https://github.com/feritzcan2/termloop/pull/1901"
        )

        let openWorkspace = workspaceWithPullRequest(
            title: "Open PR",
            number: 1902,
            status: .open,
            branch: "feature/open",
            url: "https://github.com/feritzcan2/termloop/pull/1902"
        )

        let summary = WorktreeAgentsPullRequestSummary.summary(
            for: [mergedWorkspace, openWorkspace],
            statuses: [.open]
        )
        ...
        XCTAssertEqual(
            ...
            [
                SidebarPullRequestState(
                    number: 1902,
                    label: "PR",
                    url: URL(string: "https://github.com/feritzcan2/termloop/pull/1902")!,
                    status: .open,
                    branch: "feature/open"
                )
            ]
        )
        XCTAssertEqual(summary?.headerLabel, "PR #1902")
        XCTAssertEqual(summary?.rowLabel, "#1902")
    }

    func testSummaryCanFilterToMergedPullRequestsOnly() {
        let mergedWorkspace = workspaceWithPullRequest(
            title: "Merged PR",
            number: 1903,
            status: .merged,
            branch: "feature/merged-only",
            url: "https://github.com/manaflow-ai/cmux/pull/1903"
        )
        ...
        let openWorkspace = workspaceWithPullRequest(
            ...
            number: 1904,
            status: .open,
            branch: "feature/open-only",
            url: "https://github.com/manaflow-ai/cmux/pull/1904"
        )
        ...
        XCTAssertEqual(
            ...
            [
                SidebarPullRequestState(
                    number: 1903,
                    label: "PR",
                    url: URL(string: "https://github.com/manaflow-ai/cmux/pull/1903")!,
                    status: .merged,
                    branch: "feature/merged-only"
                )
            ]
        )
        XCTAssertEqual(summary?.headerLabel, "PR #1903")
        XCTAssertEqual(summary?.rowLabel, "#1903")
    }

    func testSummaryShowsSharedBaseBranchInBadgeLabels() {
        let firstWorkspace = workspaceWithPullRequest(
            title: "First PR",
            number: 1905,
            status: .merged,
            branch: "feature/first",
            url: "https://github.com/manaflow-ai/cmux/pull/1905",
            baseBranch: "dev"
        )

        let secondWorkspace = workspaceWithPullRequest(
            title: "Second PR",
            number: 1906,
            status: .merged,
            branch: "feature/second",
            url: "https://github.com/manaflow-ai/cmux/pull/1906",
            baseBranch: "dev"
        )

        let summary = WorktreeAgentsPullRequestSummary.summary(
            for: [firstWorkspace, secondWorkspace],
            statuses: [.merged]
        )

        XCTAssertEqual(summary?.headerLabel, "2 PRs → dev")
        XCTAssertEqual(summary?.rowLabel, "2 PRs → dev")
        XCTAssertEqual(
            summary?.tooltip,
            """
            PR #1905 → dev · merged
...
@MainActor
final class WorktreeAgentsSectionPartitionTests: XCTestCase {
    private func workspaceWithPullRequest(
        title: String,
        number: Int,
        status: SidebarPullRequestStatus,
    ...
    func testPartitionMovesOpenAndMergedPullRequestGroupsAboveRegularWorktrees() {
        let prWorkspace = workspaceWithPullRequest(
            title: "PR Workspace",
            number: 1910,
            status: .open,
            branch: "feature/pr",
            url: "https://github.com/feritzcan2/termloop/pull/1910"
        )
        ...
        let mergedWorkspace = workspaceWithPullRequest(
            ...
            status: .merged,
            branch: "feature/merged",
            url: "https://github.com/feritzcan2/termloop/pull/1911"
        )

        let plainWorkspace = Workspace(title: "Plain Workspace")

        let sections = WorktreeAgentsSectionPartition.partition(
            [
                WorktreeAgentsGroup(branch: "feature/pr", workspaces: [prWorkspace], worktreePath: nil),
                WorktreeAgentsGroup(branch: "feature/merged", workspaces: [mergedWorkspace], worktreePath: nil),
                WorktreeAgentsGroup(branch: "feature/plain", workspaces: [plainWorkspace], worktreePath: nil)
            ]
        )

        XCTAssertEqual(sections.openPullRequests.map { $0.branch }, ["feature/pr"])
        XCTAssertEqual(sections.mergedPullRequests.map { $0.branch }, ["feature/merged"])
        XCTAssertEqual(sections.worktrees.map { $0.branch }, ["feature/plain"])
    }
}
...
final class TerminalControllerSidebarDedupeTests: XCTestCase {
    ...
    func testShouldReplacePullRequestReturnsTrueWhenCurrentStateIsStale() throws {
        let url = try XCTUnwrap(URL(string: "https://github.com/feritzcan2/termloop/pull/42"))
    ...
    func testNormalizeReportedDirectoryTrimsWhitespace() {
        XCTAssertEqual(
            TerminalController.normalizeReportedDirectory("   /Users/termloop/project   "),
            "/Users/termloop/project"
        )
    }

    func testNormalizeReportedDirectoryResolvesFileURL() {
        XCTAssertEqual(
            TerminalController.normalizeReportedDirectory("file:///Users/termloop/project"),
            "/Users/termloop/project"
        )
    }
...
@MainActor
final class TerminalAgentAggregateResolverTests: XCTestCase {
    override func tearDown() {
        TerminalAgentActivityStore.shared.replaceForTesting([:])
        super.tearDown()
    }

    func testAggregatePrioritizesErrorThenNeedsInputThenRunningThenCompleted() {
        let running = Workspace(title: "Running")
        let needsInput = Workspace(title: "Needs Input")
        let completed = Workspace(title: "Completed")
        let error = Workspace(title: "Error")

        TerminalAgentActivityStore.shared.replaceForTesting([
            running.id: TerminalAgentActivityState(
            ...
            needsInput.id: TerminalAgentActivityState(
                workspaceId: needsInput.id,
                agentId: "codex",
                phase: .waiting,
            ...
            completed.id: TerminalAgentActivityState(
            ...
            error.id: TerminalAgentActivityState(
                workspaceId: error.id,
                agentId: "codex",
                phase: .failed,
                attentionKind: .error,
                preview: nil,
        ...
        let summary = TerminalAgentActivityStore.shared.aggregate(for: [
            running, needsInput, completed, error
        ])

        XCTAssertEqual(summary.runningCount, 1)
        XCTAssertEqual(summary.needsInputCount, 1)
        XCTAssertEqual(summary.completedCount, 1)
        XCTAssertEqual(summary.errorCount, 1)
        XCTAssertEqual(summary.dominantState, .error)
        XCTAssertTrue(summary.hasVisibleActivity)
    }

    func testAggregateUsesCompletedStatusEntryFallback() {
        let workspace = Workspace(title: "Fallback")
        WorkspaceMetadataStore.shared.setTerminalAgentId("codex", for: workspace.id)
        ...
        let summary = TerminalAgentActivityStore.shared.aggregate(for: [workspace])

        XCTAssertEqual(summary.completedCount, 1)
        XCTAssertEqual(summary.dominantState, .completed)
        XCTAssertNotNil(summary.latestActivityAt)
    }
}

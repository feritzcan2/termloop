...
final class TabManagerChildExitCloseTests: XCTestCase {
    func testChildExitOnLastPanelPromptsAndKeepsSelectedWorkspaceOpenWhenCancelled() {
        ...
        XCTAssertEqual(
            ...
            second.id,
            "Expected child-exit cancellation to leave the workspace selected"
        )
        XCTAssertEqual(prompts.count, 1)
        XCTAssertEqual(
            prompts.first?.title,
            String(localized: "dialog.childExitedTerminal.title", defaultValue: "Delete terminal?")
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(second.panels.count, 1, "Expected the exited terminal to remain visible")
        XCTAssertNotNil(second.panels[secondPanelId], "Expected cancellation to keep the exited panel")
    }

    func testChildExitOnLastPanelClosesWorkspaceAfterConfirmation() {
        let manager = TabManager()
        let first = manager.tabs[0]
        let second = manager.addWorkspace()
        ...
        XCTAssertEqual(
            manager.selectedTabId,
            first.id,
            "Expected confirmation to close the selected workspace"
        )
        XCTAssertNil(second.panels[secondPanelId])
        XCTAssertTrue(second.panels.isEmpty)
    }
    ...
    func testChildExitOnLastRemotePanelKeepsWorkspaceAndDemotesToLocal() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/termloop-debug-test.sock",
...
final class TabManagerPullRequestProbeTests: XCTestCase {
    func testGitHubRepositorySlugsPrioritizeOriginThenUpstreamAndDeduplicate() {
        let output = """
        origin https://github.com/austinwang/cmux.git (fetch)
        origin https://github.com/austinwang/cmux.git (push)
        upstream git@github.com:feritzcan2/termloop.git (fetch)
        upstream git@github.com:feritzcan2/termloop.git (push)
        backup ssh://git@github.com/feritzcan2/termloop.git (fetch)
        mirror https://gitlab.com/feritzcan2/termloop.git (fetch)
        """

        XCTAssertEqual(
            TabManager.githubRepositorySlugs(fromGitRemoteVOutput: output),
            ["austinwang/cmux", "feritzcan2/termloop"]
        )
    }

    func testGitRemoteParserDetectsAzureDevOpsRemoteFormats() {
        let output = """
        origin https://dev.azure.com/my-org/MyProject/_git/Repo.Name (fetch)
        origin https://dev.azure.com/my-org/MyProject/_git/Repo.Name (push)
        ssh git@ssh.dev.azure.com:v3/other-org/OtherProject/OtherRepo.git (fetch)
        legacy https://legacyorg.visualstudio.com/LegacyProject/_git/LegacyRepo (fetch)
        github https://github.com/feritzcan2/termloop.git (fetch)
        """

        let identities = GitRemoteParser.identities(fromRemoteVOutput: output)
        let azureSlugs = identities.compactMap(\.azureDevOps?.slug)

        XCTAssertEqual(
            azureSlugs,
            [
                "my-org/MyProject/Repo.Name",
                "other-org/OtherProject/OtherRepo",
                "legacyorg/LegacyProject/LegacyRepo",
            ]
        )
        XCTAssertEqual(
            GitRemoteParser.githubRepositorySlugs(fromRemoteVOutput: output),
            ["feritzcan2/termloop"]
        )
    }

    func testGitRemoteParserFixtureMatrix() {
        let cases: [(String, GitHostKind, String)] = [
            ("https://github.com/openai/codex.git", .github, "openai/codex"),
            ("git@github.com:openai/codex.git", .github, "openai/codex"),
            ("ssh://git@github.com/openai/codex.git", .github, "openai/codex"),
            ("https://github.com/openai/codex.git?foo=bar", .github, "openai/codex"),
            ("https://github.enterprise.local/org/repo.git", .githubEnterprise, "org/repo"),
            ("git@github.enterprise.local:org/repo.git", .githubEnterprise, "org/repo"),
            ("https://dev.azure.com/example-org/ExampleProject/_git/sample-app", .azureDevOps, "example-org/ExampleProject/sample-app"),
            ("https://dev.azure.com/example-org/ExampleProject/_git/sample-app.git", .azureDevOps, "example-org/ExampleProject/sample-app"),
            ("git@ssh.dev.azure.com:v3/example-org/ExampleProject/sample-app", .azureDevOps, "example-org/ExampleProject/sample-app"),
            ("git@ssh.dev.azure.com:v3/example-org/ExampleProject/sample-app.git", .azureDevOps, "example-org/ExampleProject/sample-app"),
            ("https://example-org.visualstudio.com/ExampleProject/_git/sample-app", .azureDevOps, "example-org/ExampleProject/sample-app"),
            ("https://gitlab.com/org/repo.git", .unknown, "gitlab.com/org/repo"),
        ]

        for (remoteURL, expectedHost, expectedSlug) in cases {
            let identity = GitRemoteParser.identity(remoteName: "origin", cloneURL: remoteURL)
            XCTAssertEqual(identity?.host, expectedHost, remoteURL)
            XCTAssertEqual(identity?.displaySlug, expectedSlug, remoteURL)
        }
    }

    func testGitProjectStoreProjectInvalidationRefreshesCachedRemoteIdentity() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "termloop-git-project-invalidation-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer {
            GitProjectStore.shared.invalidateAll()
            try? fileManager.removeItem(at: repoURL)
        }

        GitProjectStore.shared.invalidateAll()
        _ = try runGit(["init"], in: repoURL)
        _ = try runGit(["remote", "add", "origin", "https://github.com/acme/old.git"], in: repoURL)

        let first = GitProjectStore.shared.identity(for: repoURL.path)
        XCTAssertEqual(first?.primaryRemote?.displaySlug, "acme/old")

        let rawCommonDir = try runGit(["rev-parse", "--git-common-dir"], in: repoURL)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let commonDir = rawCommonDir.hasPrefix("/")
            ? rawCommonDir
            : URL(fileURLWithPath: rawCommonDir, relativeTo: repoURL).standardizedFileURL.path

        _ = try runGit(["remote", "set-url", "origin", "https://github.com/acme/new.git"], in: repoURL)

        XCTAssertEqual(
            GitProjectStore.shared.identity(for: repoURL.path)?.primaryRemote?.displaySlug,
            "acme/old",
            "Precondition: store should serve the cached identity before explicit invalidation"
        )

        GitPresentationInvalidationCenter.invalidate(
            [.project(commonDir)],
            reason: "test",
            source: "remote-set-url"
        )
    ...
    func testGitProjectStoreRemotesPublisherRefetchesOnProjectInvalidation() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "termloop-git-project-publisher-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer {
        ...
        _ = try runGit(["init"], in: repoURL)
        _ = try runGit(["remote", "add", "origin", "https://github.com/acme/old.git"], in: repoURL)
        _ = GitProjectStore.shared.identity(for: repoURL.path)

        let expectation = expectation(description: "project remotes publisher refreshes")
        var cancellable: AnyCancellable?
        cancellable = GitProjectStore.shared.remotesPublisher(for: repoURL.path)
            .sink { remotes in
                if remotes.first?.displaySlug == "acme/new" {
                    expectation.fulfill()
                }
            }
    ...
    func testGitWorktreeBranchPublisherRefetchesOnWorktreeInvalidation() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "termloop-git-worktree-publisher-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer {
            GitWorktreePresentationStore.shared.invalidateAll()
            try? fileManager.removeItem(at: repoURL)
        }

        GitWorktreePresentationStore.shared.invalidateAll()
        _ = try runGit(["init"], in: repoURL)
        _ = GitWorktreePresentationStore.shared.snapshot(for: repoURL.path)

        let expectation = expectation(description: "worktree branch publisher refreshes")
        var cancellable: AnyCancellable?
        cancellable = GitWorktreePresentationStore.shared.branchPublisher(for: repoURL.path)
            .sink { branch in
                if branch == "feature/reactive-git" {
                    expectation.fulfill()
                }
            }
        defer { cancellable?.cancel() }

        _ = try runGit(["checkout", "-b", "feature/reactive-git"], in: repoURL)
        GitPresentationInvalidationCenter.invalidate(
            [.worktree(repoURL.path)],
            reason: "test",
            source: "checkout"
        )
    ...
    func testGitHostAuthResolverUsesAzurePATEnvironment() {
        setenv("ADO_PAT", "secret-token", 1)
        defer {
            unsetenv("ADO_PAT")
        ...
        let identity = GitRemoteParser.identity(
            remoteName: "origin",
            cloneURL: "https://dev.azure.com/example-org/ExampleProject/_git/sample-app"
        )
        let authorization = identity.flatMap(GitHostAuthResolver.authorization)

        XCTAssertEqual(authorization?.source, .personalAccessToken)
        XCTAssertEqual(
            authorization?.headerValue,
            "Basic \(Data("termloop:secret-token".utf8).base64EncodedString())"
        )
    }

    func testGitCommandRunnerTimesOutExternalAlias() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "termloop-git-timeout-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        ...
        XCTAssertThrowsError(
            try GitCommandRunner.runThrowing(
                ["-c", "alias.pause=!sleep 2", "pause"],
                in: repoURL.path,
                kind: .genericRead,
                caller: "test.timeout",
                timeout: 0.1
            ...
            guard case GitCommandRunner.RunError.timedOut = error else {
    ...
    func testGitCommandRunnerDrainsLargeStatusOutput() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "termloop-git-drain-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
        ...
        for index in 0..<4_000 {
            let fileURL = repoURL.appendingPathComponent("generated-\(index)-xxxxxxxxxxxxxxxxxxxxxxxx.txt")
            try "x\n".write(to: fileURL, atomically: true, encoding: .utf8)
        }

        let output = try GitCommandRunner.runThrowing(
            ["status", "--porcelain=v1"],
            in: repoURL.path,
            kind: .status,
            caller: "test.drain",
    ...
    func testGitCommandRunnerDisablesOptionalLocksForReadCommands() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "termloop-git-optional-locks-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
        ...
        let output = try GitCommandRunner.runThrowing(
            [
                "-c",
                #"alias.print-env=!sh -c 'printf %s "$GIT_OPTIONAL_LOCKS"'"#,
                "print-env",
            ],
            in: repoURL.path,
            kind: .genericRead,
            caller: "test.optionalLocks",
            timeout: 5
        )
        XCTAssertEqual(output, "0")
    }

    func testGitPullRequestProbeMapsAzureCompletedStatusAndPagedBranchEndpoint() throws {
        let data = Data("""
        {
          "value": [
            {
              ...
              "status": "completed",
              "title": "DEMO-504",
              "sourceRefName": "refs/heads/feature/DEMO-504",
              "targetRefName": "refs/heads/development",
              "creationDate": "2026-04-01T10:00:00Z",
              "closedDate": "2026-04-02T10:00:00Z"
            }
          ]
        }
        """.utf8)

        let items = GitHostPullRequestProbe.decodedProbeItemsForTests(
            repoSlug: "example-org/ExampleProject/sample-app",
            data: data
        )
        XCTAssertEqual(items?.first?.state, "MERGED")
        XCTAssertEqual(items?.first?.statusDetail, "completed")
        XCTAssertEqual(items?.first?.headRefName, "feature/DEMO-504")

        let endpoint = GitHostPullRequestProbe.branchEndpointForTests(
            repoSlug: "example-org/ExampleProject/sample-app",
            branch: "feature/DEMO-504",
            page: 3,
            pageSize: 100
        )
        XCTAssertTrue(endpoint?.contains("$skip=200") == true || endpoint?.contains("%24skip=200") == true)
    }

    func testGitPresentationInvalidationWatcherDebouncesRapidEvents() {
        let expectation = expectation(description: "debounced invalidation")
        var count = 0
        let token = GitPresentationInvalidationCenter.observe { event in
            guard event.reason == "debounce-test" else { return }
            count += 1
            expectation.fulfill()
        }
        defer { token.cancel() }
        ...
        for _ in 0..<5 {
            GitPresentationInvalidationWatcher.shared.debugScheduleInvalidationForTests(
                target: .worktree("/tmp/termloop-debounce-test"),
                reason: "debounce-test",
                source: "unit-test"
            )
        }
    ...
    func testShouldSkipWorkspacePullRequestLookupOnlyForExactProtectedBranches() {
        ...
        XCTAssertTrue(TabManager.shouldSkipWorkspacePullRequestLookup(branch: "dev"))
        XCTAssertTrue(TabManager.shouldSkipWorkspacePullRequestLookup(branch: "development"))
        XCTAssertTrue(TabManager.shouldSkipWorkspacePullRequestLookup(branch: " master \n"))
        XCTAssertTrue(TabManager.shouldSkipWorkspacePullRequestLookup(branch: " development \n"))
        ...
        XCTAssertFalse(TabManager.shouldSkipWorkspacePullRequestLookup(branch: "Dev"))
        XCTAssertFalse(TabManager.shouldSkipWorkspacePullRequestLookup(branch: "development-foo"))
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesIncludeMainAndMasterPanels() throws {
        ...
        let staleURL = try XCTUnwrap(URL(string: "https://github.com/feritzcan2/termloop/pull/371"))
    ...
    func testPeriodicWorkspaceGitMetadataRefreshClearsStalePullRequestAfterBranchReset() throws {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1052,
            label: "PR",
            url: try XCTUnwrap(URL(string: "https://github.com/feritzcan2/termloop/pull/1052")),
            status: .open,
            branch: "feature/sidebar-pr"
        )

        XCTAssertEqual(workspace.panelGitBranches[panelId]?.branch, "feature/sidebar-pr")
        XCTAssertEqual(workspace.primaryPanelPullRequest(panelId: panelId)?.number, 1052)

...
final class KeyboardShortcutSettingsFileStoreTests: XCTestCase {
    ...
    private let settingsFileBackupsDefaultsKey = "termloop.settingsFile.backups.v1"
    ...
    func testBootstrapCreatesCommentedTemplateWhenPrimaryAndFallbackAreMissing() throws {
        ...
        XCTAssertTrue(contents.contains(#""$schema": "https://raw.githubusercontent.com/feritzcan2/termloop/main/web/data/cmux-settings.schema.json""#))
    ...
    func testSettingsFileStoreParsesJSONCCommentsAndTrailingCommas() throws {
        ...
        try writeSettingsFile(
            """
            {
              "$schema": "https://raw.githubusercontent.com/feritzcan2/termloop/main/web/data/cmux-settings.schema.json",
              "schemaVersion": 1,
              // tmux-like prefix
              "shortcuts": {
                "bindings": {
...
final class WorkspaceCreationConfigSanitizationTests: XCTestCase {
    private final class UnsafeConfigSnapshotTabManager: TabManager {
        ...
        func installInjectedConfig(fontSize: Float) {
            ...
            config.workingDirectory = "/tmp/termloop-workspace-snapshot"
            config.command = "echo snapshot"
            config.environmentVariables = ["TERMLOOP_INHERITED_ENV": "1"]
    ...
    func testAddWorkspacePassesSanitizedInheritedConfigTemplate() {
        ...
        XCTAssertEqual(Double(capturedConfig.fontSize), 19, accuracy: 0.001)
...
final class TerminalConfigFreshShellSanitizationTests: XCTestCase {
    func testSanitizedForFreshShellStripsRuntimeShellStateButKeepsVisualConfig() {
        var config = CmuxSurfaceConfigTemplate()
        config.fontSize = 17
        config.workingDirectory = "/tmp/worktree"
        config.command = "exec /bin/zsh"
        config.environmentVariables = ["PATH": "/tmp/bin", "ZDOTDIR": "/tmp/z"]
        config.initialInput = "echo hi"
        config.waitAfterCommand = true

        let sanitized = config.sanitizedForFreshShell()

        XCTAssertEqual(Double(sanitized?.fontSize ?? .nan), 17, accuracy: 0.001)
        XCTAssertNil(sanitized?.workingDirectory)
        XCTAssertNil(sanitized?.command)
        XCTAssertTrue(sanitized?.environmentVariables.isEmpty == true)
        XCTAssertNil(sanitized?.initialInput)
        XCTAssertEqual(sanitized?.waitAfterCommand, false)
    }
}
...
final class WorkspaceSplitWorkingDirectoryTests: XCTestCase {
    ...
    func testNewTerminalSplitFallsBackToRequestedWorkingDirectoryWhenReportedDirectoryIsStale() {
        ...
        let requestedDirectory = "/tmp/termloop-requested-split-cwd-\(UUID().uuidString)"
...
final class WorkspacePanelGitBranchTests: XCTestCase {
    ...
    func testUpdatingFocusedPanelPullRequestWithSameStateDoesNotRepublishWorkspace() {
        ...
        let pullRequestURL = URL(string: "https://github.com/feritzcan2/termloop/pull/2388")!
    ...
    func testSidebarDerivedCollectionsMatchWhenUsingPrecomputedPanelOrder() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 101,
            label: "PR",
            url: URL(string: "https://github.com/feritzcan2/termloop/pull/101")!,
            status: .open
        )
        workspace.updatePanelPullRequest(
            ...
            number: 18,
            label: "MR",
            url: URL(string: "https://gitlab.com/manaflow/termloop/-/merge_requests/18")!,
            status: .merged
        )

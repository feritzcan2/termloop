...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let relayDir = home.appendingPathComponent(".termloop/relay/64011.shell")
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "SHELL=/bin/zsh",
                "USER=\(NSUserName())",
                "TERMLOOP_REAL_ZDOTDIR=\(home.path)",
    ...
    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        ...
        let relayDir = home.appendingPathComponent(".termloop/relay")
        let socketAddrURL = home.appendingPathComponent(".termloop/socket_addr")
    ...
    func testRemoteTerminalSessionEndRequestsControlMasterCleanupWhenWorkspaceDemotes() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/termloop-ssh-%C",
        ...
        XCTAssertEqual(
            ...
            [
                ...
                "-o", "ControlPath=/tmp/termloop-ssh-%C",
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/termloop-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/termloop-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/termloop-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/termloop-ssh-%C"))
    ...
    func testDaemonTransportArgumentsReuseConfiguredControlPath() {
        ...
        let arguments = WorkspaceRemoteSSHBatchCommandBuilder.daemonTransportArguments(
            ...
            remotePath: "/remote/termloopd-remote"
        )

        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains(where: { $0 == "ControlPath /tmp/termloop-ssh-%C" || $0 == "ControlPath=/tmp/termloop-ssh-%C" }))
        XCTAssertTrue(arguments.contains("cmux-macmini"))
        XCTAssertTrue(arguments.last?.contains("/remote/termloopd-remote") ?? false)
    }
    ...
    func testReverseRelayControlMasterArgumentsReuseConfiguredControlSocket() throws {
        ...
        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains("ControlPath=/tmp/termloop-ssh-%C"))
    ...
    func testDetectedSSHSessionBracketsIPv6LiteralSCPDestination() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/termloop-drop-123.png")
    }
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    private func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "cmux",
                  item.path.contains(".app/Contents/Resources/bin/termloop") else {
                continue
            }
            return item.path
        }

        throw XCTSkip("Bundled termloop CLI not found in \(appBundleURL.path)")
    }
    ...
    func testSSHCommandCreatesConfiguresAndSelectsRemoteWorkspaceViaCLI() throws {
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "--ssh-option", "ControlPath /tmp/termloop-ssh-%C",
        ...
        XCTAssertTrue(sshOptions.contains("ControlPath /tmp/termloop-ssh-%C"))
    ...
    func testSSHBootstrapStartupCommandPassesRemoteInstallScriptAsSingleSSHCommand() throws {
        ...
        let fileManager = FileManager.default
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent("termloop-ssh-bootstrap-\(UUID().uuidString)")
        ...
        args = sys.argv[1:]
        with open(os.environ["TERMLOOP_FAKE_SSH_LOG"], "a", encoding="utf-8") as handle:
        ...
        startupEnvironment["TERMLOOP_FAKE_SSH_LOG"] = fakeSSHLog.path
        startupEnvironment["TERMLOOP_SOCKET_PATH"] = socketPath
        startupEnvironment["TERMLOOP_WORKSPACE_ID"] = workspaceID
        startupEnvironment["TERMLOOP_CLI_SENTRY_DISABLED"] = "1"
        startupEnvironment["TERMLOOP_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testNotifyInTmuxPrefersCallerTTYOverStaleValidSurfaceID() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["TERMLOOP_SOCKET_PATH"] = socketPath
        environment["TERMLOOP_WORKSPACE_ID"] = workspaceId
        environment["TERMLOOP_SURFACE_ID"] = staleSurface
        environment["TERMLOOP_CLI_TTY_NAME"] = callerTTY

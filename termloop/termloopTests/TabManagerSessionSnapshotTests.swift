...
final class TabManagerSessionSnapshotTests: XCTestCase {
    ...
    func testSessionAutosaveFingerprintChangesWhenVisibleWorkspaceTitleChanges() {
        let manager = TabManager()
        ...
        workspace.applyProcessTitle("Prompt Refactor")
        let renamed = manager.sessionAutosaveFingerprint()

        XCTAssertNotEqual(
            baseline,
            renamed,
            "autosave fingerprint should track untitled workspace name changes so forced restarts restore the latest agent title"
        )
    }
}

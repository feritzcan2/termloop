...
final class TerminalControllerSocketSecurityTests: XCTestCase {
    ...
    func testSocketPathOwnershipPredicateRejectsReboundSocketPath() throws {
        let socketPath = makeSocketPath("rebound")
        let firstListener = try bindUnixListener(at: socketPath)
        defer { Darwin.close(firstListener) }

        XCTAssertTrue(
            TerminalController.shouldUnlinkSocketPathOwnedByListener(
                socketPath,
                listenerSocket: firstListener
            )
        )

        XCTAssertEqual(Darwin.unlink(socketPath), 0)
        let secondListener = try bindUnixListener(at: socketPath)
        defer {
            Darwin.close(secondListener)
    ...
    private nonisolated func bindUnixListener(at socketPath: String) throws -> Int32 {
        Darwin.unlink(socketPath)
        let fd = Darwin.socket(AF_UNIX, SOCK_STREAM, 0)
        guard fd >= 0 else {
            throw posixError("socket(AF_UNIX)")
        ...
        guard bindResult == 0 else {
            let error = posixError("bind(\(socketPath))")
            Darwin.close(fd)
            throw error
        }
        guard Darwin.listen(fd, 1) == 0 else {
            let error = posixError("listen(\(socketPath))")
            Darwin.close(fd)
            throw error
        }
        return fd

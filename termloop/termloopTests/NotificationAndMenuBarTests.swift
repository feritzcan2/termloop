...
final class NotificationDockBadgeTests: XCTestCase {
    ...
    func testFocusedTerminalSuppressedNotificationRunsCustomCommand() throws {
        ...
        defaults.set("none", forKey: NotificationSoundSettings.key)
        defaults.set(
            "printf '%s\\n%s\\n%s' \"$TERMLOOP_NOTIFICATION_TITLE\" \"$TERMLOOP_NOTIFICATION_SUBTITLE\" \"$TERMLOOP_NOTIFICATION_BODY\" > '\(commandOutputURL.path)'",
...
final class MenuBarBuildHintFormatterTests: XCTestCase {
    func testReleaseBuildShowsNoHint() {
        XCTAssertNil(MenuBarBuildHintFormatter.menuTitle(appName: "TermLoop DEV menubar-extra", isDebugBuild: false))
    }

    func testDebugBuildWithTagShowsTag() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "TermLoop DEV menubar-extra", isDebugBuild: true),
    ...
    func testDebugBuildWithoutTagShowsUntagged() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "TermLoop DEV", isDebugBuild: true),
            "Build: DEV (untagged)"
        )
    }
}

...
- **TCP bridge in `TerminalController`:** runs alongside the Unix listener (configured via `socketControl.tcpPort` / `socketControl.tcpBindAll` defaults or `TERMLOOP_SOCKET_TCP_*` env). `handleClient(isTcpClient:true)` skips the `cmuxOnly` ancestry check (no peer PID) and refuses the connection unless `accessMode` is `.password` or `.allowAll`. Default bind is `0.0.0.0` for Tailscale peers.

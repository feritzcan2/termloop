...
<p align="center">
  <a href="https://github.com/feritzcan2/termloop/releases/latest/download/termloop-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="Baixar TermLoop para macOS" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="Captura de tela do TermLoop" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ Vídeo de demonstração</a> · <a href="https://termloop.ai/blog/zen-of-cmux">O Zen do TermLoop</a>
</p>
...
- **Comandos personalizados** — Defina ações específicas do projeto em [`cmux.json`](https://termloop.ai/docs/custom-commands) que são lançadas pela paleta de comandos
...
Abra o `.dmg` e arraste o TermLoop para a pasta Aplicativos. O TermLoop se atualiza automaticamente via Sparkle, então você só precisa baixar uma vez.
...
Eu tentei alguns orquestradores de código, mas a maioria era apps Electron/Tauri e o desempenho me incomodava. Eu também prefiro o terminal, já que orquestradores GUI te prendem no fluxo de trabalho deles. Então eu construí o TermLoop como um app nativo macOS em Swift/AppKit. Ele usa o libghostty para renderização do terminal e lê sua configuração existente do Ghostty para temas, fontes e cores.
...
O TermLoop não é prescritivo sobre como os desenvolvedores usam suas ferramentas. É um terminal e navegador com uma CLI, e o resto é com você.

O TermLoop é uma primitiva, não uma solução. Ele te dá um terminal, um navegador, notificações, workspaces, divisões, abas e uma CLI para controlar tudo isso. O TermLoop não te força a usar agentes de programação de uma forma específica. O que você constrói com as primitivas é seu.
...
Para mais informações sobre como configurar o TermLoop, [acesse nossa documentação](https://termloop.ai/docs/getting-started?utm_source=readme).
...
[Baixar TermLoop NIGHTLY](https://github.com/feritzcan2/termloop/releases/download/nightly/termloop-nightly-macos.dmg)

O TermLoop NIGHTLY é um app separado com seu próprio bundle ID, então roda ao lado da versão estável. Construído automaticamente a partir do último commit em `main` e se atualiza automaticamente via seu próprio feed Sparkle.

## Restauração de sessão (comportamento atual)

Ao reiniciar, o TermLoop atualmente restaura apenas o layout do app e metadados:
...
O TermLoop **não** restaura o estado de processos ativos dentro de apps de terminal. Por exemplo, sessões ativas de Claude Code/tmux/vim não são retomadas após reiniciar ainda.
...
- Participe da conversa no [Discord](https://github.com/feritzcan2/termloop/discussions)
- Crie e participe de [issues no GitHub](https://github.com/feritzcan2/termloop/issues) e [discussões](https://github.com/feritzcan2/termloop/discussions)
- Nos conte o que você está construindo com o TermLoop
...
O TermLoop é gratuito, open source, e sempre será. Se você gostaria de apoiar o desenvolvimento e ter acesso antecipado ao que está por vir:
...
- **Acesso antecipado: TermLoop AI que te dá contexto sobre cada workspace, aba e painel**
...
TermLoop é open source sob [GPL-3.0-or-later](LICENSE).

Se sua organização não puder cumprir a GPL, uma licença comercial está disponível. Entre em contato com [feritzcan93@gmail.com](mailto:feritzcan93@gmail.com) para detalhes.

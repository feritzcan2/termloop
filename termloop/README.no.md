...
<p align="center">
  <a href="https://github.com/feritzcan2/termloop/releases/latest/download/termloop-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="Last ned TermLoop for macOS" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="TermLoop skjermbilde" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ Demovideo</a> · <a href="https://termloop.ai/blog/zen-of-cmux">The Zen of TermLoop</a>
</p>
...
- **Egendefinerte kommandoer** — Definer prosjektspesifikke handlinger i [`cmux.json`](https://termloop.ai/docs/custom-commands) som startes fra kommandopaletten
...
Åpne `.dmg`-filen og dra TermLoop til Programmer-mappen. TermLoop oppdaterer seg selv automatisk via Sparkle, så du trenger bare å laste ned én gang.
...
Jeg prøvde noen kodeorkestratorer, men de fleste var Electron/Tauri-apper og ytelsen irriterte meg. Jeg foretrekker også terminalen siden GUI-orkestratorer låser deg inn i arbeidsflyten deres. Så jeg bygde TermLoop som en nativ macOS-app i Swift/AppKit. Den bruker libghostty for terminalgjengivelse og leser din eksisterende Ghostty-konfigurasjon for temaer, skrifttyper og farger.
...
TermLoop er ikke foreskrivende om hvordan utviklere bruker verktøyene sine. Det er en terminal og nettleser med en CLI, og resten er opp til deg.

TermLoop er en primitiv, ikke en løsning. Det gir deg en terminal, en nettleser, varsler, arbeidsområder, delinger, faner og en CLI for å kontrollere alt sammen. TermLoop tvinger deg ikke inn i en bestemt måte å bruke kodeagenter på. Hva du bygger med primitivene er ditt.
...
For mer informasjon om hvordan du konfigurerer TermLoop, [gå til dokumentasjonen vår](https://termloop.ai/docs/getting-started?utm_source=readme).
...
[Last ned TermLoop NIGHTLY](https://github.com/feritzcan2/termloop/releases/download/nightly/termloop-nightly-macos.dmg)

TermLoop NIGHTLY er en separat app med sin egen bundle-ID, så den kjører ved siden av den stabile versjonen. Bygges automatisk fra den siste `main`-commiten og oppdateres automatisk via sin egen Sparkle-feed.

## Sesjonssgjenoppretting (nåværende oppførsel)

Ved omstart gjenoppretter TermLoop for øyeblikket kun applayouten og metadata:
- Vindu-/arbeidsområde-/panellayout
- Arbeidsmapper
- Terminal-rullingshistorikk (best effort)
- Nettleser-URL og navigasjonshistorikk

TermLoop gjenoppretter **ikke** aktive prosesstilstander inne i terminalapper. For eksempel blir aktive Claude Code/tmux/vim-sesjoner ikke gjenopptatt etter omstart ennå.
...
- Bli med i samtalen på [Discord](https://github.com/feritzcan2/termloop/discussions)
- Opprett og delta i [GitHub-issues](https://github.com/feritzcan2/termloop/issues) og [diskusjoner](https://github.com/feritzcan2/termloop/discussions)
- Fortell oss hva du bygger med TermLoop
...
TermLoop er gratis, åpen kildekode, og vil alltid være det. Hvis du vil støtte utviklingen og få tidlig tilgang til det som kommer:

**[Få Grunnleggerutgaven](https://buy.stripe.com/3cI00j2Ld0it5OU33r5EY0q)**

- **Prioriterte funksjonsforespørsler/feilrettinger**
- **Tidlig tilgang: TermLoop AI som gir deg kontekst om hvert arbeidsområde, fane og panel**
...
TermLoop er åpen kildekode under [GPL-3.0-or-later](LICENSE).

Hvis organisasjonen din ikke kan overholde GPL, er en kommersiell lisens tilgjengelig. Kontakt [feritzcan93@gmail.com](mailto:feritzcan93@gmail.com) for detaljer.

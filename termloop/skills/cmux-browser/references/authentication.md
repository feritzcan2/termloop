...
Login flows, session persistence, OAuth, and 2FA patterns for termloop browser surfaces.
...
if [ -f "$STATE_FILE" ]; then
  termloop browser "$SURFACE" state load "$STATE_FILE"
fi
...
if printf '%s' "$URL" | grep -q '/login'; then
  termloop browser "$SURFACE" snapshot --interactive
  termloop browser "$SURFACE" fill e1 "$APP_USERNAME"
  termloop browser "$SURFACE" fill e2 "$APP_PASSWORD"
  termloop browser "$SURFACE" click e3
  termloop browser "$SURFACE" wait --url-contains "/dashboard" --timeout-ms 20000
  termloop browser "$SURFACE" state save "$STATE_FILE"
fi
```

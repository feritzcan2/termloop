...
private let agentLifecycleCLILogger = Logger(
    subsystem: "com.termloop.fork",
    category: "agent-lifecycle.cli"
)

private func agentLifecycleCLILog(_ message: @autoclosure () -> String) {
    #if DEBUG
    let rendered = message()
    agentLifecycleCLILogger.debug("\(rendered, privacy: .public)")
    #endif
}
...
private final class CLISocketSentryTelemetry {
    ...
    private static let sentryInfoDictionaryKey = "TermLoopSentryDSN"
    private static let sentryEnvironmentKeys = ["TERMLOOP_SENTRY_DSN", "SENTRY_DSN"]
    ...
    private static func currentSentryBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["TERMLOOP_BUNDLE_ID"]?
    ...
    private static func configuredDSN(
        ...
        for key in sentryEnvironmentKeys {
            if let dsn = normalizedDSN(processEnv[key]) {
                return dsn
            }
        }

        if let rawValue = currentSentryBundle()?.infoDictionary?[sentryInfoDictionaryKey] as? String,
           let dsn = normalizedDSN(rawValue) {
            return dsn
        }
    ...
    private static func normalizedDSN(_ rawValue: String?) -> String? {
    ...
    init(command: String, commandArgs: [String], socketPath: String, processEnv: [String: String]) {
        ...
        self.envSocketPath = processEnv["TERMLOOP_SOCKET_PATH"] ?? processEnv["TERMLOOP_SOCKET"]
        self.workspaceId = processEnv["TERMLOOP_WORKSPACE_ID"]
        self.surfaceId = processEnv["TERMLOOP_SURFACE_ID"]
        self.disabledByEnv =
            processEnv["TERMLOOP_CLI_SENTRY_DISABLED"] == "1" ||
            processEnv["TERMLOOP_CLAUDE_HOOK_SENTRY_DISABLED"] == "1"
    }
    ...
    func breadcrumb(_ message: String, data: [String: Any] = [:]) {
        ...
        let crumb = Breadcrumb(level: .info, category: "termloop.cli")
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "termloop-cli", key: "component")
    ...
    private var shouldEmit: Bool {
        Self.telemetryEnabled && !disabledByEnv
    }
    ...
    private func socketDiagnostics() -> [String: Any] {
        ...
        if CLISocketPathResolver.isImplicitDefaultPath(socketPath),
           ...
           !taggedSockets.isEmpty {
            context["possible_root_cause"] = "TERMLOOP_SOCKET_PATH/TERMLOOP_SOCKET missing while tagged sockets exist"
        }
    ...
    private static func ensureStarted() -> Bool {
        ...
        guard let dsn = configuredDSN() else {
            return false
        }
...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.termloop/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = processEnv["TERMLOOP_CLAUDE_HOOK_STATE_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
enum SocketPasswordResolver {
    private static let service = "com.termloop.app.socket-control"
    private static let account = "local-socket-password"
    private static let directoryName = "termloop"
    ...
    static func resolve(explicit: String?, socketPath: String) -> String? {
        ...
        if let env = normalized(ProcessInfo.processInfo.environment["TERMLOOP_SOCKET_PASSWORD"]) {
            return env
        }
    ...
    private static func keychainScope(
        ...
        if let tag = normalized(environment["TERMLOOP_TAG"]) {
        ...
        let prefixes = ["termloop-debug-", "termloop-"]
...
private enum CLISocketPathResolver {
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = normalized(environment["TERMLOOP_TAG"]) {
            ...
            candidates.append("/tmp/termloop-debug-\(slug).sock")
            candidates.append("/tmp/termloop-\(slug).sock")
        }
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("termloop") && name.hasSuffix(".sock") {
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["TERMLOOP_CLI_RESPONSE_TIMEOUT_SEC"],
    ...
    private static func relayCredentials(for endpoint: RelayEndpoint) throws -> RelayCredentials {
        ...
        if let relayID = trimmedEnvValue(environment["TERMLOOP_RELAY_ID"]),
           let relayTokenHex = trimmedEnvValue(environment["TERMLOOP_RELAY_TOKEN"]),
        ...
        let authURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
            .appendingPathComponent(".termloop/relay/\(endpoint.port).auth", isDirectory: false)
    ...
    private func authenticateRelay(credentials: RelayCredentials) throws {
        ...
        guard let challengeData = challengeLine.data(using: .utf8),
              ...
              (challenge["protocol"] as? String) == "termloop-relay-auth",
    ...
    static func waitForConnectableSocket(path: String, timeout: TimeInterval) throws -> SocketClient {
        ...
        guard let watchDirectory = existingWatchDirectory(forPath: path) else {
            throw CLIError(message: "TermLoop app did not start in time (socket not found at \(path))")
        }
        ...
        let queue = DispatchQueue(label: "com.termloop.cli.socket-watch.\(UUID().uuidString)")
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.termloop.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct CMUXCLI {
    ...
    private static func debugSocketPathFromHintFile() -> String? {
        ...
        guard let hinted = normalizedEnvValue(raw),
              hinted.hasPrefix("/tmp/termloop-debug"),
    ...
    private static func defaultSocketPath(environment: [String: String]) -> String {
        if let explicit = normalizedEnvValue(environment["TERMLOOP_SOCKET_PATH"]) {
            return explicit
        }
#if DEBUG
        ...
        return "/tmp/termloop-debug.sock"
#else
        return "/tmp/termloop.sock"
#endif
    }

    private static func shouldRetrySocketConnect(command: String) -> Bool {
        command.hasSuffix("-hook")
    }

    private static func hookSocketConnectRetryTimeout(environment: [String: String]) -> TimeInterval {
        guard let raw = normalizedEnvValue(environment["TERMLOOP_HOOK_SOCKET_RETRY_SEC"]),
    ...
    func run() throws {
        ...
        let envSocketPath: String? = {
            for key in ["TERMLOOP_SOCKET_PATH", "TERMLOOP_SOCKET"] {
        ...
        // so help text is available even when termloop is not running.
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run 'termloop help' to see available commands.")
            return
        }
        ...
        // Codex hook handler: gracefully no-op when not inside TermLoop
        // (before socket connection, so it doesn't fail when no socket exists).
        // Codex strips TERMLOOP_SURFACE_ID from hook subprocess env in some
        // versions, so accept TERMLOOP_WORKSPACE_ID as a fallback indicator —
        // both are set on every termloop PTY by GhosttyTerminalView.
        if command == "codex-hook" {
            let env = ProcessInfo.processInfo.environment
            guard env["TERMLOOP_SURFACE_ID"] != nil || env["TERMLOOP_WORKSPACE_ID"] != nil else {
                print("{}")
                return
            }
        }
        ...
        // Cursor hook handler: gracefully no-op when not inside TermLoop
        ...
        // Gemini hook handler: gracefully no-op when not inside TermLoop
        ...
        var activeSocketPath = resolvedSocketPath
        var client = SocketClient(path: activeSocketPath)
        ...
        do {
            try client.connect()
            cliTelemetry.breadcrumb("socket.connect.success", data: ["path": activeSocketPath])
        } catch {
            var lastError = error
            let shouldRetry = Self.shouldRetrySocketConnect(command: command)
            let retryDeadline = Date().addingTimeInterval(
                shouldRetry ? Self.hookSocketConnectRetryTimeout(environment: processEnv) : 0
            )
            var didConnect = false

            while shouldRetry && Date() < retryDeadline {
                usleep(100_000)
                activeSocketPath = CLISocketPathResolver.resolve(
                    requestedPath: socketPath,
                    source: socketPathSource,
                    environment: processEnv
                )
                let retryClient = SocketClient(path: activeSocketPath)
                do {
                    try retryClient.connect()
                    client = retryClient
                    didConnect = true
                    cliTelemetry.breadcrumb("socket.connect.retry_success", data: ["path": activeSocketPath])
                    break
                } catch {
                    retryClient.close()
                    lastError = error
        ...
        case "rpc":
            guard let method = commandArgs.first?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !method.isEmpty else {
                throw CLIError(message: "Usage: termloop rpc <method> [json-params]")
            }
        ...
        case "ask-to", "ask_to":
            try runAskToCommand(commandArgs: commandArgs, client: client, jsonOutput: jsonOutput)
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"] : nil)
        ...
        // MARK: termloop-hook
        case "agent":
            exit(AgentsCLI.run(commandArgs, client: client, jsonOutput: jsonOutput))
        // MARK: /termloop-hook

        // MARK: termloop-hook
        case "session":
            exit(SessionReplayCLI.run(commandArgs))
        // MARK: /termloop-hook
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let callerWorkspaceArg = preferTTYFallback
                ? nil
                : (windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"],
        ...
        case "opencode-hook":
            ...
            do {
                try runOpenCodeHook(commandArgs: commandArgs, client: client, telemetry: cliTelemetry)
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: termloop markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: termloop markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: termloop markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: termloop markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: termloop markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    /// Open a path in termloop by creating a new workspace with the given directory.
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["TERMLOOP_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
    ...
    private func parseSSHCommandOptions(_ commandArgs: [String], localSocketPath: String = "", remoteRelayPort: Int = 0) throws -> SSHCommandOptions {
        ...
        guard let destination else {
            throw CLIError(message: "ssh requires a destination (example: termloop ssh user@host)")
        }
    ...
    private func stagedRemoteBootstrapCommandShell(
        ...
        lines.append("/bin/sh \"$HOME/.termloop/relay/\(remoteRelayPort).bootstrap.sh\"")
        return lines.joined(separator: "\n")
    }
    ...
    func buildInteractiveRemoteShellScript(
        ...
        commonShellExportLines.append("export PATH=\"$HOME/.termloop/bin:$PATH\"")
        commonShellExportLines.append("export TERMLOOP_BUNDLED_CLI_PATH=\"$HOME/.termloop/bin/termloop\"")
        commonShellExportLines.append("export TERMLOOP_SHELL_INTEGRATION_DIR=\"\(shellStateDir)\"")
        if let relaySocket {
            commonShellExportLines.append("export TERMLOOP_SOCKET_PATH=\(relaySocket)")
            commonShellExportLines.append("export TERMLOOP_SOCKET=\(relaySocket)")
        }
        ...
        zshShellLines.append(
            #"if [ "${TERMLOOP_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${TERMLOOP_SHELL_INTEGRATION_DIR}/termloop-zsh-integration.zsh" ]; then . "${TERMLOOP_SHELL_INTEGRATION_DIR}/termloop-zsh-integration.zsh"; fi"#
        )
        var bashShellLines = commonShellExportLines
        bashShellLines.append(
            #"if [ "${TERMLOOP_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${TERMLOOP_SHELL_INTEGRATION_DIR}/termloop-bash-integration.bash" ]; then . "${TERMLOOP_SHELL_INTEGRATION_DIR}/termloop-bash-integration.bash"; fi"#
        )
        ...
        let bundledZshIntegration = bundledShellIntegrationScript(named: "termloop-zsh-integration.zsh")
        let bundledBashIntegration = bundledShellIntegrationScript(named: "termloop-bash-integration.bash")
        let bashRCLines = [
        ...
        var outerLines: [String] = [
            "mkdir -p \"$HOME/.termloop/relay\"",
            "termloop_shell_dir=\"\(shellStateDir)\"",
            "mkdir -p \"$termloop_shell_dir\"",
        ]
        ...
        outerLines += [
            "    export TERMLOOP_REAL_ZDOTDIR=\"${ZDOTDIR:-$HOME}\"",
            "    export ZDOTDIR=\"$termloop_shell_dir\"",
        ...
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$TERMLOOP_LOGIN_SHELL\" --rcfile \"$termloop_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
        outerLines.append(contentsOf: commonShellExportLines)
    ...
    private func shellStateDirForRemoteRelayPort(_ remoteRelayPort: Int) -> String {
        "$HOME/.termloop/relay/\(max(remoteRelayPort, 0)).shell"
    }
    ...
    private func writeSSHStartupScript(_ scriptBody: String, remoteRelayPort: Int) throws -> String {
        ...
        let scriptURL = tempDir.appendingPathComponent(
            "termloop-ssh-startup-\(remoteRelayPort)-\(UUID().uuidString.lowercased()).sh"
        )
        let script = "#!/bin/sh\n\(scriptBody)\n"
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            "if [ -n \"${TERMLOOP_BUNDLED_CLI_PATH:-}\" ]",
            "&& [ -x \"${TERMLOOP_BUNDLED_CLI_PATH}\" ]",
            "&& [ -n \"${TERMLOOP_SOCKET_PATH:-}\" ]",
            "&& [ -n \"${TERMLOOP_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${TERMLOOP_SURFACE_ID:-}\" ]; then",
            "\"${TERMLOOP_BUNDLED_CLI_PATH}\" --socket \"${TERMLOOP_SOCKET_PATH}\" ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${TERMLOOP_WORKSPACE_ID}\" --surface \"${TERMLOOP_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "elif command -v termloop >/dev/null 2>&1",
            "&& [ -n \"${TERMLOOP_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${TERMLOOP_SURFACE_ID:-}\" ]; then",
            "termloop ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${TERMLOOP_WORKSPACE_ID}\" --surface \"${TERMLOOP_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
    ...
    private func runSSHSessionEnd(commandArgs: [String], client: SocketClient) throws {
        ...
        let workspaceRaw = optionValue(commandArgs, name: "--workspace") ?? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"]
        let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"]
        guard let workspaceRaw,
              let workspaceId = try normalizeWorkspaceHandle(workspaceRaw, client: client),
              !workspaceId.isEmpty else {
            throw CLIError(message: "ssh-session-end requires --workspace or TERMLOOP_WORKSPACE_ID")
        }
        guard let surfaceRaw,
            ...
            throw CLIError(message: "ssh-session-end requires --surface or TERMLOOP_SURFACE_ID")
        }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let downloadCommand = "gh release download \(releaseTag) --repo feritzcan2/termloop --pattern \(assetName)"
        let downloadChecksumsCommand = "gh release download \(releaseTag) --repo feritzcan2/termloop --pattern \(checksumsAssetName)"
        let checksumVerifyCommand = "shasum -a 256 -c \(checksumsAssetName) --ignore-missing"
        let signerWorkflow = releaseTag == "nightly"
            ? "feritzcan2/termloop/.github/workflows/nightly.yml"
            : "feritzcan2/termloop/.github/workflows/release.yml"
        let verifyCommand = "gh attestation verify ./\(assetName) --repo feritzcan2/termloop --signer-workflow \(signerWorkflow)"

        let payload: [String: Any] = [
            "app_version": remoteDaemonVersionString(from: info),
            "build": info["CFBundleVersion"] ?? NSNull(),
            "commit": info["TERMLOOPCommit"] ?? NSNull(),
            ...
            "dev_local_build_fallback": ProcessInfo.processInfo.environment["TERMLOOP_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1",
        ...
        if manifest == nil {
            print("note: this build has no embedded remote daemon manifest. Set TERMLOOP_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 only for dev builds.")
        }
    }
    ...
    private func remoteDaemonManifest() -> RemoteDaemonManifest? {
        for plistURL in candidateInfoPlistURLs() {
            guard let raw = NSDictionary(contentsOf: plistURL) as? [String: Any],
                  let rawManifest = raw["TERMLOOPRemoteDaemonManifestJSON"] as? String,
    ...
    private func remoteDaemonCacheURL(version: String, goOS: String, goArch: String) -> URL {
        ...
        do {
            ...
            return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("termloop-remote-daemons", isDirectory: true)
                .appendingPathComponent(version, isDirectory: true)
                .appendingPathComponent("\(goOS)-\(goArch)", isDirectory: true)
                .appendingPathComponent("termloopd-remote", isDirectory: false)
        }
        return root
    ...
    private func defaultSSHControlPathTemplate(remoteRelayPort: Int? = nil) -> String {
        if let remoteRelayPort, remoteRelayPort > 0 {
            return "/tmp/termloop-ssh-\(getuid())-\(remoteRelayPort)-%C"
        }
        return "/tmp/termloop-ssh-\(getuid())-%C"
    }
    ...
    private func cliDebugLog(_ message: @autoclosure () -> String) {
#if DEBUG
        let trimmedExplicit = ProcessInfo.processInfo.environment["TERMLOOP_DEBUG_LOG"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let path: String? = {
            if let trimmedExplicit, !trimmedExplicit.isEmpty {
                return trimmedExplicit
            }
            guard let marker = try? String(contentsOfFile: "/tmp/termloop-last-debug-log-path", encoding: .utf8) else {
                return nil
            }
        ...
        let line = "\(timestamp) [termloop-cli] \(message())\n"
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'termloop browser <surface> get url' to verify navigation")
            }
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["TERMLOOP_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("termloop-browser-screenshots-cli", isDirectory: true)
    ...
    private func runAskToCommand(commandArgs: [String], client: SocketClient, jsonOutput: Bool) throws {
        let (workspaceRaw, afterWorkspace) = parseOption(commandArgs, name: "--workspace")
        let (targetRaw, afterTarget) = parseOption(afterWorkspace, name: "--target")
        let (messageRaw, afterMessage) = parseOption(afterTarget, name: "--message")
        let (targetPromptRaw, afterTargetPromptDashed) = parseOption(afterMessage, name: "--target-prompt")
        let (targetPromptRawUnderscore, remaining) = parseOption(afterTargetPromptDashed, name: "--target_prompt")

        let positional = remaining.filter { $0 != "--" }
        let target = (targetRaw ?? positional.first ?? "codex")
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        guard ["codex", "claude", "gemini"].contains(target) else {
            throw CLIError(message: "Invalid --target: expected codex, claude, or gemini")
        }

        let message = (messageRaw ?? (positional.count >= 2 ? positional.dropFirst().joined(separator: " ") : nil) ?? "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !message.isEmpty else {
            throw CLIError(message: "Usage: termloop ask-to --target <codex|claude|gemini> --message <text> [--target-prompt <text>] [--workspace <id|ref|index>]")
        }

        let workspaceId = try resolveWorkspaceId(
            workspaceRaw ?? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"],
            client: client
        )
        let result = try client.sendV2(method: "bridge.ask_to", params: [
            "workspace_id": workspaceId,
            "target": target,
            "message": message,
            "target_prompt": targetPromptRaw ?? targetPromptRawUnderscore ?? ""
        ])
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the termloop socket server.
            """
        case "capabilities":
            return """
            Usage: termloop capabilities

            Print server capabilities as JSON.
            """
        case "rpc":
            return """
            Usage: termloop rpc <method> [json-params]

            Call a raw v2 method with an optional JSON object for params.
            Example: termloop rpc surface.report_tty '{"workspace_id":"...","surface_id":"...","tty_name":"ttys001"}'
            """
        case "ask-to", "ask_to":
            return """
            Usage: termloop ask-to --target <codex|claude|gemini> --message <text> [--target-prompt <text>] [--workspace <id|ref|index>]

            Start an Ask-To bridge from the current workspace. This is the CLI
            equivalent of the built-in MCP `ask_to` tool and the sidebar Ask action.

            Examples:
              termloop ask-to --target codex --message "Review this diff"
              termloop ask-to claude "Investigate this failing test"
            """
        case "help":
            return """
            Usage: termloop help
        ...
        case "welcome":
            return """
            Usage: termloop welcome

            Show a welcome screen with the termloop logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: termloop shortcuts

            Open the Settings window to Keyboard Shortcuts.
            """
        case "feedback":
            return """
            Usage: termloop feedback
                   termloop feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "themes":
            return """
            Usage: termloop themes
                   termloop themes list
                   termloop themes set <theme>
                   termloop themes set --light <theme> [--dark <theme>]
                   termloop themes set --dark <theme> [--light <theme>]
                   termloop themes clear

            When run in a TTY, `termloop themes` opens an interactive theme picker with
            live app preview. Use `termloop themes list` for a plain listing.

            The picker previews the selected theme across the running termloop app and
            ...
            Commands:
              ...
              clear                     Remove the termloop theme override and fall back to other config

            Examples:
              termloop themes
              termloop themes list
              termloop themes set "Catppuccin Mocha"
              termloop themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              termloop themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: termloop claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses termloop splits
            ...
            The tmux shim translates supported tmux window/pane commands into TermLoop
            workspace and split operations in the current termloop session.

            Examples:
              termloop claude-teams
              termloop claude-teams --continue
              termloop claude-teams --model sonnet
            """)
        case "omo":
            return String(localized: "cli.omo.usage", defaultValue: """
            Usage: termloop omo [opencode-args...]

            Launch OpenCode with oh-my-openagent in an TermLoop-aware environment.
            ...
            termloop splits with sidebar metadata and notifications.

            This command:
              - sets a tmux-like environment so oh-my-openagent uses termloop splits
              - prepends a private tmux shim to PATH
              - forwards all remaining arguments to opencode

            The tmux shim translates tmux window/pane commands into termloop workspace
            and split operations in the current termloop session.

            Examples:
              termloop omo
              termloop omo --continue
              termloop omo --model claude-sonnet-4-6
            """)
        case "omx":
            return String(localized: "cli.omx.usage", defaultValue: """
            Usage: termloop omx [omx-args...]

            Launch Oh My Codex (OMX) with native termloop pane integration.
            ...
            become native termloop splits.

            This command:
              - sets a tmux-like environment so OMX uses termloop splits
              - prepends a private tmux shim to PATH
        ...
        case "omc":
            ...
            Usage: termloop omc [omc-args...]

            Launch Oh My Claude Code (OMC) with native termloop pane integration.
            ...
            native termloop splits.

            This command:
              - sets a tmux-like environment so OMC uses termloop splits
              - prepends a private tmux shim to PATH
            ...
            Examples:
              termloop omc
              termloop omc team 3:claude "implement feature"
              termloop omc --watch
            """)
        case "identify":
            return """
            Usage: termloop identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]

            Print server identity and caller context details.

            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $TERMLOOP_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $TERMLOOP_SURFACE_ID)
              --no-caller                  Omit caller context from the request
            """
        case "list-windows":
            return """
            Usage: termloop list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: termloop current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: termloop new-window
        ...
        case "focus-window":
            return """
            Usage: termloop focus-window --window <id|ref|index>
            ...
            Example:
              termloop focus-window --window 0
              termloop focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: termloop close-window --window <id|ref|index>
            ...
            Example:
              termloop close-window --window 0
              termloop close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: termloop move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              termloop move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: termloop move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              termloop move-surface --surface surface:1 --workspace workspace:2
              termloop move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: termloop reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              termloop reorder-surface --surface surface:1 --index 0
              termloop reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: termloop reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              termloop reorder-workspace --workspace workspace:2 --index 0
              termloop reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: termloop workspace-action --action <name> [flags]
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$TERMLOOP_WORKSPACE_ID)
            ...
            Example:
              termloop workspace-action --workspace workspace:2 --action pin
              termloop workspace-action --action rename --title "infra"
              termloop workspace-action close-others
              termloop workspace-action --action set-color --color blue
              termloop workspace-action --action set-color --color "#C0392B"
              termloop workspace-action set-color Amber
              termloop workspace-action --action set-description --description "Ship checklist"
              termloop workspace-action --action set-description $'Ship checklist\n- verify build\n- post notes'
              termloop workspace-action clear-color
            """
        case "tab-action":
            return """
            Usage: termloop tab-action --action <name> [flags]
            ...
            Example:
              termloop tab-action --tab tab:3 --action pin
              termloop tab-action --action close-right
              termloop tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: termloop rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>
            ...
            Flags:
              --workspace <id|ref>   Workspace context (default: current/$TERMLOOP_WORKSPACE_ID)
            ...
            Examples:
              termloop rename-tab "build logs"
              termloop rename-tab --tab tab:3 "staging server"
              termloop rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: termloop new-workspace [--name <title>] [--description <text>] [--cwd <path>] [--command <text>]
            ...
            Example:
              termloop new-workspace
              termloop new-workspace --name "Build Server"
              termloop new-workspace --name "Launch" --description "Ship checklist"
              termloop new-workspace --cwd ~/projects/myapp
              termloop new-workspace --cwd . --command "npm test"
            """
        case "list-workspaces":
            return """
            Usage: termloop list-workspaces

            List workspaces in the current window.

            Example:
              termloop list-workspaces
            """
        case "ssh":
            return """
            Usage: termloop ssh <destination> [flags] [-- <remote-command-args>]

            Create a new workspace, mark it as remote-SSH, and start an SSH session in that workspace.
            termloop will also establish a local SSH proxy endpoint so browser traffic can egress from the remote host.
            ...
            Example:
              termloop ssh dev@my-host
              termloop ssh dev@my-host --name "gpu-box" --port 2222 --identity ~/.ssh/id_ed25519
              termloop ssh dev@my-host --ssh-option UserKnownHostsFile=/dev/null --ssh-option StrictHostKeyChecking=no
            """
        case "remote-daemon-status":
            return """
            Usage: termloop remote-daemon-status [--os <darwin|linux>] [--arch <arm64|amd64>]

            Show the embedded termloopd-remote release manifest, local cache status, checksum verification state,
            and the GitHub attestation verification command for a target platform.

            Example:
              termloop remote-daemon-status
              termloop remote-daemon-status --os linux --arch arm64
            """
        case "new-split":
            return """
            Usage: termloop new-split <left|right|up|down> [flags]

            Split the current pane in the given direction.

            Flags:
              --workspace <id|ref>   Target workspace (default: $TERMLOOP_WORKSPACE_ID)
              --surface <id|ref>     Surface to split from (default: $TERMLOOP_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              termloop new-split right
              termloop new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: termloop list-panes [--workspace <id|ref>]

            List panes in a workspace.

            Flags:
              --workspace <id|ref>   Workspace context (default: $TERMLOOP_WORKSPACE_ID)

            Example:
              termloop list-panes
              termloop list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: termloop list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              termloop list-pane-surfaces
              termloop list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: termloop tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `termloop tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              termloop tree
              termloop tree --all
              termloop tree --workspace workspace:2
              termloop --json tree --all
            """
        case "focus-pane":
            return """
            Usage: termloop focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              termloop focus-pane --pane pane:2
              termloop focus-pane pane:1
              termloop focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: termloop new-pane [flags]
            ...
            Example:
              termloop new-pane
              termloop new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: termloop new-surface [flags]
            ...
            Example:
              termloop new-surface
              termloop new-surface --type browser --pane pane:1 --url https://example.com
            """
        case "close-surface":
            return """
            Usage: termloop close-surface [flags]

            Close a surface. Defaults to the focused surface if none specified.

            Flags:
              --surface <id|ref>     Surface to close (default: $TERMLOOP_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $TERMLOOP_WORKSPACE_ID)

            Example:
              termloop close-surface
              termloop close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: termloop drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              termloop drag-surface-to-split --surface surface:1 right
              termloop drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: termloop refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "reload-config":
            return """
            Usage: termloop reload-config

            Run the same configuration reload as the Reload Configuration shortcut.
            This reloads Ghostty config, re-reads ~/.config/termloop/settings.json, and refreshes terminals.

            Example:
              termloop reload-config
            """
        case "surface-health":
            return """
            Usage: termloop surface-health [--workspace <id|ref>]
            ...
            Example:
              termloop surface-health
              termloop surface-health --workspace workspace:2
            """
        case "debug-terminals":
            return """
            Usage: termloop debug-terminals
        ...
        case "trigger-flash":
            return """
            Usage: termloop trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]

            Trigger the unread flash indicator for a surface.

            Flags:
              --workspace <id|ref>   Workspace context (default: $TERMLOOP_WORKSPACE_ID)
              --surface <id|ref>     Target surface (default: $TERMLOOP_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              termloop trigger-flash
              termloop trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: termloop list-panels [--workspace <id|ref>]
            ...
            Example:
              termloop list-panels
              termloop list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: termloop focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              termloop focus-panel --panel surface:2
              termloop focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: termloop close-workspace --workspace <id|ref|index>
            ...
            Example:
              termloop close-workspace --workspace workspace:2
            """
        case "select-workspace":
            return """
            Usage: termloop select-workspace --workspace <id|ref|index>
            ...
            Example:
              termloop select-workspace --workspace workspace:2
              termloop select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: termloop rename-workspace [--workspace <id|ref|index>] [--] <title>

            Rename a workspace. Defaults to the current workspace.
            tmux-compatible alias: rename-window

            Flags:
              --workspace <id|ref|index>   Workspace to rename (default: current/$TERMLOOP_WORKSPACE_ID)

            Example:
              termloop rename-workspace "backend logs"
              termloop rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: termloop current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: termloop capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]

            tmux-compatible alias for reading terminal text from a pane.

            Flags:
              --workspace <id|ref>   Workspace context (default: $TERMLOOP_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $TERMLOOP_SURFACE_ID)
              --scrollback           Include scrollback
              --lines <n>            Return only the last N lines (implies --scrollback)

            Example:
              termloop capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: termloop resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: termloop pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: termloop wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: termloop swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: termloop break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: termloop join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "find-window":
            return """
            Usage: termloop find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: termloop clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: termloop set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: termloop set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: termloop paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: termloop list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: termloop respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: termloop display-message [-p|--print] <text>
        ...
        case "read-screen":
            return """
            Usage: termloop read-screen [flags]
            ...
            Example:
              termloop read-screen
              termloop read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: termloop send [flags] [--] <text>
            ...
            Example:
              termloop send "echo hello"
              termloop send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: termloop send-key [flags] [--] <key>
            ...
            Example:
              termloop send-key enter
              termloop send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: termloop send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              termloop send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: termloop send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              termloop send-key-panel --panel surface:2 enter
              termloop send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: termloop notify [flags]
            ...
            Example:
              termloop notify --title "Build done" --body "All tests passed"
              termloop notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: termloop list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: termloop clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: termloop set-status <key> <value> [flags]
            ...
            Example:
              termloop set-status build "compiling" --icon hammer --color "#ff9500"
              termloop set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: termloop clear-status <key> [flags]
            ...
            Example:
              termloop clear-status build
            """
        case "list-status":
            return """
            Usage: termloop list-status [flags]
            ...
            Example:
              termloop list-status
              termloop list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: termloop set-progress <0.0-1.0> [flags]
            ...
            Example:
              termloop set-progress 0.5 --label "Building..."
              termloop set-progress 1.0 --label "Done"
            """
        case "clear-progress":
            return """
            Usage: termloop clear-progress [flags]
            ...
            Example:
              termloop clear-progress
            """
        case "log":
            return """
            Usage: termloop log [flags] [--] <message>
            ...
            Example:
              termloop log "Build started"
              termloop log --level error --source build "Compilation failed"
              termloop log --level success -- "All 42 tests passed"
            """
        case "clear-log":
            return """
            Usage: termloop clear-log [flags]
        ...
        case "list-log":
            ...
            Example:
              termloop list-log
              termloop list-log --limit 5
            """
        case "sidebar-state":
            return """
            Usage: termloop sidebar-state [flags]
            ...
            Example:
              termloop sidebar-state
              termloop sidebar-state --workspace workspace:2
            """
        case "set-app-focus":
            return """
            Usage: termloop set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              termloop set-app-focus inactive
              termloop set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: termloop simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: termloop claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | termloop claude-hook session-start
              echo '{}' | termloop claude-hook stop
            """
        case "codex":
            return """
            Usage: termloop codex <install-hooks|uninstall-hooks>

            Manage Codex CLI hooks integration.

            Subcommands:
              install-hooks     Install termloop hooks into ~/.codex/hooks.json
              uninstall-hooks   Remove termloop hooks from ~/.codex/hooks.json
            """
        case "codex-hook":
            return """
            Usage: termloop codex-hook <session-start|prompt-submit|stop> [flags]

            Hook for Codex CLI integration. Reads JSON from stdin.
            Gracefully no-ops when not running inside TermLoop.
        ...
        case "termloop-mcp":
            return """
            Usage: termloop termloop-mcp

            Internal stdio MCP server used by TermLoop-managed agent sessions.
            """
        case "opencode-hook":
            return """
            Usage: termloop opencode-hook <session-start|prompt-submit|stop|session-end|permission|error> [flags]

            Hook bridge for OpenCode plugin integration. Reads JSON from stdin.
            Gracefully no-ops when not running inside TermLoop.

            Subcommands:
              session-start   Register an OpenCode session
              prompt-submit   Set Running status on user prompt / busy transition
              stop            Send completion notification, set Completed
              session-end     Clear active OpenCode activity
              permission      Mark the workspace as waiting on permission
              error           Mark the workspace as waiting on an error
        ...
        case "browser":
            return """
            Usage: termloop browser [--surface <id|ref|index> | <surface>] <subcommand> [args]
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $TERMLOOP_WORKSPACE_ID when --workspace is omitted and --window is not set
        ...
        case "markdown":
            ...
            Usage: termloop markdown open <path> [options]
                   termloop markdown <path>       (shorthand for 'open')
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $TERMLOOP_WORKSPACE_ID)
              --surface <id|ref|index>     Source surface to split from (default: focused surface)
              --window <id|ref|index>      Target window
              --direction <left|right|up|down>  Split direction (default: right)

            Examples:
              termloop markdown open plan.md
              termloop markdown ~/project/CHANGELOG.md
              termloop markdown open ./docs/design.md --workspace 0
              termloop markdown open plan.md --direction down
            """
        default:
            return nil
        }
    ...
    private func dispatchSubcommandHelp(command: String, commandArgs: [String]) -> Bool {
        ...
        print("termloop \(command)")
        print("")
        print(text)
        return true
    }

    private static let termloopThemeOverrideBundleIdentifier = "com.termloop.app"
    private static let termloopThemesBlockStart = "# termloop themes start"
    private static let termloopThemesBlockEnd = "# termloop themes end"
    private static let termloopThemesReloadNotificationName = "com.termloop.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        if let light = selection.light {
            environment["TERMLOOP_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["TERMLOOP_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func runThemes(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'termloop themes --help'.")
            }
    ...
    private func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'termloop themes' to list available themes.")
    }
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.termloopThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.termloopThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func cmuxThemeOverrideConfigURL() throws -> URL {
        ...
        return appSupport
            .appendingPathComponent(Self.termloopThemeOverrideBundleIdentifier, isDirectory: true)
    ...
    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.termloopThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.termloopThemesBlockEnd)
        """
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["TERMLOOP_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["TERMLOOP_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["TERMLOOP_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"])
    }
    ...
    private func tmuxResolveSurfaceTarget(
        ...
        if tmuxPaneSelector(from: raw) != nil {
            ...
            // exact surface (TERMLOOP_SURFACE_ID) over the pane's currently selected
    ...
    private func isCmuxClaudeWrapper(at path: String) -> Bool {
        ...
        return prefix.contains("termloop claude wrapper - injects hooks and session tracking")
    }
    ...
    private func configureTmuxCompatEnvironment(
        ...
        setenv("TERMLOOP_SOCKET_PATH", socketPath, 1)
        setenv("TERMLOOP_SOCKET", socketPath, 1)
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            setenv("TERMLOOP_SOCKET_PASSWORD", explicitPassword, 1)
        }
        ...
        if let focusedContext {
            setenv("TERMLOOP_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("TERMLOOP_SURFACE_ID", surfaceId, 1)
            }
        }
    }
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        guard let restoreModuleURL = try? createClaudeNodeOptionsRestoreModule() else {
            unsetenv("TERMLOOP_ORIGINAL_NODE_OPTIONS_PRESENT")
            unsetenv("TERMLOOP_ORIGINAL_NODE_OPTIONS")
            return
        }
        if let existing = processEnvironment["NODE_OPTIONS"] {
            setenv("TERMLOOP_ORIGINAL_NODE_OPTIONS_PRESENT", "1", 1)
            setenv("TERMLOOP_ORIGINAL_NODE_OPTIONS", existing, 1)
        } else {
            setenv("TERMLOOP_ORIGINAL_NODE_OPTIONS_PRESENT", "0", 1)
            unsetenv("TERMLOOP_ORIGINAL_NODE_OPTIONS")
        }
        setenv(
            "NODE_OPTIONS",
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        #!/usr/bin/env bash
        set -euo pipefail
        exec "${TERMLOOP_CLAUDE_TEAMS_TERMLOOP_BIN:-termloop}" __tmux-compat "$@"
        """
    ...
    private func createClaudeNodeOptionsRestoreModule() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("termloop-claude-node-options", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true, attributes: nil)
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["TERMLOOP_SOCKET_PATH"] = socketPath
        launcherEnvironment["TERMLOOP_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            launcherEnvironment["TERMLOOP_SOCKET_PASSWORD"] = explicitPassword
        }
        let shimDirectory = try createClaudeTeamsShimDirectory()
        let executablePath = resolvedExecutableURL()?.path ?? (args.first ?? "termloop")
        ...
        let claudeExecutablePath: String? = {
            ...
            let candidates = [
                launcherEnvironment["TERMLOOP_CUSTOM_CLAUDE_PATH"],
    ...
    // MARK: - termloop omo (OpenCode + oh-my-openagent)
    ...
    private func createOMOShimDirectory() throws -> URL {
        // tmux shim: redirects tmux commands to termloop __tmux-compat
        ...
        exec "${TERMLOOP_OMO_TERMLOOP_BIN:-termloop}" __tmux-compat "$@"
        """
        ...
        // terminal-notifier shim: intercepts macOS notifications and routes to termloop notify
        let notifierURL = root.appendingPathComponent("terminal-notifier", isDirectory: false)
        let notifierScript = """
        #!/usr/bin/env bash
        # Intercept terminal-notifier calls and route through termloop notify.
        ...
        exec "${TERMLOOP_OMO_TERMLOOP_BIN:-termloop}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
        """
    ...
    private func createOMXShimDirectory() throws -> URL {
        ...
        exec "${TERMLOOP_OMX_TERMLOOP_BIN:-termloop}" __tmux-compat "$@"
        """
        return try createTmuxCompatShimDirectory(
    ...
    // MARK: - termloop omc (Oh My Claude Code)
    ...
    private func createOMCShimDirectory() throws -> URL {
        ...
        exec "${TERMLOOP_OMC_TERMLOOP_BIN:-termloop}" __tmux-compat "$@"
        """
        return try createTmuxCompatShimDirectory(
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in termloop claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in termloop claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/termloop-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"] : nil)
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in termloop CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in termloop CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "TermLoop", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["TERMLOOP_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["TERMLOOP_SURFACE_ID"] : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["TERMLOOP_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
            ...
            // MARK: termloop-hook
            if let sessionId = parsedInput.sessionId {
                TermLoopCLICommands.reportClaudeSessionStart(
                    workspaceId: workspaceId,
                    sessionId: sessionId,
                    cwd: parsedInput.cwd,
                    pid: claudePid,
                ...
                TermLoopCLICommands.reportAgentActivity(
                    workspaceId: workspaceId,
                    agentId: "claude",
                    phase: "ready",
                    sessionId: sessionId,
                ...
                telemetry.breadcrumb("claude-hook.session-start.report")
            }
            // MARK: /termloop-hook
            // Register PID for stale-session detection and OSC suppression.
            // Sidebar shows a gray "Ready" pill via the .ready phase reported
            // above; it transitions to "Running" once the user submits a
            ...
            do {
                ...
                if let completion {
                    ...
                    TermLoopCLICommands.reportAgentActivity(
                        ...
                        messagePreview: completion.body,
        ...
        case "notification", "notify":
            ...
            TermLoopCLICommands.reportAgentActivity(
                ...
                agentId: "claude",
                phase: "waiting",
                attentionKind: attentionKind(forNotificationSubtitle: summary.subtitle),
                messagePreview: summary.body,
                sessionId: parsedInput.sessionId,
                cwd: parsedInput.cwd,
                pid: mappedSession?.pid,
                client: client
        ...
        case "session-end":
            ...
            if let consumedSession {
                ...
                _ = try? sendV1Command("clear_notifications --tab=\(workspaceId)", client: client)
                // MARK: termloop-hook
                TermLoopCLICommands.clearClaudeSession(workspaceId: workspaceId, client: client)
                TermLoopCLICommands.clearAgentActivity(workspaceId: workspaceId, client: client)
                // MARK: /termloop-hook
            }
            print("OK")
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                termloop claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func resolveWorkspaceIdAllowingFallback(
        ...
        // Env-var fallback: every termloop PTY exports TERMLOOP_WORKSPACE_ID. Codex's
        // hook subprocess sometimes doesn't have a controlling TTY that
        // matches debug.terminals, so TTY lookup fails — use the env var
        // before falling back to "currently selected workspace".
        if let envWorkspaceId = ProcessInfo.processInfo
            .environment["TERMLOOP_WORKSPACE_ID"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func resolveCallerTTYName() -> String? {
        let env = ProcessInfo.processInfo.environment
        for key in ["TERMLOOP_CLI_TTY_NAME", "TERMLOOP_TTY_NAME", "TTY", "SSH_TTY"] {
    ...
    private func extractClaudeHookSessionId(from object: [String: Any]) -> String? {
        let rootKeys = ["session_id", "sessionId", "sessionID", "conversation_id", "conversationId", "id"]
        if let id = firstString(in: object, keys: rootKeys) {
            return id
        }

        for key in ["notification", "data", "context", "payload", "event"] {
            if let nested = object[key] as? [String: Any],
               let id = firstString(in: nested, keys: rootKeys) {
                return id
            }
        }
        if let session = object["session"] as? [String: Any],
           let id = firstString(
               in: session,
               keys: ["id", "session_id", "sessionId", "sessionID", "conversation_id", "conversationId"]
           ) {
            return id
        }
        ...
        if let path = object["path"] as? [String: Any],
           let id = firstString(in: path, keys: rootKeys) {
            return id
        }
        return nil
    }
    ...
    private func extractClaudeHookCWD(from object: [String: Any]) -> String? {
        let cwdKeys = ["cwd", "working_directory", "workingDirectory", "project_dir", "projectDir", "directory"]
        if let cwd = firstString(in: object, keys: cwdKeys) {
            return cwd
        }
        for key in ["notification", "data", "session", "context", "payload", "event"] {
            if let nested = object[key] as? [String: Any],
        ...
        if let path = object["path"] as? [String: Any],
           let cwd = firstString(in: path, keys: ["cwd", "directory"]) {
            return cwd
        }
        return nil
    }
    ...
    private func summarizeClaudeHookStop(
        ...
        if let lastMsg = transcript?.lastAssistantMessage {
            ...
            return (
                subtitle,
                truncate(lastMsg, maxLength: 200)
            )
        }
    ...
    private func attentionKind(forNotificationSubtitle subtitle: String) -> String {
    ...
    /// The hooks.json content that termloop installs into ~/.codex/.
    /// Each hook calls `termloop codex-hook <event>` which gracefully no-ops
    /// when not running inside TermLoop. The command checks for termloop on PATH
    /// first so it silently succeeds even when termloop is not installed
    /// (e.g. user opened codex in a non-TermLoop terminal).
    ...
    private enum AgentHookAction {
        case sessionStart, promptSubmit, permissionRequest, shellExec, stop, sessionEnd, noop
    }
    ...
    private static let openCodeHookRuntimeDef = AgentHookDef(
        name: "opencode",
    ...
    private func hookCommand(for def: AgentHookDef, event: AgentHookDef.HookEvent) -> String {
        // Accept either TERMLOOP_SURFACE_ID or TERMLOOP_WORKSPACE_ID as a "we're inside
        // TermLoop" indicator. Codex's TUI strips TERMLOOP_SURFACE_ID from hook
        // subprocess env in some versions, which made the surface-only guard
        // suppress every hook silently. Both vars are set on every termloop PTY.
        //
    ...
    private func runGenericAgentHook(def: AgentHookDef, commandArgs: [String], client: SocketClient, telemetry: CLISocketSentryTelemetry) throws {
        ...
        let hookWsFlag = optionValue(hookArgs, name: "--workspace")
        let workspaceArg = hookWsFlag ?? env["TERMLOOP_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? env["TERMLOOP_SURFACE_ID"] : nil)

        let rawInput = String(data: FileHandle.standardInput.readDataToEndOfFile(), encoding: .utf8) ?? ""
        let input = parseClaudeHookInput(rawInput: rawInput)
        agentLifecycleCLILog(
            "hook.enter agent=\(def.name) subcommand=\(subcommand) workspaceArg=\(workspaceArg ?? "nil") surfaceArg=\(surfaceArg ?? "nil") session=\(input.sessionId ?? "nil") cwd=\(input.cwd ?? "nil") rawBytes=\(rawInput.count)"
        )

        var sessionStoreEnv = env
        if sessionStoreEnv["TERMLOOP_CLAUDE_HOOK_STATE_PATH"] == nil {
            sessionStoreEnv["TERMLOOP_CLAUDE_HOOK_STATE_PATH"] =
                "~/.termloop/\(def.sessionStoreSuffix)-hook-sessions.json"
        }
        let store = ClaudeHookSessionStore(
            processEnv: sessionStoreEnv
        )
        ...
        switch action {
        case .sessionStart:
            let sessionId = normalizedHandleValue(input.sessionId)
            ...
            agentLifecycleCLILog(
                "hook.sessionStart.resolved agent=\(def.name) workspace=\(workspaceId) surface=\(surfaceId) session=\(sessionId ?? "nil") pid=\(pid.map(String.init) ?? "nil") cwd=\(input.cwd ?? "nil")"
            )
            if let sessionId {
                try? store.upsert(sessionId: sessionId, workspaceId: workspaceId, surfaceId: surfaceId, cwd: input.cwd, pid: pid)
            }
            agentLifecycleCLILog(
                "hook.sessionStart.report agent=\(def.name) workspace=\(workspaceId) phase=ready session=\(sessionId ?? "nil") pid=\(pid.map(String.init) ?? "nil")"
            )
            TermLoopCLICommands.reportAgentActivity(
                workspaceId: workspaceId,
                agentId: def.name,
            ...
            if let pid, let sessionId {
                let pidKey = "\(def.statusKey).\(sessionId)"
                _ = try? sendV1Command("set_agent_pid \(pidKey) \(pid) --tab=\(workspaceId)", client: client)
            }

        case .promptSubmit:
            let mapped = try? store.resolve(sessionId: input.sessionId, workspaceId: workspaceArg, surfaceId: surfaceArg)
            let sessionId = normalizedHandleValue(input.sessionId) ?? mapped?.sessionId
            ...
            agentLifecycleCLILog(
                "hook.promptSubmit.resolved agent=\(def.name) workspace=\(workspaceId) mappedWorkspace=\(mapped?.workspaceId ?? "nil") session=\(sessionId ?? "nil") mappedSession=\(mapped?.sessionId ?? "nil") pid=\(pid.map(String.init) ?? "nil") cwd=\((input.cwd ?? mapped?.cwd) ?? "nil")"
            )
            if let sessionId {
            ...
            _ = try? sendV1Command("clear_notifications --tab=\(workspaceId)", client: client)
            agentLifecycleCLILog(
                "hook.promptSubmit.report agent=\(def.name) workspace=\(workspaceId) phase=running session=\(sessionId ?? "nil") pid=\(pid.map(String.init) ?? "nil")"
            )
            TermLoopCLICommands.reportAgentActivity(
                workspaceId: workspaceId,
                agentId: def.name,
        ...
        case .permissionRequest:
            ...
            let sessionId = normalizedHandleValue(input.sessionId) ?? mapped?.sessionId
            let workspaceId = try resolvePreferredWorkspaceIdForClaudeHook(
                preferred: mapped?.workspaceId, fallback: workspaceArg, client: client
            )
            let pid = mapped?.pid ?? inferredCodexAgentPID()

            // Clear optional ready marker, mirroring permission-request.sh.
            if let marker = env["TERMLOOP_CODEX_READY_MARKER"], !marker.isEmpty {
                try? FileManager.default.removeItem(atPath: marker)
            }

            // Extract a short preview from the payload the same way the bash
            // script did (reason → description → summary → request).
            let preview: String = {
                guard let obj = input.object else { return "Approval requested" }
            ...
            agentLifecycleCLILog(
                "hook.permissionRequest.report agent=\(def.name) workspace=\(workspaceId) phase=waiting preview=\(preview.prefix(40))"
            )
            TermLoopCLICommands.reportAgentActivity(
                workspaceId: workspaceId,
                agentId: def.name,
        ...
        case .shellExec:
            ...
            agentLifecycleCLILog(
                "hook.shellExec.report agent=\(def.name) workspace=\(workspaceId) phase=running session=\(sessionId ?? "nil") pid=\(pid.map(String.init) ?? "nil") cwd=\(cwd ?? "nil")"
            )
            TermLoopCLICommands.reportAgentActivity(
                workspaceId: workspaceId,
                agentId: def.name,
        ...
        case .stop:
            do {
                ...
                agentLifecycleCLILog(
                    "hook.stop.resolved agent=\(def.name) workspace=\(workspaceId) surface=\(surfaceId) session=\(sessionId ?? "nil") pid=\(pid.map(String.init) ?? "nil") cwd=\(cwd ?? "nil") hasLastMsg=\(lastMsg == nil ? 0 : 1)"
                )
                ...
                _ = try? sendV1Command("notify_target \(workspaceId) \(surfaceId) \(payload)", client: client)
                agentLifecycleCLILog(
                    "hook.stop.report agent=\(def.name) workspace=\(workspaceId) phase=waiting attention=completion session=\(sessionId ?? "nil") pid=\(pid.map(String.init) ?? "nil") body=\(body)"
                )
                TermLoopCLICommands.reportAgentActivity(
                    workspaceId: workspaceId,
                    agentId: def.name,
        ...
        case .sessionEnd:
            if let mapped = try? store.consume(sessionId: input.sessionId, workspaceId: workspaceArg, surfaceId: surfaceArg) {
                let pidKey = "\(def.statusKey).\(mapped.sessionId)"
                _ = try? sendV1Command("clear_agent_pid \(pidKey) --tab=\(mapped.workspaceId)", client: client)
                let shouldPreserveCompletion = mapped.lastSubtitle?
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                    .lowercased()
                    .hasPrefix("completed") == true
                agentLifecycleCLILog(
                    "hook.sessionEnd.consume agent=\(def.name) workspace=\(mapped.workspaceId) session=\(mapped.sessionId) preserveCompletion=\(shouldPreserveCompletion ? 1 : 0)"
                )
                if !shouldPreserveCompletion {
                    agentLifecycleCLILog(
                        "hook.sessionEnd.clear agent=\(def.name) workspace=\(mapped.workspaceId) session=\(mapped.sessionId)"
                    )
                    TermLoopCLICommands.clearAgentActivity(workspaceId: mapped.workspaceId, client: client)
                }
            } else {
                agentLifecycleCLILog(
                    "hook.sessionEnd.consumeMiss agent=\(def.name) workspaceArg=\(workspaceArg ?? "nil") surfaceArg=\(surfaceArg ?? "nil") session=\(input.sessionId ?? "nil")"
                )
            }
    ...
    private func runOpenCodeHook(commandArgs: [String], client: SocketClient, telemetry: CLISocketSentryTelemetry) throws {
        let subcommand = commandArgs.first?.lowercased() ?? ""
        if ["session-start", "prompt-submit", "stop", "session-end"].contains(subcommand) {
            try runGenericAgentHook(
                def: Self.openCodeHookRuntimeDef,
                commandArgs: commandArgs,
                client: client,
                telemetry: telemetry
            )
        ...
        if sessionStoreEnv["TERMLOOP_CLAUDE_HOOK_STATE_PATH"] == nil {
            sessionStoreEnv["TERMLOOP_CLAUDE_HOOK_STATE_PATH"] =
                "~/.termloop/\(Self.openCodeHookRuntimeDef.sessionStoreSuffix)-hook-sessions.json"
        }
        let sessionStore = ClaudeHookSessionStore(
            processEnv: sessionStoreEnv
        )
        let mapped = try? sessionStore.resolve(sessionId: input.sessionId, workspaceId: workspaceArg, surfaceId: env["TERMLOOP_SURFACE_ID"])
        let sessionId = normalizedHandleValue(input.sessionId) ?? mapped?.sessionId
        let workspaceId = try resolvePreferredWorkspaceIdForClaudeHook(
            preferred: mapped?.workspaceId,
            fallback: workspaceArg,
        ...
        let cwd = input.cwd ?? mapped?.cwd
        let pid = mapped?.pid ?? inferredCodexAgentPID()
        let preview = sanitizeNotificationField(
            truncate(
                normalizedSingleLine(
                    (input.object?["message_preview"] as? String)
                        ?? (input.object?["message"] as? String)
        ...
        case "permission":
            TermLoopCLICommands.reportAgentActivity(
                workspaceId: workspaceId,
                agentId: Self.openCodeHookRuntimeDef.name,
                phase: "waiting",
                attentionKind: "permission",
                messagePreview: preview,
                sessionId: sessionId,
    ...
    private func runSetupHooks(uninstall: Bool = false) throws {
        ...
        print("termloop \(isUninstall ? "uninstall" : "setup")-hooks: \(verb) agent hooks")
        if !isUninstall {
    ...
    private func versionSummary() -> String {
        let info = resolvedVersionInfo()
        let commit = info["TERMLOOPCommit"].flatMap { normalizedCommitHash($0) }
        let baseSummary: String
        if let version = info["CFBundleShortVersionString"], let build = info["CFBundleVersion"] {
            baseSummary = "termloop \(version) (\(build))"
        } else if let version = info["CFBundleShortVersionString"] {
            baseSummary = "termloop \(version)"
        } else if let build = info["CFBundleVersion"] {
            baseSummary = "termloop build \(build)"
        } else {
            baseSummary = "termloop version unknown"
        }
    ...
    private func printWelcome() {
        ...
        print(shortcuts)
        print()
        print("  \(bold)Docs\(reset)\(subdued)                https://termloop.ai/docs\(reset)")
        print("  \(bold)Discord\(reset)\(subdued)             https://github.com/feritzcan2/termloop/discussions\(reset)")
        print("  \(bold)GitHub\(reset)\(subdued)              https://github.com/feritzcan2/termloop (please leave a star ⭐)\(reset)")
        print("  \(bold)Email\(reset)\(subdued)               feritzcan93@gmail.com\(reset)")
        print()
        print("  \(subdued)Run \(reset)\(bold)termloop --help\(reset)\(subdued) for all commands.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)termloop shortcuts\(reset)\(subdued) to edit shortcuts.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)termloop feedback\(reset)\(subdued) to report a bug.\(reset)")
        print()
    }
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["TERMLOOPCommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["TERMLOOP_UPSTREAM_COMMIT"]) {
            info["TERMLOOPCommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["TERMLOOPCommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["TERMLOOPCommit"] = normalizedCommit
        }
    ...
    private func usage() -> String {
        return """
        termloop - control termloop via Unix socket

        Usage:
          termloop <path>                Open a directory in a new workspace (launches termloop if needed)
          termloop [global-options] <command> [options]
        ...
        Socket Auth:
          --password takes precedence, then TERMLOOP_SOCKET_PASSWORD env var, then password saved in Settings.
        ...
        Commands:
          ...
          ask-to --target <codex|claude|gemini> --message <text> [--workspace <id|ref|index>]

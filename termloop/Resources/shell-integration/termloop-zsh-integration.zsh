...
typeset -g _TERMLOOP_HAS_ZSOCKET=0
...
_cmux_send() {
    ...
    if (( _TERMLOOP_HAS_ZSOCKET )); then
        local fd
        zsocket "$TERMLOOP_SOCKET_PATH" 2>/dev/null || return 1
        fd=$REPLY
    ...
    if command -v ncat >/dev/null 2>&1; then
        print -r -- "$payload" | ncat -w 1 -U "$TERMLOOP_SOCKET_PATH" --send-only
    elif command -v socat >/dev/null 2>&1; then
        print -r -- "$payload" | socat -T 1 - "UNIX-CONNECT:$TERMLOOP_SOCKET_PATH" >/dev/null 2>&1
    elif command -v nc >/dev/null 2>&1; then
        if print -r -- "$payload" | nc -N -U "$TERMLOOP_SOCKET_PATH" >/dev/null 2>&1; then
            :
        else
            print -r -- "$payload" | nc -w 1 -U "$TERMLOOP_SOCKET_PATH" >/dev/null 2>&1 || true
        fi
    fi
}
...
typeset -g _TERMLOOP_GIT_LAST_PWD=""
typeset -g _TERMLOOP_GIT_LAST_RUN=0
typeset -g _TERMLOOP_GIT_JOB_PID=""
typeset -g _TERMLOOP_GIT_JOB_STARTED_AT=0
typeset -g _TERMLOOP_GIT_FORCE=0
typeset -g _TERMLOOP_GIT_HEAD_LAST_PWD=""
typeset -g _TERMLOOP_GIT_HEAD_PATH=""
typeset -g _TERMLOOP_GIT_HEAD_SIGNATURE=""
typeset -g _TERMLOOP_GIT_HEAD_WATCH_PID=""
typeset -g _TERMLOOP_PR_POLL_PID=""
typeset -g _TERMLOOP_PR_POLL_PWD=""
typeset -g _TERMLOOP_PR_LAST_BRANCH=""
typeset -g _TERMLOOP_PR_NO_PR_BRANCH=""
typeset -g _TERMLOOP_PR_POLL_INTERVAL=45
typeset -g _TERMLOOP_PR_FORCE=0
typeset -g _TERMLOOP_PR_DEBUG=${_TERMLOOP_PR_DEBUG:-0}
typeset -g _TERMLOOP_ASYNC_JOB_TIMEOUT=20
typeset -g _TERMLOOP_LAST_PR_ACTION=""
typeset -g _TERMLOOP_LAST_PR_TARGET=""

typeset -g _TERMLOOP_PORTS_LAST_RUN=0
typeset -g _TERMLOOP_CMD_START=0
typeset -g _TERMLOOP_SHELL_ACTIVITY_LAST=""
typeset -g _TERMLOOP_TTY_NAME=""
typeset -g _TERMLOOP_TTY_REPORTED=0
typeset -g _TERMLOOP_GHOSTTY_SEMANTIC_PATCHED=0
typeset -g _TERMLOOP_WINCH_GUARD_INSTALLED=0
typeset -g _TERMLOOP_TMUX_PUSH_SIGNATURE=""
typeset -g _TERMLOOP_TMUX_PULL_SIGNATURE=""
typeset -g _TERMLOOP_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT=${_TERMLOOP_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT:-0}
typeset -ga _TERMLOOP_TMUX_SYNC_KEYS=(
...
typeset -ga _TERMLOOP_TMUX_SURFACE_SCOPED_KEYS=(
    TERMLOOP_PANEL_ID
    TERMLOOP_SURFACE_ID
)
...
_cmux_precmd() {
    ...
    if [[ "$pwd" != "$_TERMLOOP_GIT_LAST_PWD" ]]; then
        should_git=1
    elif (( _TERMLOOP_GIT_FORCE )); then
        should_git=1
    elif (( now - _TERMLOOP_GIT_LAST_RUN >= 3 )); then
        should_git=1
    fi
    ...
    if (( should_git )); then
        ...
        if [[ -n "$_TERMLOOP_GIT_JOB_PID" ]] && kill -0 "$_TERMLOOP_GIT_JOB_PID" 2>/dev/null; then
            ...
            if [[ "$pwd" != "$_TERMLOOP_GIT_LAST_PWD" ]] || (( _TERMLOOP_GIT_FORCE )); then
...
_cmux_bundled_bin_dir() {
    if [[ -n "${GHOSTTY_BIN_DIR:-}" ]]; then
        ...
        [[ -d "$bin_dir" ]] && print -r -- "$bin_dir" && return 0
    fi
...
#
# Belt-and-suspenders: also install zsh functions for `claude` / `codex` /
# other agent CLIs so manual invocations route through the TermLoop wrappers
...
_cmux_fix_path() {
    ...
    if [[ -n "$bin_dir" ]]; then
        ...
        parts=("${(@)parts:#$bin_dir}")
        [[ -n "$gui_dir" ]] && parts=("${(@)parts:#$gui_dir}")
        PATH="${bin_dir}:${(j/:/)parts}"
        builtin hash -r 2>/dev/null

        # Define wrapper functions that always invoke the bundled wrapper
        # script. This makes `claude` / `codex` go through termloop wrappers
        # regardless of where the user's PATH later resolves the binary.
        local agent
        for agent in claude codex; do
            if [[ -x "$bin_dir/$agent" ]]; then
                builtin eval "$agent() { command \"$bin_dir/$agent\" \"\$@\"; }"
            fi
        done
    fi

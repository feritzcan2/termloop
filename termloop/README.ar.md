...
<p align="center">
  <a href="https://github.com/feritzcan2/termloop/releases/latest/download/termloop-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="تحميل TermLoop لنظام macOS" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="لقطة شاشة TermLoop" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ فيديو توضيحي</a> · <a href="https://termloop.ai/blog/zen-of-cmux">فلسفة TermLoop</a>
</p>
...
- **أوامر مخصصة** — حدد إجراءات خاصة بالمشروع في [`cmux.json`](https://termloop.ai/docs/custom-commands) يتم تشغيلها من لوحة الأوامر
...
افتح ملف `.dmg` واسحب TermLoop إلى مجلد التطبيقات. يتم تحديث TermLoop تلقائيًا عبر Sparkle، لذا تحتاج للتحميل مرة واحدة فقط.
...
جربت بعض منظمات البرمجة لكن معظمها كانت تطبيقات Electron/Tauri وأداؤها كان يزعجني. كما أنني أفضل الطرفية لأن منظمات GUI تحبسك في سير عملها. لذا بنيت TermLoop كتطبيق macOS أصلي بـ Swift/AppKit. يستخدم libghostty لعرض الطرفية ويقرأ إعدادات Ghostty الحالية للسمات والخطوط والألوان.
...
TermLoop لا يفرض على المطورين طريقة استخدام أدواتهم. إنه طرفية ومتصفح مع واجهة سطر أوامر، والباقي متروك لك.

TermLoop هو لبنة أساسية وليس حلًا جاهزًا. يمنحك طرفية ومتصفحًا وإشعارات ومساحات عمل وأقسامًا وعلامات تبويب وواجهة سطر أوامر للتحكم في كل ذلك. TermLoop لا يجبرك على طريقة محددة لاستخدام وكلاء البرمجة. ما تبنيه باستخدام هذه اللبنات الأساسية هو ملكك.
...
لمزيد من المعلومات حول كيفية إعداد TermLoop، [توجه إلى وثائقنا](https://termloop.ai/docs/getting-started?utm_source=readme).
...
[تحميل TermLoop NIGHTLY](https://github.com/feritzcan2/termloop/releases/download/nightly/termloop-nightly-macos.dmg)

TermLoop NIGHTLY هو تطبيق منفصل بمعرّف حزمة خاص به، لذا يعمل بجانب الإصدار المستقر. يُبنى تلقائيًا من أحدث commit على فرع `main` ويتم تحديثه تلقائيًا عبر Sparkle الخاص به.

## استعادة الجلسة (السلوك الحالي)

عند إعادة التشغيل، يستعيد TermLoop حاليًا تخطيط التطبيق والبيانات الوصفية فقط:
- تخطيط النوافذ/مساحات العمل/الأجزاء
- مجلدات العمل
- سجل تمرير الطرفية (أفضل جهد)
- عنوان URL للمتصفح وسجل التنقل

TermLoop **لا** يستعيد حالة العمليات الحية داخل تطبيقات الطرفية. على سبيل المثال، جلسات Claude Code/tmux/vim النشطة لا يتم استئنافها بعد إعادة التشغيل بعد.
...
- انضم إلى المحادثة على [Discord](https://github.com/feritzcan2/termloop/discussions)
- أنشئ وشارك في [قضايا GitHub](https://github.com/feritzcan2/termloop/issues) و[المناقشات](https://github.com/feritzcan2/termloop/discussions)
- أخبرنا بما تبنيه باستخدام TermLoop
...
TermLoop مجاني ومفتوح المصدر وسيظل كذلك دائمًا. إذا كنت ترغب في دعم التطوير والحصول على وصول مبكر لما هو قادم:

**[احصل على إصدار المؤسسين](https://buy.stripe.com/3cI00j2Ld0it5OU33r5EY0q)**

- **أولوية لطلبات الميزات/إصلاح الأخطاء**
- **وصول مبكر: ذكاء اصطناعي لـ TermLoop يمنحك سياقًا عن كل مساحة عمل وعلامة تبويب ولوحة**
...
TermLoop مفتوح المصدر بموجب [GPL-3.0-or-later](LICENSE).

إذا لم تستطع مؤسستك الامتثال لـ GPL، فهناك ترخيص تجاري متاح. تواصل مع [feritzcan93@gmail.com](mailto:feritzcan93@gmail.com) للتفاصيل.

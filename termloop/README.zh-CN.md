...
<p align="center">
  <a href="https://github.com/feritzcan2/termloop/releases/latest/download/termloop-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="下载 TermLoop macOS 版" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="TermLoop 截图" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ 演示视频</a> · <a href="https://termloop.ai/blog/zen-of-cmux">The Zen of TermLoop</a>
</p>
...
- **自定义命令** — 在 [`cmux.json`](https://termloop.ai/docs/custom-commands) 中定义项目专属操作，通过命令面板启动
...
打开 `.dmg` 文件并将 TermLoop 拖动到"应用程序"文件夹。TermLoop 通过 Sparkle 自动更新，您只需下载一次。
...
我试过几个编程协调工具，但大多数都是 Electron/Tauri 应用，性能让我不满意。我也更喜欢终端，因为 GUI 协调工具会把你锁定在它们的工作流里。所以我用 Swift/AppKit 构建了 TermLoop，作为一个原生 macOS 应用。它使用 libghostty 进行终端渲染，并读取您现有的 Ghostty 配置中的主题、字体和颜色设置。
...
TermLoop 不规定开发者应该如何使用工具。它是一个带有 CLI 的终端和浏览器，其余的由你决定。

TermLoop 是原语，而非解决方案。它提供终端、浏览器、通知、工作区、分割、标签页，以及控制这一切的 CLI。TermLoop 不强迫你以特定方式使用编程代理。你用这些原语构建什么，完全取决于你自己。
...
有关 TermLoop 配置的更多信息，请[查看我们的文档](https://termloop.ai/docs/getting-started?utm_source=readme)。
...
[下载 TermLoop NIGHTLY](https://github.com/feritzcan2/termloop/releases/download/nightly/termloop-nightly-macos.dmg)

TermLoop NIGHTLY 是一个拥有独立 Bundle ID 的单独应用，因此可以与稳定版并行运行。它从最新的 `main` 提交自动构建，并通过独立的 Sparkle 更新源自动更新。

## 会话恢复（当前行为）

重新启动时，TermLoop 目前仅恢复应用布局和元数据：
- 窗口/工作区/窗格布局
- 工作目录
- 终端回滚缓冲区（尽力恢复）
- 浏览器 URL 和导航历史

TermLoop **不会**恢复终端应用内部的实时进程状态。例如，活动的 Claude Code/tmux/vim 会话在重启后尚无法恢复。
...
- 加入 [Discord](https://github.com/feritzcan2/termloop/discussions) 讨论
- 创建和参与 [GitHub Issues](https://github.com/feritzcan2/termloop/issues) 和[讨论](https://github.com/feritzcan2/termloop/discussions)
- 告诉我们您在用 TermLoop 构建什么
...
TermLoop 免费、开源，并将一直如此。如果您想支持开发并提前体验即将推出的功能：

**[获取 Founder's Edition](https://buy.stripe.com/3cI00j2Ld0it5OU33r5EY0q)**

- **功能请求/Bug 修复优先处理**
- **抢先体验：为每个工作区、标签页和面板提供上下文的 TermLoop AI**
...
TermLoop 以 [GPL-3.0-or-later](LICENSE) 开源。

如果您的组织无法遵守 GPL，可提供商业许可证。详情请联系 [feritzcan93@gmail.com](mailto:feritzcan93@gmail.com)。

...
<p align="center">
  <a href="https://github.com/feritzcan2/termloop/releases/latest/download/termloop-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="Télécharger TermLoop pour macOS" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="Capture d'écran de TermLoop" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ Vidéo de démonstration</a> · <a href="https://termloop.ai/blog/zen-of-cmux">The Zen of TermLoop</a>
</p>
...
- **Commandes personnalisées** — Définissez des actions spécifiques au projet dans [`cmux.json`](https://termloop.ai/docs/custom-commands) qui se lancent depuis la palette de commandes
...
Ouvrez le `.dmg` et glissez TermLoop dans votre dossier Applications. TermLoop se met à jour automatiquement via Sparkle, vous n'avez donc besoin de le télécharger qu'une seule fois.
...
TermLoop ne prescrit pas comment les développeurs utilisent leurs outils. C'est un terminal et un navigateur avec un CLI, le reste vous appartient.

TermLoop est une primitive, pas une solution. Il vous donne un terminal, un navigateur, des notifications, des espaces de travail, des divisions, des onglets et un CLI pour tout contrôler. TermLoop ne vous impose pas une façon préconçue d'utiliser les agents de programmation. Ce que vous construisez avec ces primitives vous appartient.
...
Pour plus d'informations sur la configuration de TermLoop, [consultez notre documentation](https://termloop.ai/docs/getting-started?utm_source=readme).
...
[Télécharger TermLoop NIGHTLY](https://github.com/feritzcan2/termloop/releases/download/nightly/termloop-nightly-macos.dmg)

TermLoop NIGHTLY est une application séparée avec son propre identifiant de bundle, elle fonctionne donc en parallèle de la version stable. Construite automatiquement à partir du dernier commit `main` et mise à jour automatiquement via son propre flux Sparkle.

## Restauration de session (comportement actuel)

Au relancement, TermLoop restaure actuellement uniquement la disposition et les métadonnées de l'application :
...
TermLoop ne restaure **pas** l'état des processus actifs dans les applications du terminal. Par exemple, les sessions actives de Claude Code/tmux/vim ne sont pas encore reprises après un redémarrage.
...
- Rejoignez la conversation sur [Discord](https://github.com/feritzcan2/termloop/discussions)
- Créez et participez aux [issues GitHub](https://github.com/feritzcan2/termloop/issues) et aux [discussions](https://github.com/feritzcan2/termloop/discussions)
- Dites-nous ce que vous construisez avec TermLoop
...
TermLoop est gratuit, open source, et le restera toujours. Si vous souhaitez soutenir le développement et obtenir un accès anticipé à ce qui arrive :
...
- **Accès anticipé : TermLoop AI qui vous donne du contexte sur chaque espace de travail, onglet et panneau**
...
TermLoop est open source sous [GPL-3.0-or-later](LICENSE).

Si votre organisation ne peut pas se conformer à la GPL, une licence commerciale est disponible. Contactez [feritzcan93@gmail.com](mailto:feritzcan93@gmail.com) pour plus de détails.

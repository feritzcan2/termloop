...
Release build with isolated "TermLoop STAGING" identity. Runs side-by-side with
the production TermLoop app.
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="TermLoop STAGING ${TAG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.termloop.app.staging.${TAG_ID}"
  fi
  if [[ "$DERIVED_SET" -eq 0 ]]; then
    DERIVED_DATA="/tmp/termloop-staging-${TAG_SLUG}"
  fi
fi
...
if [[ -f "$INFO_PLIST" ]]; then
  ...
  TERMLOOPD_SOCKET="${APP_SUPPORT_DIR}/termloopd-${STAGING_SLUG}.sock"
  TERMLOOP_SOCKET="/tmp/termloop-${STAGING_SLUG}.sock"
...
"${OPEN_CLEAN_ENV[@]}" TERMLOOP_SOCKET_PATH="$TERMLOOP_SOCKET" TERMLOOPD_UNIX_PATH="$TERMLOOPD_SOCKET" open -g "$APP_PATH"

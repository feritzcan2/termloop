...
if [ "$(id -un)" != "cmux" ]; then
  ...
  echo "Run via: ssh cmux-vm 'cd /Users/termloop/GhosttyTabs && ./scripts/run-tests-v1.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/termloop-tests-v1"

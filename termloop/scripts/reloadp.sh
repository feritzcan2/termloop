...
xcodebuild -project GhosttyTabs.xcodeproj -scheme termloop -configuration Release -destination 'platform=macOS' build
pkill -x TermLoop || true
sleep 0.2
APP_PATH="$(
  find "$HOME/Library/Developer/Xcode/DerivedData" -path "*/Build/Products/Release/TermLoop.app" -print0 \
...
if [[ -z "${APP_PATH}" ]]; then
  echo "TermLoop.app not found in DerivedData" >&2
  exit 1
fi
...
APP_PROCESS_PATH="${APP_PATH}/Contents/MacOS/TermLoop"
ATTEMPT=0
MAX_ATTEMPTS=20

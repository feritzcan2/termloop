...
# It is intentionally guarded so we don't accidentally kill the host user's termloop instances.
if [ "$(id -un)" != "cmux" ]; then
  echo "ERROR: This script is intended to be run on the cmux-vm (user: cmux)." >&2
  echo "Run via: ssh cmux-vm 'cd /Users/termloop/GhosttyTabs && ./scripts/run-tests-v2.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/termloop-tests-v2"
APP="$DERIVED_DATA_PATH/Build/Products/Debug/TermLoop DEV.app"
RUN_TAG="tests-v2"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: TermLoop DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "TermLoop DEV" || true
  pkill -x "TermLoop" || true
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "TermLoop DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.termloop.app.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  TERMLOOP_TAG="$RUN_TAG" TERMLOOP_UI_TEST_MODE=1 "$APP/Contents/MacOS/TermLoop DEV" >/dev/null 2>&1 &

  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/termloop-debug*.sock /tmp/cmux*.sock 2>/dev/null | head -1 || true)
  ...
  export TERMLOOP_SOCKET_PATH="$SOCK"
  export TERMLOOP_SOCKET="$SOCK"

  # Ensure LaunchServices has a visible/main window attached for rendering checks.
  TERMLOOP_TAG="$RUN_TAG" open "$APP" >/dev/null 2>&1 || true
  sleep 0.5
...
from termloop import termloop  # type: ignore

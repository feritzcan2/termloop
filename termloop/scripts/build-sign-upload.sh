...
ENTITLEMENTS="termloop.entitlements"
APP_PATH="build/Build/Products/Release/TermLoop.app"
...
echo "Building app..."
rm -rf build/
xcodebuild -scheme termloop -configuration Release -derivedDataPath build CODE_SIGNING_ALLOWED=NO build 2>&1 | tail -5
...
/usr/libexec/PlistBuddy -c "Add :SUFeedURL string https://github.com/feritzcan2/termloop/releases/latest/download/termloop-appcast.xml" "$APP_PLIST"
...
# --- Notarize app ---
echo "Notarizing app..."
ditto -c -k --sequesterRsrc --keepParent "$APP_PATH" termloop-notary.zip
xcrun notarytool submit termloop-notary.zip \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple "$APP_PATH"
xcrun stapler validate "$APP_PATH"
rm -f termloop-notary.zip
echo "App notarized"

# --- Create and notarize DMG ---
echo "Creating DMG..."
rm -f termloop-macos.dmg
create-dmg --codesign "$SIGN_HASH" termloop-macos.dmg "$APP_PATH"
echo "Notarizing DMG..."
xcrun notarytool submit termloop-macos.dmg \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple termloop-macos.dmg
xcrun stapler validate termloop-macos.dmg
echo "DMG notarized"

# --- Generate Sparkle appcast ---
echo "Generating appcast..."
./scripts/sparkle_generate_appcast.sh termloop-macos.dmg "$TAG" termloop-appcast.xml
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  for asset in termloop-macos.dmg termloop-appcast.xml; do
  ...
  if [[ "$ALLOW_OVERWRITE" == "true" ]]; then
    ...
    gh release upload "$TAG" termloop-macos.dmg termloop-appcast.xml --clobber
  else
    echo "Uploading to existing release $TAG..."
    gh release upload "$TAG" termloop-macos.dmg termloop-appcast.xml
  fi
else
  echo "Creating release $TAG and uploading..."
  gh release create "$TAG" termloop-macos.dmg termloop-appcast.xml --title "$TAG" --notes "See CHANGELOG.md for details"
fi
...
if [[ "$TAG" != *"-nightly"* ]]; then
  VERSION="${TAG#v}"
  DMG_SHA256=$(shasum -a 256 termloop-macos.dmg | cut -d' ' -f1)
  echo "Updating homebrew cask to $VERSION (SHA: $DMG_SHA256)..."
  CASK_FILE="homebrew-cmux/Casks/termloop.rb"
  if [ -f "$CASK_FILE" ]; then
...
CASKEOF
    cd homebrew-cmux
    git add Casks/termloop.rb
    if git diff --staged --quiet; then
      echo "Homebrew cask already up to date"
    else
      git commit -m "Update termloop to ${VERSION}"
      git push
      echo "Homebrew cask updated"
    fi
    cd ..
  else
    echo "WARNING: vendored homebrew-cmux checkout not found, skipping cask update"
  fi
fi

# --- Cleanup ---
rm -rf build/ termloop-macos.dmg termloop-appcast.xml
echo ""
echo "=== Release $TAG complete ==="
say "TermLoop release complete"

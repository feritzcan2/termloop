...
Builds termloopd-remote release assets for the supported remote platforms and emits:
  termloopd-remote-<goos>-<goarch>[-<suffix>]
  termloopd-remote-checksums[-<suffix>].txt
  termloopd-remote-manifest[-<suffix>].json
...
if ! command -v go >/dev/null 2>&1; then
  echo "error: go is required to build termloopd-remote release assets" >&2
  exit 1
fi
...
rm -f "$OUTPUT_DIR"/termloopd-remote-* "$OUTPUT_DIR"/termloopd-remote-checksums.txt "$OUTPUT_DIR"/termloopd-remote-manifest.json
...
CHECKSUMS_ASSET_NAME="termloopd-remote-checksums${SUFFIX_TAG}.txt"
CHECKSUMS_PATH="${OUTPUT_DIR}/${CHECKSUMS_ASSET_NAME}"
MANIFEST_PATH="${OUTPUT_DIR}/termloopd-remote-manifest${SUFFIX_TAG}.json"
...
: > "$CHECKSUMS_PATH"
ENTRIES_FILE="$(mktemp "${TMPDIR:-/tmp}/termloopd-remote-entries.XXXXXX")"
trap 'rm -f "$ENTRIES_FILE"' EXIT
: > "$ENTRIES_FILE"

for target in "${TARGETS[@]}"; do
  read -r GOOS GOARCH <<<"$target"
  ASSET_NAME="termloopd-remote-${GOOS}-${GOARCH}${SUFFIX_TAG}"
  ...
  BUILD_PATH="${OUTPUT_DIR}/termloopd-remote-${GOOS}-${GOARCH}.build"
  (
    cd "$DAEMON_ROOT"
    GOOS="$GOOS" \
    GOARCH="$GOARCH" \
...
echo "Built termloopd-remote assets in ${OUTPUT_DIR}"

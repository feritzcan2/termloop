...
APP="$HOME/Library/Developer/Xcode/DerivedData/termloop-${TAG_SLUG}/Build/Products/Debug/TermLoop DEV ${TAG}.app"
BID="com.termloop.app.debug.${TAG_ID}"
SOCK="/tmp/termloop-debug-${TAG_SLUG}.sock"
DSOCK="$HOME/Library/Application Support/TermLoop/termloopd-dev-${TAG_SLUG}.sock"
LOG="/tmp/termloop-debug-${TAG_SLUG}.log"
...
sleep 0.5
pkill -f "TermLoop DEV ${TAG}.app/Contents/MacOS/TermLoop DEV" || true
rm -f "$SOCK" "$DSOCK"
sleep 0.5
...
OPEN_ENV=(
  ...
  -u TERMLOOP_PORT
  -u TERMLOOP_PORT_END
  -u TERMLOOP_PORT_RANGE
  -u TERMLOOP_DEBUG_LOG
  -u TERMLOOP_BUNDLE_ID
  -u TERMLOOP_SHELL_INTEGRATION
  -u TERMLOOP_SHELL_INTEGRATION_DIR
  -u TERMLOOP_LOAD_GHOSTTY_ZSH_INTEGRATION
  -u GHOSTTY_BIN_DIR
  ...
  -u TERMINFO
  -u XDG_DATA_DIRS
  "TERMLOOP_SOCKET_MODE=${MODE}"
  "TERMLOOP_SOCKET_PATH=${SOCK}"
  "TERMLOOPD_UNIX_PATH=${DSOCK}"
  "TERMLOOP_DEBUG_LOG=${LOG}"
)

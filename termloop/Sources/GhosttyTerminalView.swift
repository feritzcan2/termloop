...
class GhosttyApp {
    ...
    private static let releaseBundleIdentifier = "com.termloop.app"
    ...
    private static func resolveBackgroundLogURL(
        ...
        if let explicitPath = environment["TERMLOOP_DEBUG_BG_LOG"],
           !explicitPath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return URL(fileURLWithPath: explicitPath)
        }

        if let debugLogPath = environment["TERMLOOP_DEBUG_LOG"],
        ...
        return URL(fileURLWithPath: "/tmp/termloop-bg.log")
    }
                ...
                TerminalImageTransferPlanner.execute(
                    ...
                    uploadWorkspaceRemote: { fileURLs, operation, finish in
                        guard let workspace = MainActor.assumeIsolated({
                            ...
                            finish(.failure(NSError(domain: "termloop.remote.paste", code: 3)))
    ...
    let backgroundLogEnabled = {
        if ProcessInfo.processInfo.environment["TERMLOOP_DEBUG_BG"] == "1" {
            return true
        }
        if ProcessInfo.processInfo.environment["TERMLOOP_DEBUG_LOG"] != nil {
            return true
        }
    ...
    private var backgroundLogHandle: FileHandle?
    ...
    #if DEBUG
    private static let initLogPath = "/tmp/termloop-ghostty-init.log"
    ...
    private func initializeGhostty() {
        ...
        if let created = ghostty_app_new(&runtimeConfig, primaryConfig) {
            ...
            // termloop still launches with working terminals.
            ...
            loadInlineGhosttyConfig(
                ...
                into: fallbackConfig,
                prefix: "termloop-layer-bg",
                logLabel: "layer background (fallback)"
            )
            loadInlineGhosttyConfig(
                "shell-integration = none",
                into: fallbackConfig,
                prefix: "termloop-shell-integration-override",
    ...
    private func loadDefaultConfigFilesWithLegacyFallback(_ config: ghostty_config_t) {
        ...
        if !useHostLayerBackground {
            ...
            loadInlineGhosttyConfig(
                ...
                into: config,
                prefix: "termloop-layer-bg-image-override",
                logLabel: "layer background image override"
            )
        } else {
            // termloop provides the terminal background via backgroundView (CALayer)
        ...
        // Prevent Ghostty from overriding ZDOTDIR — termloop handles shell
    ...
    ///
    /// See: https://github.com/feritzcan2/termloop/pull/1017
    private func loadCJKFontFallbackIfNeeded(_ config: ghostty_config_t) {
        ...
        loadInlineGhosttyConfig(
            lines,
            into: config,
            prefix: "termloop-cjk-font-fallback",
    ...
    /// font already covers the ranges termloop would otherwise auto-map.
    ...
    /// Returns only the CJK mappings termloop should auto-inject after respecting
    ...
    /// application-support config paths that termloop may load at runtime.
    ...
    /// Returns the top-level config paths that termloop will actually load before
    ...
    /// Scans a single config file for font settings relevant to TermLoop's
...
#if DEBUG
        dlog(
            "loaded termloop app support ghostty config from: \(urls.map(\.path).joined(separator: ", "))"
        )
#endif
        #endif
    }
    ...
    private func handleAction(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        if target.tag != GHOSTTY_TARGET_SURFACE {
            ...
            if action.tag == GHOSTTY_ACTION_DESKTOP_NOTIFICATION {
                ...
                return performOnMain {
                    ...
                    // Suppress OSC notifications for workspaces with active terminal-agent hook
                    // sessions. The hook system manages notifications with proper lifecycle
                    // tracking; raw OSC notifications would duplicate or outlive the structured
                    // hooks.
                    ...
                    if let workspace = owningManager.tabs.first(where: { $0.id == tabId }),
                       TerminalAgentActivityStore.shared.state(forWorkspaceId: workspace.id) != nil || !workspace.agentPIDs.isEmpty {
                        return true
                    }
        ...
        if action.tag == GHOSTTY_ACTION_SHOW_CHILD_EXITED {
            ...
            // handles this action. For TermLoop, the correct behavior is to close
        ...
        case GHOSTTY_ACTION_OPEN_URL:
            ...
            if !BrowserLinkOpenSettings.openTerminalLinksInTermLoopBrowser() {
                #if DEBUG
    ...
    func logBackground(_ message: String) {
        ...
        let line =
            "\(timestamp) seq=\(sequence) t+\(String(format: "%.3f", uptimeMs))ms thread=\(threadLabel) frame60=\(frame60) frame120=\(frame120) termloop bg: \(message)\n"
        guard let data = line.data(using: .utf8) else { return }
        _ = openBackgroundLogHandleIfNeeded()
        do {
            try backgroundLogHandle?.write(contentsOf: data)
        } catch {
            try? backgroundLogHandle?.close()
            backgroundLogHandle = nil
            if let handle = openBackgroundLogHandleIfNeeded() {
                try? handle.write(contentsOf: data)
            }
        }
    }

    private func openBackgroundLogHandleIfNeeded() -> FileHandle? {
        if let backgroundLogHandle {
            return backgroundLogHandle
        }
        if !FileManager.default.fileExists(atPath: backgroundLogURL.path) {
            FileManager.default.createFile(atPath: backgroundLogURL.path, contents: nil)
        }
        backgroundLogHandle = try? FileHandle(forWritingTo: backgroundLogURL)
        try? backgroundLogHandle?.seekToEnd()
        return backgroundLogHandle
    }
}
...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    /// Port ordinal for TERMLOOP_PORT range assignment
    ...
    /// Desired renderer visibility. This is tracked even before the Ghostty runtime
    /// surface exists so background-created hidden surfaces do not start their
    /// display link as visible by default.
    private var desiredOcclusionVisible: Bool = true
#if DEBUG
...
#if DEBUG
    private static let surfaceLogPath = "/tmp/termloop-ghostty-surface.log"
    private static let sizeLogPath = "/tmp/termloop-ghostty-size.log"
    ...
    private static func sizeLog(_ message: String) {
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1" else { return }
        let timestamp = ISO8601DateFormatter().string(from: Date())
        let line = "[\(timestamp)] \(message)\n"
    ...
    private func createSurface(for view: GhosttyNSView) {
        ...
        // MARK: termloop-hook
        TermLoopHooks.applyManagedGitEnvironment(to: &env, protectedKeys: &protectedStartupEnvironmentKeys)
        // MARK: /termloop-hook
        func setManagedEnvironmentValue(_ key: String, _ value: String) {
            env[key] = value
            protectedStartupEnvironmentKeys.insert(key)
        }
        let discoveredCLIDirectories = CLIDiscovery.pathDirectories()
        func setManagedDiscoveredCLIPath(_ key: String, executableName: String) {
            guard let resolvedPath = CLIDiscovery.findBinary(
                name: executableName,
                in: discoveredCLIDirectories
            )?.path else {
                return
            }
            setManagedEnvironmentValue(key, resolvedPath)
        }
        ...
        setManagedEnvironmentValue("TERMLOOP_SOCKET", socketPath)
        if let socketTag = Self.cmuxTag(fromSocketPath: socketPath) {
            setManagedEnvironmentValue("TERMLOOP_TAG", socketTag)
            setManagedEnvironmentValue("TERMLOOP_DEBUG_LOG", "/tmp/termloop-debug-\(socketTag).log")
            setManagedEnvironmentValue("TERMLOOP_DEBUG_BG_LOG", "/tmp/termloop-debug-\(socketTag)-bg.log")
        }
        if let bundledCLIURL = Bundle.main.resourceURL?.appendingPathComponent("bin/termloop"),
           FileManager.default.isExecutableFile(atPath: bundledCLIURL.path) {
            setManagedEnvironmentValue("TERMLOOP_BUNDLED_CLI_PATH", bundledCLIURL.path)
        }
        if let bundleId = Bundle.main.bundleIdentifier, !bundleId.isEmpty {
            setManagedEnvironmentValue("TERMLOOP_BUNDLE_ID", bundleId)
        }
        ...
        do {
            ...
            setManagedEnvironmentValue("TERMLOOP_PORT", String(startPort))
            setManagedEnvironmentValue("TERMLOOP_PORT_END", String(startPort + Self.sessionPortRangeSize - 1))
            setManagedEnvironmentValue("TERMLOOP_PORT_RANGE", String(Self.sessionPortRangeSize))
        }

        let claudeHooksEnabled = ClaudeCodeIntegrationSettings.hooksEnabled()
        if !claudeHooksEnabled {
            setManagedEnvironmentValue("TERMLOOP_CLAUDE_HOOKS_DISABLED", "1")
        }
        if let customClaudePath = ClaudeCodeIntegrationSettings.customClaudePath() {
            setManagedEnvironmentValue("TERMLOOP_CUSTOM_CLAUDE_PATH", customClaudePath)
        }
        setManagedDiscoveredCLIPath("TERMLOOP_REAL_CLAUDE_PATH", executableName: "claude")
        setManagedDiscoveredCLIPath("TERMLOOP_REAL_CODEX_PATH", executableName: "codex")
        if !CursorIntegrationSettings.hooksEnabled() {
            setManagedEnvironmentValue("TERMLOOP_CURSOR_HOOKS_DISABLED", "1")
        }
        if !GeminiIntegrationSettings.hooksEnabled() {
            setManagedEnvironmentValue("TERMLOOP_GEMINI_HOOKS_DISABLED", "1")
        }
        ...
        if shellIntegrationEnabled,
           let integrationDir = Bundle.main.resourceURL?.appendingPathComponent("shell-integration").path {
            setManagedEnvironmentValue("TERMLOOP_SHELL_INTEGRATION", "1")
            setManagedEnvironmentValue("TERMLOOP_SHELL_INTEGRATION_DIR", integrationDir)
            ...
            if shellName == "zsh" {
                if GhosttyApp.shared.userGhosttyShellIntegrationMode != "none" {
                    setManagedEnvironmentValue("TERMLOOP_LOAD_GHOSTTY_ZSH_INTEGRATION", "1")
                }
                ...
                if let candidateZdotdir, !candidateZdotdir.isEmpty {
                    ...
                    if !isGhosttyInjected {
                        setManagedEnvironmentValue("TERMLOOP_ZSH_ZDOTDIR", candidateZdotdir)
                    }
                }
                ...
                if GhosttyApp.shared.userGhosttyShellIntegrationMode != "none" {
                    setManagedEnvironmentValue("TERMLOOP_LOAD_GHOSTTY_BASH_INTEGRATION", "1")
                }
                // macOS ships /bin/bash 3.2, where Ghostty's automatic bash
                // integration is unsupported and HOME-based wrapper startup is
                // not reliable. Bootstrap termloop bash integration on the first
                // interactive prompt instead.
                setManagedEnvironmentValue("PROMPT_COMMAND", """
                unset PROMPT_COMMAND; \
                if [[ "${TERMLOOP_LOAD_GHOSTTY_BASH_INTEGRATION:-0}" == "1" && -n "${GHOSTTY_RESOURCES_DIR:-}" ]]; then \
                ...
                _cmux_bash_integration="$TERMLOOP_SHELL_INTEGRATION_DIR/termloop-bash-integration.bash"; \
...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    private static let focusDebugEnabled: Bool = {
        if ProcessInfo.processInfo.environment["TERMLOOP_FOCUS_DEBUG"] == "1" {
            return true
        }
    ...
    private static let sidebarTabReorderPasteboardType = NSPasteboard.PasteboardType("com.termloop.sidebar-tab-reorder")
    ...
    private static let keyLatencyProbeEnabled: Bool = {
        if ProcessInfo.processInfo.environment["TERMLOOP_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
    ...
    private func hoverModsFromFlags(
        ...
        if suppressCommandPathHover, flags.contains(.command) {
            _ = CmuxUITestCapture.mutateJSONObjectIfConfigured(
                envKey: "TERMLOOP_UI_TEST_CMD_HOVER_DIAGNOSTICS_PATH"
            ) { payload in
    ...
    private func cmuxTerminalCmdClickQuicklookOverride(_ decodedWord: String) -> String {
        let env = ProcessInfo.processInfo.environment
        guard let override = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_QUICKLOOK_OVERRIDE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func cmuxTerminalCmdClickViewportOffsetDelta(_ viewportOffsetStart: Int) -> Int {
        let env = ProcessInfo.processInfo.environment
        guard let delta = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_VIEWPORT_OFFSET_DELTA"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    static func handleDropForTesting(
        ...
        TerminalImageTransferPlanner.execute(
            ...
            uploadDetectedSSH: { _, _, _, finish in
                finish(.failure(NSError(domain: "termloop.remote.drop", code: 4)))
            },
            insertText: sendText,
        ...
        TerminalImageTransferPlanner.execute(
            ...
            uploadWorkspaceRemote: { [weak self] fileURLs, operation, finish in
                guard let workspace = MainActor.assumeIsolated({
                    ...
                    finish(.failure(NSError(domain: "termloop.remote.drop", code: 3)))
                    GhosttyPasteboardHelper.cleanupTransferredTemporaryImageFiles(fileURLs)
                    return
                }
                workspace.uploadDroppedFilesForRemoteTerminal(
    ...
    fileprivate func debugSimulateFileDrop(paths: [String]) -> Bool {
        ...
        let pbName = NSPasteboard.Name("termloop.debug.drop.\(UUID().uuidString)")
...
final class GhosttySurfaceScrollView: NSView {
    ...
    func isAttached(to terminalSurface: TerminalSurface) -> Bool {
        surfaceView.terminalSurface === terminalSurface &&
            terminalSurface.isAttached(to: surfaceView)
    }
    ...
    func triggerFlash(style: FlashStyle = .navigation) {
        DispatchQueue.main.async { [weak self] in
            ...
            self.flashLayer.add(animation, forKey: "termloop.flash")
        }
    }
    ...
    func ensureFocus(for tabId: UUID, surfaceId: UUID) {
        // MARK: termloop-hook
        guard !TermLoopHooks.shouldDeferTerminalFocusSteal() else { return }
        // MARK: /termloop-hook

...
enum SocketControlMode: String, CaseIterable, Identifiable {
    ...
    var displayName: String {
        ...
        case .cmuxOnly:
            return String(localized: "socketControl.cmuxOnly.name", defaultValue: "TermLoop processes only")
        case .automation:
    ...
    var description: String {
        ...
        case .cmuxOnly:
            return String(localized: "socketControl.cmuxOnly.description", defaultValue: "Only processes started inside TermLoop terminals can send commands.")
        case .automation:
...
enum SocketControlPasswordStore {
    static let directoryName = "termloop"
    static let fileName = "socket-control-password"
    static let didChangeNotification = Notification.Name("termloop.socketControlPasswordDidChange")
    ...
    private static let legacyKeychainService = "com.termloop.app.socket-control"
...
struct SocketControlSettings {
    ...
    static let allowSocketPathOverrideKey = "TERMLOOP_ALLOW_SOCKET_OVERRIDE"
    static let socketPasswordEnvKey = "TERMLOOP_SOCKET_PASSWORD"
    static let launchTagEnvKey = "TERMLOOP_TAG"
    static let baseDebugBundleIdentifier = "com.termloop.app.debug"
    private static let socketDirectoryName = "termloop"
    private static let stableSocketFileName = "termloop.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyStableDefaultSocketPath = "/tmp/termloop.sock"
    static let legacyLastSocketPathFile = "/tmp/termloop-last-socket-path"
    ...
    static func shouldBlockUntaggedDebugLaunch(
        ...
        // so isRunningUnderXCTest() misses it. Check for any TERMLOOP_UI_TEST_ env var.
        if environment.keys.contains(where: { $0.hasPrefix("TERMLOOP_UI_TEST_") }) {
            return false
        }
    ...
    static func socketPath(
        ...
        if let taggedDebugPath = taggedDebugSocketPath(
            ...
            if isTruthy(environment[allowSocketPathOverrideKey]),
               let override = environment["TERMLOOP_SOCKET_PATH"],
               !override.isEmpty {
                return override
            }
        ...
        guard let override = environment["TERMLOOP_SOCKET_PATH"], !override.isEmpty else {
            return fallback
        }
    ...
    static func defaultSocketPath(
        ...
        if bundleIdentifier == "com.termloop.app.nightly" {
            return "/tmp/termloop-nightly.sock"
        }
        ...
        if isStagingBundleIdentifier(bundleIdentifier) {
            return "/tmp/termloop-staging.sock"
        }
    ...
    static func userScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        stableSocketDirectoryURL()?
            .appendingPathComponent("termloop-\(currentUserID).sock", isDirectory: false)
            .path ?? "/tmp/termloop-\(currentUserID).sock"
    }
    ...
    static func isDebugLikeBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == "com.termloop.app.debug"
            || bundleIdentifier.hasPrefix("com.termloop.app.debug.")
    }

    /// Tagged DEV builds have bundle IDs like `com.termloop.app.debug.<tag>`.
    ...
    static func taggedDebugSocketPath(
        ...
        if bundleId.hasPrefix("\(baseDebugBundleIdentifier).") {
            ...
            if !slug.isEmpty {
                return "/tmp/termloop-debug-\(slug).sock"
            }
        }
        ...
        return "/tmp/termloop-debug-\(tag).sock"
    }

    static func isStagingBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        guard let bundleIdentifier else { return false }
        return bundleIdentifier == "com.termloop.app.staging"
    ...
    static func envOverrideEnabled(
        ...
        guard let raw = environment["TERMLOOP_SOCKET_ENABLE"], !raw.isEmpty else {
            return nil
        }
    ...
    static func envOverrideMode(
        ...
        guard let raw = environment["TERMLOOP_SOCKET_MODE"], !raw.isEmpty else {
            return nil
        }

...
final class WindowTerminalHostView: NSView {
    private struct DividerRegion {
        ...
        var cursorKind: DividerCursorKind {
            isVertical ? .vertical : .horizontal
        }

        func contains(_ windowPoint: NSPoint, expansion: CGFloat) -> Bool {
            rectInWindow.insetBy(dx: -expansion, dy: -expansion).contains(windowPoint)
        }
    }
    ...
    private static let cursorRectExpansion: CGFloat = 4
    private static let splitDividerHitExpansion: CGFloat = 5
    private static let splitDividerRegionCacheLifetime: TimeInterval = 0.25
    private var cachedSidebarDividerX: CGFloat?
    private var sidebarDividerMissCount = 0
    private var splitDividerRegionCache: [DividerRegion] = []
    private var splitDividerRegionCacheRoot: ObjectIdentifier?
    private var splitDividerRegionCacheBuiltAt: TimeInterval = 0
    ...
    override func resetCursorRects() {
        ...
        let regions = splitDividerRegions(in: rootView, forceRefresh: true)
        for region in regions {
            var rectInHost = convert(region.rectInWindow, from: nil)
            rectInHost = rectInHost.insetBy(
                dx: region.isVertical ? -Self.cursorRectExpansion : 0,
                dy: region.isVertical ? 0 : -Self.cursorRectExpansion
            )
    ...
    private func splitDividerCursorKind(at point: NSPoint) -> DividerCursorKind? {
        ...
        return splitDividerRegions(in: rootView).first {
            $0.contains(windowPoint, expansion: Self.splitDividerHitExpansion)
        }?.cursorKind
    }
    ...
    static func hasSplitDivider(atScreenPoint screenPoint: NSPoint, in window: NSWindow) -> Bool {
        ...
        // This is called from the global mouse-down monitor, outside a host view
        // instance, so it intentionally builds a fresh one-off region list.
    ...
    private func invalidateSplitDividerRegionCache() {
        splitDividerRegionCache.removeAll(keepingCapacity: true)
        splitDividerRegionCacheRoot = nil
        splitDividerRegionCacheBuiltAt = 0
    }

    private func splitDividerRegions(in rootView: NSView, forceRefresh: Bool = false) -> [DividerRegion] {
        let root = ObjectIdentifier(rootView)
        let now = ProcessInfo.processInfo.systemUptime
        if !forceRefresh,
           splitDividerRegionCacheRoot == root,
           now - splitDividerRegionCacheBuiltAt < Self.splitDividerRegionCacheLifetime {
            return splitDividerRegionCache
        }

        var regions: [DividerRegion] = []
        Self.collectSplitDividerRegions(in: rootView, into: &regions)
        splitDividerRegionCache = regions
        splitDividerRegionCacheRoot = root
        splitDividerRegionCacheBuiltAt = now
        return regions
    }
...
final class WindowTerminalPortal: NSObject {
...
#if TERMLOOP_ISSUE_483_PORTAL_RECOVERY
    ...
    fileprivate func synchronizeAllEntriesFromExternalGeometryChange() {
        guard ensureInstalled(performLayout: false) else { return }
    ...
    @discardableResult
    private func ensureInstalled(performLayout: Bool = true) -> Bool {
    ...
    func bind(hostedView: GhosttySurfaceScrollView, to anchorView: NSView, visibleInUI: Bool, zPriority: Int = 0) {
        ...
        synchronizeHostedView(withId: hostedId, ensurePortalInstalled: false)
    ...
    func synchronizeHostedViewForAnchor(_ anchorView: NSView) {
        ...
        if let primaryHostedId {
            synchronizeHostedView(withId: primaryHostedId, ensurePortalInstalled: false)
        }
    ...
    private func synchronizeHostedView(withId hostedId: ObjectIdentifier, ensurePortalInstalled: Bool = true) {
        if ensurePortalInstalled {
            guard ensureInstalled() else { return }
        }
        guard var entry = entriesByHostedId[hostedId] else { return }

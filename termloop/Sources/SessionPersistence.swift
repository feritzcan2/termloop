...
enum SessionPersistencePolicy {
    static let defaultSidebarWidth: Double = 280
...
enum SessionRestorePolicy {
    static func isRunningUnderAutomatedTests(
        ...
        if environment["TERMLOOP_UI_TEST_MODE"] == "1" {
            return true
        }
    ...
    static func shouldAttemptRestore(
        ...
        if environment["TERMLOOP_DISABLE_SESSION_RESTORE"] == "1" {
            return false
        }
...
struct SessionWorkspaceSnapshot: Codable, Sendable {
    ...
    var gitBranch: SessionGitBranchSnapshot?
    /// Stable TermLoop-owned identity used to join the upstream workspace
    /// snapshot with the TermLoop sidecar across relaunches. Workspace UUIDs
    /// are runtime-only and are intentionally reminted on restore.
...
enum SessionScrollbackReplayStore {
    static let environmentKey = "TERMLOOP_RESTORE_SCROLLBACK_FILE"
    private static let directoryName = "termloop-session-scrollback"

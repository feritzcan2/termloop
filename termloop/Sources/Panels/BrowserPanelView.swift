...
struct BrowserPanelView: View {
    ...
    private var remoteSuggestionsEnabled: Bool {
        ...
        if ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_REMOTE_SUGGESTIONS_JSON"] != nil ||
            UserDefaults.standard.string(forKey: "TERMLOOP_UI_TEST_REMOTE_SUGGESTIONS_JSON") != nil {
            return true
        }
        // Keep UI tests deterministic by disabling network suggestions when requested.
        if ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_DISABLE_REMOTE_SUGGESTIONS"] == "1" {
            return false
        }
    ...
    private func forcedRemoteSuggestionsForUITest() -> [String]? {
        let raw = ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_REMOTE_SUGGESTIONS_JSON"]

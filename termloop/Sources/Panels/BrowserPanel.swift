...
final class BrowserProfileStore: ObservableObject {
    ...
    func historyFileURL(for profileID: UUID) -> URL? {
        ...
        let bundleId = Bundle.main.bundleIdentifier ?? "termloop"
...
enum BrowserLinkOpenSettings {
    static let openTerminalLinksInTermLoopBrowserKey = "browserOpenTerminalLinksInTermLoopBrowser"
    static let defaultOpenTerminalLinksInTermLoopBrowser: Bool = true

    static let openSidebarPullRequestLinksInTermLoopBrowserKey = "browserOpenSidebarPullRequestLinksInTermLoopBrowser"
    static let defaultOpenSidebarPullRequestLinksInTermLoopBrowser: Bool = true

    static let openSidebarPortLinksInTermLoopBrowserKey = "browserOpenSidebarPortLinksInTermLoopBrowser"
    static let defaultOpenSidebarPortLinksInTermLoopBrowser: Bool = true

    static let interceptTerminalOpenCommandInTermLoopBrowserKey = "browserInterceptTerminalOpenCommandInTermLoopBrowser"
    static let defaultInterceptTerminalOpenCommandInTermLoopBrowser: Bool = true
    ...
    static func openTerminalLinksInTermLoopBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: openTerminalLinksInTermLoopBrowserKey) == nil {
            return defaultOpenTerminalLinksInTermLoopBrowser
        }
        return defaults.bool(forKey: openTerminalLinksInTermLoopBrowserKey)
    }

    static func openSidebarPullRequestLinksInTermLoopBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: openSidebarPullRequestLinksInTermLoopBrowserKey) == nil {
            return defaultOpenSidebarPullRequestLinksInTermLoopBrowser
        }
        return defaults.bool(forKey: openSidebarPullRequestLinksInTermLoopBrowserKey)
    }

    static func openSidebarPortLinksInTermLoopBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: openSidebarPortLinksInTermLoopBrowserKey) == nil {
            return defaultOpenSidebarPortLinksInTermLoopBrowser
        }
        return defaults.bool(forKey: openSidebarPortLinksInTermLoopBrowserKey)
    }

    static func interceptTerminalOpenCommandInTermLoopBrowser(defaults: UserDefaults = .standard) -> Bool {
        if defaults.object(forKey: interceptTerminalOpenCommandInTermLoopBrowserKey) != nil {
            return defaults.bool(forKey: interceptTerminalOpenCommandInTermLoopBrowserKey)
        }

        return defaultInterceptTerminalOpenCommandInTermLoopBrowser
    }

    static func initialInterceptTerminalOpenCommandInTermLoopBrowserValue(defaults: UserDefaults = .standard) -> Bool {
        interceptTerminalOpenCommandInTermLoopBrowser(defaults: defaults)
    }
...
func normalizedBrowserHistoryNamespace(bundleIdentifier: String) -> String {
    if bundleIdentifier.hasPrefix("com.termloop.app.debug.") {
        return "com.termloop.app.debug"
    }
    if bundleIdentifier.hasPrefix("com.termloop.app.staging.") {
        return "com.termloop.app.staging"
    }
...
actor BrowserSearchSuggestionService {
    ...
    func suggestions(engine: BrowserSearchEngine, query: String) async -> [String] {
        ...
        let forced = ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_REMOTE_SUGGESTIONS_JSON"]
            ?? UserDefaults.standard.string(forKey: "TERMLOOP_UI_TEST_REMOTE_SUGGESTIONS_JSON")
        if let forced,
...
final class BrowserPanel: Panel, ObservableObject {
    ...
    (() => {
      try {
        ...
        let id = active.getAttribute("data-termloop-addressbar-focus-id");
        if (!id) {
          id = "termloop-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
          active.setAttribute("data-termloop-addressbar-focus-id", id);
        }
    ...
    (() => {
      try {
        ...
        const ensureFocusId = (el) => {
          let id = el.getAttribute("data-termloop-addressbar-focus-id");
          if (!id) {
            id = "termloop-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
            el.setAttribute("data-termloop-addressbar-focus-id", id);
          }
          return id;
        };
    ...
    (() => {
      try {
        ...
        const selector = '[data-termloop-addressbar-focus-id="' + state.id + '"]';
...
class BrowserDownloadDelegate: NSObject, WKDownloadDelegate {
    ...
    private static let tempDir: URL = {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("termloop-downloads", isDirectory: true)
    ...
    var errorDescription: String? {
        ...
        case .missingDestinationProfile:
            return String(
                ...
                defaultValue: "The selected TermLoop browser profile no longer exists. Pick a destination profile again."
            )
        case .profileCreationFailed(let name):
            return String(
                format: String(
                    localized: "browser.import.error.destinationCreateFailed",
                    defaultValue: "TermLoop could not create the destination profile \"%@\"."
                ),
                name
            )
        }
            ...
            lines.append(
                String(
                    format: String(
                        ...
                        defaultValue: "Created TermLoop profiles: %@"
                    ),
...
enum BrowserDataImporter {
    ...
    private static func querySQLiteRows(
        ...
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent(
            "termloop-browser-import-\(UUID().uuidString)",
            isDirectory: true
        )
...
enum BrowserImportUITestFixtureLoader {
    ...
    static func browsers(from environment: [String: String]) -> [InstalledBrowserCandidate]? {
        guard let rawFixture = environment["TERMLOOP_UI_TEST_BROWSER_IMPORT_FIXTURE"],
        ...
        let resolvedProfiles = fixture.profiles.enumerated().map { index, name in
            InstalledBrowserProfile(
                ...
                rootURL: FileManager.default.temporaryDirectory
                    .appendingPathComponent("termloop-ui-test-browser-import")
    ...
    static func destinationProfiles(from environment: [String: String]) -> [BrowserProfileDefinition]? {
        guard let rawDestinations = environment["TERMLOOP_UI_TEST_BROWSER_IMPORT_DESTINATIONS"],
...
final class BrowserDataImportCoordinator {
    ...
    private func presentImportDialog(
        ...
        guard !browsers.isEmpty else {
            ...
            alert.informativeText = String(
                ...
                defaultValue: "TermLoop could not find browser profiles to import from on this Mac."
            )
            alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK"))
            alert.runModal()
            return
    ...
    private func captureSelectionIfRequested(
        ...
        let environment = ProcessInfo.processInfo.environment
        guard environment["TERMLOOP_UI_TEST_BROWSER_IMPORT_MODE"] == "capture-only" else { return false }
        guard let path = environment["TERMLOOP_UI_TEST_BROWSER_IMPORT_CAPTURE_PATH"], !path.isEmpty else {
            return true
        }
    ...
    private final class ImportWizardWindowController: NSObject, @preconcurrency NSWindowDelegate {
        ...
        private func setupSourceProfilesContainer() {
            ...
            sourceProfilesHelpLabel.stringValue = String(
                ...
                defaultValue: "Choose one or more source profiles. Step 3 lets you keep them separate or merge them into one TermLoop profile."
            )
        ...
        private func setupDataTypesContainer() {
            ...
            mergeProfilesRadio.title = String(
                ...
                defaultValue: "Merge all into one TermLoop profile"
            )
            ...
            let destinationTitleLabel = NSTextField(
                labelWithString: String(
                    localized: "browser.import.destination.termloop",
                    defaultValue: "TermLoop destination"
                )
            )
        ...
        private func rebuildStep3DestinationUI() {
            ...
            if presentation.showsSeparateRows {
                destinationHelpLabel.stringValue = String(
                    ...
                    defaultValue: "Missing TermLoop profiles are created when import starts."
                )
                destinationHelpLabel.isHidden = false
            } else if plan.entries.count > 1 {
                destinationHelpLabel.stringValue = String(
                    localized: "browser.import.destinationProfile.mergeHelp",
                    defaultValue: "All selected source profiles will be merged into the chosen TermLoop browser profile."
                )
                destinationHelpLabel.isHidden = false
            } else {

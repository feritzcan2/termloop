...
enum ContextBankPrompt {
    ...
    - When sibling files in the same directory are kept identical (canonically `CLAUDE.md` + `AGENTS.md` + `GEMINI.md`), DO NOT call the tool three times. Make ONE call with the primary file as `target_path` and the identical-twin paths in `mirror_paths`. The applier writes the same `add_text` to all of them and is symlink-aware (if AGENTS.md is a symlink to CLAUDE.md it is written once). **Matching line counts in the same directory is a candidate signal for mirrors, not proof** — if uncertain, use the `Read` tool to confirm the files actually share content before grouping. When in doubt, target a single file rather than risk mirroring divergent content.

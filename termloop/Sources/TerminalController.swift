...
extension Notification.Name {
    static let socketListenerDidStart = Notification.Name("termloop.socketListenerDidStart")
    ...
    static let mainWindowContextsDidChange = Notification.Name("termloop.mainWindowContextsDidChange")
...
class TerminalController {
    ...
    nonisolated static func shouldReplacePullRequest(
        ...
        return current.number != number
            ...
            || current.statusDetail != statusDetail
    ...
    private final class SocketFastPathState: @unchecked Sendable {
        private let queue = DispatchQueue(label: "com.termloop.socket-fast-path")
    ...
    func start(tabManager: TabManager, socketPath: String, accessMode: SocketControlMode) {
        ...
        let existingSocketPathExists = FileManager.default.fileExists(atPath: socketPath)
        if existing.isRunning && existing.socketPath == socketPath && existing.acceptLoopAlive && existingSocketPathExists {
            self.accessMode = accessMode
        ...
        // MARK: termloop-hook
        TermLoopTCPBridge.shared.start { [weak self] fd in
            guard let self else { Darwin.close(fd); return }
            if TermLoopTCPBridge.preflightAuth(socket: fd, accessMode: self.accessMode) {
                self.handleClient(fd, peerPid: nil)
            } else {
                Darwin.close(fd)
            }
        }
    ...
    nonisolated func stop() {
        ...
        if socketToClose >= 0 {
            unlinkSocketPathIfOwnedByListener(socketPathToUnlink, listenerSocket: socketToClose)
            close(socketToClose)
        } else {
            unlinkSocketPathIfNoConnectableListener(socketPathToUnlink)
        }
        // MARK: termloop-hook
        TermLoopTCPBridge.shared.stop()
        // MARK: /termloop-hook
    }

    nonisolated static func shouldUnlinkSocketPathOwnedByListener(_ path: String, listenerSocket: Int32) -> Bool {
        var socketStat = stat()
        guard fstat(listenerSocket, &socketStat) == 0 else {
            return false
        }
        ...
        guard (pathStat.st_mode & mode_t(S_IFMT)) == mode_t(S_IFSOCK) else {
            return false
        }

        return socketStat.st_dev == pathStat.st_dev
            && socketStat.st_ino == pathStat.st_ino
    }

    private nonisolated func unlinkSocketPathIfOwnedByListener(_ path: String, listenerSocket: Int32) {
        // Avoid a cross-process reload race: an older app instance can stop
        // after a newer instance has already rebound the same pathname. In
        // that case blindly unlinking `path` deletes the new listener's
        // filesystem entry while the new process keeps an anonymous open
        // socket, making CLI/MCP calls fail with "Socket not found".
        guard Self.shouldUnlinkSocketPathOwnedByListener(path, listenerSocket: listenerSocket) else { return }
        unlink(path)
    }

    private nonisolated func unlinkSocketPathIfNoConnectableListener(_ path: String) {
        guard TerminalController.probeSocketCommand("ping", at: path, timeout: 0.05) == nil else {
            return
        }
        unlink(path)
    }
    ...
    private func writeSocketResponse(_ response: String, to socket: Int32) {
        ...
        // MARK: termloop-hook
        TermLoopSocketIO.writeText(payload, to: socket)
        // MARK: /termloop-hook
    }
        ...
        defer {
            ...
            if let pathToUnlink = cleanup.pathToUnlink {
                if cleanup.socketToClose >= 0 {
                    unlinkSocketPathIfOwnedByListener(pathToUnlink, listenerSocket: cleanup.socketToClose)
                } else {
    ...
    private func handleClient(_ socket: Int32, peerPid: pid_t? = nil) {
        defer { close(socket) }
        // MARK: termloop-hook
        TermLoopHooks.handleAcceptedClient(socket: socket); defer { TermLoopHooks.handleClientDisconnect(socket: socket) }
        // MARK: /termloop-hook
        ...
        if accessMode == .cmuxOnly {
            ...
            if let pid {
                guard isDescendant(pid) else {
                    let msg = "ERROR: Access denied — only processes started inside termloop can connect\n"
                    // MARK: termloop-hook
                    TermLoopSocketIO.writeText(msg, to: socket)
                    // MARK: /termloop-hook
                    return
                }
            }
    ...
    private func processV2Command(_ jsonLine: String) -> String {
        ...
        // MARK: termloop-hook
        TermLoopHooks.refreshV2KnownRefsIfNeeded(method: method) { self.v2MainSync { self.v2RefreshKnownRefs() } }
        // MARK: /termloop-hook
        ...
        case "workspace.changes":
            return v2Result(id: id, self.v2WorkspaceChanges(params: params))
    ...
    // MARK: termloop-hook
    // TermLoopSocketCommands returns this type across file boundaries, so it
    // must be visible to the TermLoop module. Otherwise identical to upstream.
    enum V2CallResult {
        case ok(Any)
    ...
    private func v2WorkspaceSummaryPayload(
        ...
        // MARK: termloop-hook
        payload.merge(self.termLoopWorkspaceSummaryFields(for: workspace)) { _, new in new }
        // MARK: /termloop-hook
        if let index {
            payload["index"] = index
        }
    ...
    private func v2WorkspaceChanges(params: [String: Any]) -> V2CallResult {
        guard let tabManager = v2ResolveTabManager(params: params) else {
            return .err(code: "unavailable", message: "TabManager not available", data: nil)
        }
    ...
    private func v2BrowserEnsureInitScriptsApplied(surfaceId: UUID, browserPanel: BrowserPanel) {
        ...
        for css in styles {
            ...
            let styleScript = """
            (() => {
              const id = 'termloop-init-style-' + btoa(unescape(encodeURIComponent(\(cssLiteral)))).replace(/=+$/g, '');
    ...
    private func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let screenshotsDirectory = FileManager.default.temporaryDirectory
                .appendingPathComponent("termloop-browser-screenshots", isDirectory: true)
    ...
    private func helpText() -> String {
        ...
        Split & surface commands:
          ...
          reload_config                   - Reload Ghostty config, termloop settings, and refresh terminals
    ...
    private func dragPasteboardType(from token: String) -> NSPasteboard.PasteboardType? {
        ...
        case "sidebarreorder", "sidebar-reorder", "sidebar_tab_reorder",
            "com.termloop.sidebar-tab-reorder":
    ...
    private func panelSnapshot(_ args: String) -> String {
        ...
        let outputDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("termloop-screenshots")
    ...
    private func upsertSidebarMetadata(_ args: String, missingError: String) -> String {
        ...
        scheduleSidebarMutation(target: target) { controller, tab in
            ...
            // MARK: termloop-hook
            if TerminalAgentRegistry.shared.statusKeys.contains(key) {
                TermLoopHooks.notifyTerminalAgentStatusChange(workspaceId: tab.id, newValue: value)
            }
            // MARK: /termloop-hook
            if let pidValue {
                tab.agentPIDs[key] = pidValue
    ...
    private func reportPullRequest(_ args: String) -> String {
        ...
        case "closed", "abandoned":
            ...
            statusDetail = statusRaw
        default:
            return "ERROR: Invalid pull request state '\(statusRaw)' — use: open, draft, active, merged, completed, closed, abandoned"
        }

...
extension Notification.Name {
    static let termloopNotificationsPopoverVisibilityDidChange = Notification.Name("termloop.notificationsPopoverVisibilityDidChange")
}
...
private func postNotificationsPopoverVisibilityDidChange(isShown: Bool) {
    NotificationCenter.default.post(
        name: .termloopNotificationsPopoverVisibilityDidChange,
        object: nil,
...
struct TitlebarControlsView: View {
    ...
    var body: some View {
        ...
        controlsGroup(config: config)
            ...
            .onReceive(NotificationCenter.default.publisher(for: .termloopNotificationsPopoverVisibilityDidChange)) { notification in
    ...
    private func controlsGroup(config: TitlebarControlsStyleConfig) -> some View {
        ...
        let content = HStack(spacing: config.spacing) {
            ...
            // MARK: termloop-hook
            TermLoopHooks.titlebarSettingsButton(config: config)
            // MARK: /termloop-hook
        }
...
final class UpdateTitlebarAccessoryController {
    ...
    private let controlsIdentifier = NSUserInterfaceItemIdentifier("termloop.titlebarControls")
    ...
    private func isSettingsWindow(_ window: NSWindow) -> Bool {
        if window.identifier?.rawValue == "termloop.settings" {
            return true
        }

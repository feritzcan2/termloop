...
extension UpdateDriver: SPUUpdaterDelegate {
    ...
    func feedURLString(for updater: SPUUpdater) -> String? {
        ...
        if let override = env["TERMLOOP_UI_TEST_FEED_URL"], !override.isEmpty {
        ...
        // - termloop NIGHTLY has the nightly appcast URL injected by CI
        ...
        guard !resolved.url.isEmpty else {
            UpdateLogStore.shared.append("update feed disabled; no feed URL configured")
            recordFeedURLString("", usedFallback: false)
            return nil
        }

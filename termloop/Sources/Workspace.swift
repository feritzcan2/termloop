...
extension CmuxSurfaceConfigTemplate {
    /// Fresh shells should inherit visual terminal state, not the source shell's
    /// runtime cwd/command/env snapshot.
    func sanitizedForFreshShell() -> CmuxSurfaceConfigTemplate? {
        var sanitized = self
        sanitized.workingDirectory = nil
        sanitized.command = nil
        sanitized.environmentVariables = [:]
        sanitized.initialInput = nil
        sanitized.waitAfterCommand = false

        let hasMeaningfulState = sanitized.fontSize > 0
            || sanitized.workingDirectory != nil
            || sanitized.command != nil
            || !sanitized.environmentVariables.isEmpty
            || sanitized.initialInput != nil
            || sanitized.waitAfterCommand
        return hasMeaningfulState ? sanitized : nil
    }
}
...
extension Workspace {
    ...
    func sessionSnapshot(includeScrollback: Bool) -> SessionWorkspaceSnapshot {
        ...
        return SessionWorkspaceSnapshot(
            ...
            termLoopRestoreId: termLoopRestoreId
        )
    }

    func restoreSessionSnapshot(_ snapshot: SessionWorkspaceSnapshot) {
        applyTermLoopRestoreId(snapshot.termLoopRestoreId)
    ...
    private func sessionPanelSnapshot(panelId: UUID, includeScrollback: Bool) -> SessionPanelSnapshot? {
        // MARK: termloop-hook
        TermLoopHooks.ensurePanelDirectoryBeforeSnapshot(workspace: self, panelId: panelId)
        // MARK: /termloop-hook
...
final class WorkspaceRemoteDaemonPendingCallRegistry {
    ...
    private let queue = DispatchQueue(label: "com.termloop.remote-ssh.daemon-rpc.pending.\(UUID().uuidString)")
...
private final class WorkspaceRemoteDaemonRPCClient {
    ...
    private let writeQueue = DispatchQueue(label: "com.termloop.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
    private let stateQueue = DispatchQueue(label: "com.termloop.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
...
private final class WorkspaceRemoteDaemonProxyTunnel {
    private final class ProxySession {
        ...
        private static let remoteLoopbackProxyAliasHost = "termloop-loopback.localtest.me"
    ...
    private let queue = DispatchQueue(label: "com.termloop.remote-ssh.daemon-tunnel.\(UUID().uuidString)", qos: .utility)
...
private final class WorkspaceRemoteProxyBroker {
    ...
    private let queue = DispatchQueue(label: "com.termloop.remote-ssh.proxy-broker", qos: .utility)
...
private final class WorkspaceRemoteCLIRelayServer {
    private final class Session {
        ...
        private let challengeProtocol = "termloop-relay-auth"
        ...
        private static func roundTripUnixSocket(socketPath: String, request: Data) throws -> Data {
            ...
            guard connectResult == 0 else {
                throw NSError(domain: "cmux.remote.relay", code: 3, userInfo: [
                    NSLocalizedDescriptionKey: "failed to connect to local termloop socket",
                ])
            }
            ...
            while true {
                ...
                if errno == EAGAIN || errno == EWOULDBLOCK {
                    ...
                    throw NSError(domain: "cmux.remote.relay", code: 5, userInfo: [
                        NSLocalizedDescriptionKey: "timed out waiting for local termloop response",
                    ])
                }
                throw NSError(domain: "cmux.remote.relay", code: 6, userInfo: [
                    NSLocalizedDescriptionKey: "failed to read local termloop response",
                ])
            }
            return response
        }
    ...
    private let relayID: String
    private let relayToken: Data
    private let queue = DispatchQueue(label: "com.termloop.remote-ssh.cli-relay.\(UUID().uuidString)", qos: .utility)
...
final class WorkspaceRemoteSessionController {
    ...
    private let queue = DispatchQueue(label: "com.termloop.remote-ssh.\(UUID().uuidString)", qos: .utility)
    ...
    private func requestBootstrapRemoteTTYIfNeededLocked() {
        ...
        let command = "sh -c \(Self.shellSingleQuoted("tty_path=\"$HOME/.termloop/relay/\(relayPort).tty\"; if [ -r \"$tty_path\" ]; then cat \"$tty_path\"; fi"))"
        do {
    ...
    private static let remotePlatformProbeOSMarker = "__TERMLOOP_REMOTE_OS__="
    private static let remotePlatformProbeArchMarker = "__TERMLOOP_REMOTE_ARCH__="
    private static let remotePlatformProbeExistsMarker = "__TERMLOOP_REMOTE_EXISTS__="
    ...
    static func remoteRelayMetadataCleanupScript(relayPort: Int) -> String {
        ...
        socket_addr_file="$HOME/.termloop/socket_addr"
        if [ -r "$socket_addr_file" ] && [ "$(tr -d '\\r\\n' < "$socket_addr_file")" = "$relay_socket" ]; then
          rm -f "$socket_addr_file"
        fi
        rm -f "$HOME/.termloop/relay/\(relayPort).auth" "$HOME/.termloop/relay/\(relayPort).daemon_path" "$HOME/.termloop/relay/\(relayPort).tty"
        """
    }
    ...
    private func probeRemoteBootstrapStateLocked(version: String) throws -> RemoteBootstrapState {
        ...
        cmux_remote_path="$HOME/.termloop/bin/termloopd-remote/\(version)/${cmux_go_os}-${cmux_go_arch}/termloopd-remote"
    ...
    static let remoteDaemonManifestInfoKey = "TERMLOOPRemoteDaemonManifestJSON"
    ...
    private static func allowLocalDaemonBuildFallback(environment: [String: String] = ProcessInfo.processInfo.environment) -> Bool {
        environment["TERMLOOP_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1"
    }
    ...
    private static func explicitRemoteDaemonBinaryURL(environment: [String: String] = ProcessInfo.processInfo.environment) -> URL? {
        ...
        guard let path = environment["TERMLOOP_REMOTE_DAEMON_BINARY"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !path.isEmpty else {
            return nil
        }
    ...
    private static func versionedRemoteDaemonBuildURL(goOS: String, goArch: String, version: String) -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("termloop-remote-daemon-build", isDirectory: true)
            .appendingPathComponent(version, isDirectory: true)
            .appendingPathComponent("\(goOS)-\(goArch)", isDirectory: true)
            .appendingPathComponent("termloopd-remote", isDirectory: false)
    }
    ...
    private static func fetchRemoteManifestLocked(releaseURL: String, version: String) -> WorkspaceRemoteDaemonManifest? {
        guard let manifestURL = URL(string: "\(releaseURL)/termloopd-remote-manifest.json") else { return nil }
    ...
    private func buildLocalDaemonBinary(goOS: String, goArch: String, version: String) throws -> URL {
        ...
        guard Self.allowLocalDaemonBuildFallback() else {
            throw NSError(domain: "cmux.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "this build does not include a verified termloopd-remote manifest for \(goOS)-\(goArch). Use a release/nightly build, or set TERMLOOP_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 for a dev-only fallback.",
            ])
        }

        guard let repoRoot = Self.findRepoRoot() else {
            throw NSError(domain: "cmux.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "cannot locate termloop repo root for dev-only termloopd-remote build fallback",
            ])
        }
        ...
        guard let goBinary = Self.which("go") else {
            throw NSError(domain: "cmux.remote.daemon", code: 22, userInfo: [
                NSLocalizedDescriptionKey: "go is required for the dev-only termloopd-remote build fallback",
            ])
        }
        ...
        let result = try runProcess(
            executable: goBinary,
            arguments: ["build", "-trimpath", "-buildvcs=false", "-ldflags", ldflags, "-o", output.path, "./cmd/termloopd-remote"],
            environment: env,
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "cmux.remote.daemon", code: 23, userInfo: [
                NSLocalizedDescriptionKey: "failed to build termloopd-remote: \(detail)",
            ])
        }
        guard FileManager.default.isExecutableFile(atPath: output.path) else {
            throw NSError(domain: "cmux.remote.daemon", code: 24, userInfo: [
                NSLocalizedDescriptionKey: "termloopd-remote build output is not executable",
            ])
        }
    ...
    private func uploadRemoteDaemonBinaryLocked(localBinary: URL, remotePath: String) throws {
        ...
        guard scpResult.status == 0 else {
            ...
            throw NSError(domain: "cmux.remote.daemon", code: 31, userInfo: [
                NSLocalizedDescriptionKey: "failed to upload termloopd-remote: \(detail)",
            ])
        }
    ...
    static func remoteDropPath(for fileURL: URL, uuid: UUID = UUID()) -> String {
        ...
        return "/tmp/termloop-drop-\(uuid.uuidString.lowercased())\(lowercasedSuffix)"
    }
    ...
    private func helloRemoteDaemonLocked(remotePath: String) throws -> DaemonHello {
        ...
        return DaemonHello(
            name: (name?.isEmpty == false ? name! : "termloopd-remote"),
    ...
    static func remoteCLIWrapperScript() -> String {
        ...
        daemon="$HOME/.termloop/bin/termloopd-remote-current"
        socket_path="${TERMLOOP_SOCKET_PATH:-}"
        if [ -z "$socket_path" ] && [ -r "$HOME/.termloop/socket_addr" ]; then
          socket_path="$(tr -d '\\r\\n' < "$HOME/.termloop/socket_addr")"
        fi

        if [ -n "$socket_path" ] && [ "${socket_path#/}" = "$socket_path" ] && [ "${socket_path#*:}" != "$socket_path" ]; then
          relay_port="${socket_path##*:}"
          relay_map="$HOME/.termloop/relay/${relay_port}.daemon_path"
          if [ -r "$relay_map" ]; then
            mapped_daemon="$(tr -d '\\r\\n' < "$relay_map")"
    ...
    static func remoteCLIWrapperInstallScript(daemonRemotePath: String) -> String {
        ...
        return """
        mkdir -p "$HOME/.termloop/bin" "$HOME/.termloop/relay"
        ln -sf "$HOME/\(trimmedRemotePath)" "$HOME/.termloop/bin/termloopd-remote-current"
        wrapper_tmp="$HOME/.termloop/bin/.termloop-wrapper.tmp.$$"
        cat > "$wrapper_tmp" <<'CMUXWRAPPER'
        \(remoteCLIWrapperScript())
        CMUXWRAPPER
        chmod 755 "$wrapper_tmp"
        mv -f "$wrapper_tmp" "$HOME/.termloop/bin/termloop"
        """
    }
    ...
    static func remoteRelayMetadataInstallScript(
        ...
        return """
        umask 077
        mkdir -p "$HOME/.termloop" "$HOME/.termloop/relay"
        chmod 700 "$HOME/.termloop/relay"
        \(remoteCLIWrapperInstallScript(daemonRemotePath: trimmedRemotePath))
        printf '%s' "$HOME/\(trimmedRemotePath)" > "$HOME/.termloop/relay/\(relayPort).daemon_path"
        cat > "$HOME/.termloop/relay/\(relayPort).auth" <<'CMUXRELAYAUTH'
        \(authPayload)
        CMUXRELAYAUTH
        chmod 600 "$HOME/.termloop/relay/\(relayPort).auth"
        printf '%s' '127.0.0.1:\(relayPort)' > "$HOME/.termloop/socket_addr"
        """
    }
    ...
    private static func remoteDaemonPath(version: String, goOS: String, goArch: String) -> String {
        ".termloop/bin/termloopd-remote/\(version)/\(goOS)-\(goArch)/termloopd-remote"
    }
    ...
    private static func isOrphanedCMUXRemoteSSHCommand(
        ...
        if trimmed.contains("termloopd-remote") && trimmed.contains(" serve --stdio") {
            return true
        }
        return false
    }
    ...
    private static func findRepoRoot() -> URL? {
        ...
        if let envRoot = environment["TERMLOOP_REMOTE_DAEMON_SOURCE_ROOT"],
        ...
        if let envRoot = environment["TERMLOOP_REPO_ROOT"],
           !envRoot.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            candidates.append(URL(fileURLWithPath: envRoot, isDirectory: true))
        }
    ...
    private static func remotePortScanScript(ttyNames: [String], excluding ports: Set<Int>) -> String {
        ...
        if [ "$cmux_used_ss" -eq 0 ] && command -v lsof >/dev/null 2>&1 && [ -n "$cmux_tty_csv" ]; then
          cmux_tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t termloop-ports)"
...
struct SidebarGitChangeItem: Equatable {
    let path: String
    let status: GitFileStatus
}

struct SidebarGitChangesState: Equatable {
...
private func normalizedSidebarPullRequestStatusDetail(_ statusDetail: String?) -> String? {
    guard let statusDetail else { return nil }
...
struct SidebarPullRequestState: Equatable {
    ...
    init(
        ...
        self.url = url
        self.status = status
        self.statusDetail = normalizedSidebarPullRequestStatusDetail(statusDetail)
        self.branch = normalizedSidebarBranchName(branch)
        self.baseBranch = normalizedSidebarBranchName(baseBranch)
...
enum SidebarBranchOrdering {
    ...
    static func orderedUniquePullRequests(
        ...
        for panelId in orderedPanelIds {
            guard let states = panelPullRequests[panelId] else { continue }
            for state in states {
...
final class Workspace: Identifiable, ObservableObject {
    struct SidebarBranchDirectorySummaryCacheEntry {
        let structureRevision: Int
        let summaryRevision: Int
        let value: SidebarBranchDirectorySummary
    }

    let id: UUID
    var termLoopRestoreId: UUID
    ...
    @Published var currentDirectory: String {
        didSet { invalidateSidebarSummaryCache() }
    }
    private(set) var preferredBrowserProfileID: UUID?

    /// Ordinal for TERMLOOP_PORT range assignment (monotonically increasing per app session)
    ...
    @Published var panelGitChanges: [UUID: SidebarGitChangesState] = [:]
    ...
    private static let sshControlMasterCleanupQueue = DispatchQueue(
        label: "com.termloop.remote-ssh.control-master-cleanup",
        qos: .utility
    )
    ...
    var sidebarSummaryRevision: Int = 0
    var cachedSidebarOrderedPanelIds: (structureRevision: Int, ids: [UUID])?
    var sidebarBranchDirectorySummaryCacheEntry: SidebarBranchDirectorySummaryCacheEntry?
    var gitChangesRevision: Int = 0
    var cachedAggregatedGitChangeCount: (structureRevision: Int, gitChangesRevision: Int, count: Int)?
    var cachedAggregatedGitChanges: (structureRevision: Int, gitChangesRevision: Int, value: SidebarGitChangesState?)?

    private func invalidateSidebarSummaryCache() {
        sidebarSummaryRevision &+= 1
        sidebarBranchDirectorySummaryCacheEntry = nil
    }

    private func invalidateGitChangesAggregationCache() {
        gitChangesRevision &+= 1
        cachedAggregatedGitChangeCount = nil
        cachedAggregatedGitChanges = nil
    }
    ...
    lazy var sidebarObservationPublisher: AnyPublisher<Void, Never> = {
        let publishers: [AnyPublisher<Void, Never>] = [
            ...
            sidebarObservationSignal($panelGitChanges),
    ...
    lazy var sidebarPullRequestObservationPublisher: AnyPublisher<Void, Never> = {
        let publishers: [AnyPublisher<Void, Never>] = [
            sidebarObservationSignal($pullRequest),
            sidebarObservationSignal($panelPullRequests),
            sidebarObservationSignal($panelGitBranches),
    ...
    lazy var sidebarGitChangesObservationPublisher: AnyPublisher<Void, Never> = {
        let publishers: [AnyPublisher<Void, Never>] = [
            sidebarObservationSignal($panelGitChanges),
            $panels
                .map(SidebarPanelObservationState.init)
    ...
    func applyTermLoopRestoreId(_ restoreId: UUID?) {
        guard let restoreId else { return }
        termLoopRestoreId = restoreId
    }
    ...
    /// Lead PR for a panel — the first entry of its producer-sorted list.
    /// For single-PR surfaces (focused pill, CLI `pr=...`, command hints).
    func primaryPanelPullRequest(panelId: UUID) -> SidebarPullRequestState? {
        panelPullRequests[panelId]?.first
    }
    ...
    func clearPanelGitBranch(panelId: UUID) {
        ...
        if panelGitChanges[panelId] != nil {
            panelGitChanges.removeValue(forKey: panelId)
            invalidateGitChangesAggregationCache()
        }
        if panelPullRequests[panelId] != nil {
            panelPullRequests.removeValue(forKey: panelId)
        }
    ...
    func updatePanelGitChanges(panelId: UUID, files: [SidebarGitChangeItem]) {
        guard panels[panelId] != nil else { return }
        let normalized = SidebarGitChangesState(
            files: files.sorted {
        ...
        if panelGitChanges[panelId] != normalized {
            panelGitChanges[panelId] = normalized
            invalidateGitChangesAggregationCache()
        }
    }

    func clearPanelGitChanges(panelId: UUID) {
        if panelGitChanges[panelId] != nil {
            panelGitChanges.removeValue(forKey: panelId)
            invalidateGitChangesAggregationCache()
        }
    ...
    func aggregatedGitChanges() -> SidebarGitChangesState? {
        guard !panelGitChanges.isEmpty else { return nil }
        let structureRevision = bonsplitController.sidebarStructureRevision
        if let cachedAggregatedGitChanges,
           cachedAggregatedGitChanges.structureRevision == structureRevision,
           cachedAggregatedGitChanges.gitChangesRevision == gitChangesRevision {
            return cachedAggregatedGitChanges.value
        }
        let orderedPanelIds = sidebarOrderedPanelIds()
        var mergedByPath: [String: SidebarGitChangeItem] = [:]
        for panelId in orderedPanelIds {
            guard let changes = panelGitChanges[panelId] else { continue }
            for file in changes.files {
                mergedByPath[file.path] = file
            }
        }
        let fallbackPanelIds = panelGitChanges.keys.sorted { $0.uuidString < $1.uuidString }
        for panelId in fallbackPanelIds where !orderedPanelIds.contains(panelId) {
            guard let changes = panelGitChanges[panelId] else { continue }
            for file in changes.files {
                mergedByPath[file.path] = file
            }
        }
        let merged = mergedByPath.values.sorted {
            $0.path.localizedStandardCompare($1.path) == .orderedAscending
        }
        let result: SidebarGitChangesState? = merged.isEmpty
            ? nil
            : SidebarGitChangesState(files: merged)
        cachedAggregatedGitChanges = (
            structureRevision: structureRevision,
            gitChangesRevision: gitChangesRevision,
            value: result
        )
        return result
    }

    func aggregatedGitChangeCount() -> Int {
        guard !panelGitChanges.isEmpty else { return 0 }
        let structureRevision = bonsplitController.sidebarStructureRevision
        if let cachedAggregatedGitChangeCount,
           cachedAggregatedGitChangeCount.structureRevision == structureRevision,
           cachedAggregatedGitChangeCount.gitChangesRevision == gitChangesRevision {
            return cachedAggregatedGitChangeCount.count
        }
        let orderedPanelIds = sidebarOrderedPanelIds()
        ...
        for panelId in panelGitChanges.keys where !orderedPanelIds.contains(panelId) {
            guard let changes = panelGitChanges[panelId] else { continue }
            for file in changes.files {
                seenPaths.insert(file.path)
            }
        }
        let count = seenPaths.count
        cachedAggregatedGitChangeCount = (
            structureRevision: structureRevision,
            gitChangesRevision: gitChangesRevision,
            count: count
        )
    ...
    /// One-at-a-time API for socket `report_pr`, command-hint reconcile, and
    /// transient-failure restamp. Multi-PR producers use `setPanelPullRequests`.
    func updatePanelPullRequest(
        ...
        let existing = primaryPanelPullRequest(panelId: panelId)
        let normalizedStatusDetail = normalizedSidebarPullRequestStatusDetail(statusDetail)
        let normalizedBranch = normalizedSidebarBranchName(branch)
        let normalizedBaseBranch = normalizedSidebarBranchName(baseBranch)
        let currentPanelBranch = normalizedSidebarBranchName(panelGitBranches[panelId]?.branch)
        let existingMatchesReview = existing.map {
            $0.number == number
                && $0.label == label
                && $0.url == url
                && $0.status == status
        ...
        let resolvedBranch: String? = {
            ...
            if let currentPanelBranch { return currentPanelBranch }
            return existingMatchesReview ? existing?.branch : nil
        }()
        let resolvedBaseBranch: String? = {
            if let normalizedBaseBranch {
                return normalizedBaseBranch
            }
            return existingMatchesReview ? existing?.baseBranch : nil
        }()
        let resolvedStatusDetail: String? = {
            if let normalizedStatusDetail {
                return normalizedStatusDetail
            }
            return existingMatchesReview ? existing?.statusDetail : nil
        }()
        ...
        setPanelPullRequests(panelId: panelId, [state])
    }

    /// Replace the panel's PR list. Producers must pass the list lead-first
    /// (open > merged > closed, then `updatedAt` desc, then number desc) —
    /// `primaryPanelPullRequest` and the focused pill rely on `.first` being
    /// the lead. Empty array clears.
    func setPanelPullRequests(panelId: UUID, _ pullRequests: [SidebarPullRequestState]) {
        if pullRequests.isEmpty {
            clearPanelPullRequest(panelId: panelId)
            return
        }
        let existing = panelPullRequests[panelId] ?? []
        if existing != pullRequests {
            panelPullRequests[panelId] = pullRequests
        }
        let lead = pullRequests.first
        if panelId == focusedPanelId, pullRequest != lead {
            pullRequest = lead
        }
    }
    ...
    func resetSidebarContext(reason: String = "unspecified") {
        ...
        if !panelGitChanges.isEmpty {
            panelGitChanges.removeAll()
            invalidateGitChangesAggregationCache()
        }
        pullRequest = nil
    ...
    func pruneSurfaceMetadata(validSurfaceIds: Set<UUID>) {
        ...
        let filteredGitChanges = panelGitChanges.filter { validSurfaceIds.contains($0.key) }
        if filteredGitChanges != panelGitChanges {
            panelGitChanges = filteredGitChanges
            invalidateGitChangesAggregationCache()
        }
    ...
    func sidebarOrderedPanelIds() -> [UUID] {
        ...
        if let cachedSidebarOrderedPanelIds,
           cachedSidebarOrderedPanelIds.structureRevision == structureRevision {
            return cachedSidebarOrderedPanelIds.ids
        }
        ...
        cachedSidebarOrderedPanelIds = (structureRevision: structureRevision, ids: ids)
        return ids
    }
    ...
    func sidebarPullRequestsInDisplayOrder(orderedPanelIds: [UUID]) -> [SidebarPullRequestState] {
        // Drop any panel whose lead PR's branch no longer matches the panel's
        // tracked git branch (stale state from a prior checkout).
        var validPanelPullRequests: [UUID: [SidebarPullRequestState]] = [:]
        for (panelId, states) in panelPullRequests {
            guard let lead = states.first else { continue }
            if let pullRequestBranch = normalizedSidebarBranchName(lead.branch),
               normalizedSidebarBranchName(panelGitBranches[panelId]?.branch) != pullRequestBranch {
                continue
            }
            validPanelPullRequests[panelId] = states
        }
    ...
    func newTerminalSurface(
        ...
        let inheritedConfig: CmuxSurfaceConfigTemplate? = {
            let config = inheritedTerminalConfig(inPane: paneId)
            guard workingDirectory != nil else { return config }
            return config?.sanitizedForFreshShell()
        }()
        let remoteTerminalStartupCommand = remoteTerminalStartupCommand()
        ...
        // MARK: termloop-hook
        TermLoopHooks.seedSpawnPanelDirectory(workspace: self, panelId: newPanel.id, directory: workingDirectory)
        // MARK: /termloop-hook
    ...
    func newTerminalSurfaceInFocusedPane(focus: Bool? = nil) -> TerminalPanel? {
        ...
        // MARK: termloop-hook
        return newTerminalSurface(inPane: focusedPaneId, focus: focus, workingDirectory: termLoopNewTabCwd)
        // MARK: /termloop-hook
    }
    ...
    private func reconcileFocusState() {
        ...
        pullRequest = primaryPanelPullRequest(panelId: targetPanelId)
    }
    ...
    private func promptMovePanel(tabId: TabID) {
        ...
        if !moved {
            ...
            failure.informativeText = String(localized: "alert.moveTab.failed.message", defaultValue: "TermLoop could not move this tab to the selected destination.")
...
extension Workspace: BonsplitDelegate {
    ...
    private func applyTabSelectionNow(
        ...
        pullRequest = primaryPanelPullRequest(panelId: panelId)

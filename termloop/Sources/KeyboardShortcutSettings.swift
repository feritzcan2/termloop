...
enum KeyboardShortcutSettings {
    static let didChangeNotification = Notification.Name("termloop.keyboardShortcutSettingsDidChange")
    static let actionUserInfoKey = "action"
    static let settingsFileDisplayPath = "~/.config/termloop/settings.json"
    ...
    enum Action: String, CaseIterable, Identifiable {
        ...
        var label: String {
            ...
            case .quit: return String(localized: "menu.quitCmux", defaultValue: "Quit TermLoop")
...
final class SystemWideHotkeyController {
    ...
    private func toggleApplicationVisibility() {
        // Only treat the hotkey as a "hide" toggle when termloop itself is the
        // frontmost app and has at least one visible window. If the user
        // pressed the hotkey from another app, termloop is not frontmost (even if
        // some of its windows are still on screen) and the expected behavior
        // is to bring termloop forward, not hide it.
...
private enum KeyboardShortcutRecorderActivity {
    static let didChangeNotification = Notification.Name("termloop.keyboardShortcutRecorderActivityDidChange")

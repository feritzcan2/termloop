...
struct cmuxApp: App {
    ...
    init() {
        ...
        // MARK: termloop-hook
        TermLoopHooks.prepareSharedTaggedBuildState()
        // MARK: /termloop-hook
        // MARK: termloop-hook
        TermLoopHooks.migrateSidebarWorkSubTabDefaultIfNeeded()
        // MARK: /termloop-hook
        ...
        // MARK: termloop-hook
        TermLoopHooks.migrateAppearanceDarkDefaultIfNeeded()
        // MARK: /termloop-hook
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged TermLoop DEV; start with ./scripts/reload.sh --tag <name> (or set TERMLOOP_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    private static func configureGhosttyEnvironment() {
        configureManagedCmuxEnvironment()
        ...
        let existingResourcesDir = getenv("GHOSTTY_RESOURCES_DIR").map { String(cString: $0) }

        func isUsableGhosttyResourcesDir(_ path: String?) -> Bool {
            guard let path, !path.isEmpty else { return false }
            let base = URL(fileURLWithPath: path)
            return fileManager.fileExists(atPath: base.path)
                && fileManager.fileExists(atPath: base.appendingPathComponent("themes").path)
                && fileManager.fileExists(atPath: base.appendingPathComponent("shell-integration/zsh/ghostty-integration").path)
        }

        if !isUsableGhosttyResourcesDir(existingResourcesDir) {
            if let bundledGhosttyURL,
               isUsableGhosttyResourcesDir(bundledGhosttyURL.path) {
                resolvedResourcesDir = bundledGhosttyURL.path
            } else if isUsableGhosttyResourcesDir(ghosttyAppResources) {
    ...
    private static func configureManagedCmuxEnvironment(
        ...
        setenv("TERMLOOP_TAG", tag, 1)
        setenv("TERMLOOP_DEBUG_LOG", "/tmp/termloop-debug-\(tag).log", 1)
        setenv("TERMLOOP_DEBUG_BG_LOG", "/tmp/termloop-debug-\(tag)-bg.log", 1)
    }

    private static func managedCmuxTag(
        socketPath: String,
        bundleIdentifier: String?,
        environment: [String: String]
    ) -> String? {
        if let taggedSocketPath = SocketControlSettings.taggedDebugSocketPath(
            bundleIdentifier: bundleIdentifier,
            environment: environment
        ) {
            return cmuxTag(fromSocketPath: taggedSocketPath)
        }

        return cmuxTag(fromSocketPath: socketPath)
    }

    private static func cmuxTag(fromSocketPath socketPath: String) -> String? {
        let socketBase = URL(fileURLWithPath: socketPath)
            .deletingPathExtension()
            .lastPathComponent
        guard socketBase.hasPrefix("termloop-debug-") else {
            return nil
        }

        let tag = String(socketBase.dropFirst("termloop-debug-".count))
    ...
    var body: some Scene {
        WindowGroup {
            ContentView(updateViewModel: appDelegate.updateViewModel, windowId: primaryWindowId)
                ...
                .onAppear {
#if DEBUG
                    if ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_MODE"] == "1" {
                    ...
                    applyAppearance()
                    if ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_SHOW_SETTINGS"] == "1" {
                ...
                // MARK: termloop-hook
                .background(TermLoopTCPBridgeCoordinator())
                // MARK: /termloop-hook
#if DEBUG
                // MARK: termloop-hook
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                // MARK: termloop-hook
                Button(String(localized: "termloop.settings.menu.label", defaultValue: "TermLoop Settings…", table: "TermLoop")) { TermLoopHooks.openTermLoopSettingsWindow() }.keyboardShortcut(",", modifiers: [.command, .option])
                // MARK: /termloop-hook
            ...
            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About TermLoop")) {
                    showAboutPanel()
                }
            ...
            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitCmux", defaultValue: "Quit TermLoop"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
            ...
            CommandMenu("Debug") {
                ...
                Menu("Debug Windows") {
                    ...
                    // MARK: termloop-hook
                    Button(String(localized: "termloop.debugEventLog.menu.label", defaultValue: "Debug Event Log…", table: "TermLoop")) { TermLoopHooks.openDebugEventLogWindow() }
                    // MARK: /termloop-hook
                ...
                // MARK: termloop-hook
                TermLoopHooks.debugTitlebarControlsStyleMenu(selection: $titlebarControlsStyle)
                // MARK: /termloop-hook
                ...
                // MARK: termloop-hook
                Button(String(localized: "termloop.deleteAllData.menu.label", defaultValue: "Delete All Data (This Build)…", table: "TermLoop")) { TermLoopHooks.promptDeleteAllBuildData() }
                // MARK: /termloop-hook
            }
#endif
...
private let cmuxAuxiliaryWindowIdentifiers: Set<String> = [
    ...
    "cmux.about",
    "cmux.licenses",
    "termloop.browser-popup",
    "termloop.settingsAboutTitlebarDebug",
...
private final class SettingsAboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("termloop.settingsAboutTitlebarDebug")
        window.center()
    ...
    private func editor(for kind: SettingsAboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, TermLoop uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("termloop.settings")
        window.center()
...
enum SettingsNavigationRequest {
    static let notificationName = Notification.Name("termloop.settings.navigate")
...
private struct AboutPanelView: View {
    ...
    private let githubURL = URL(string: "https://github.com/feritzcan2/termloop")
    private let docsURL = URL(string: "https://termloop.ai/docs")

    private var version: String? { Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String }
    private var build: String? { Bundle.main.infoDictionary?["CFBundleVersion"] as? String }
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["TERMLOOPCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["TERMLOOP_UPSTREAM_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "TermLoop"))
                        .bold()
                        .font(.title)
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/feritzcan2/termloop/commit/\(hash)")
                    }
    ...
    private func updateWindowGlassTint() {
        let window: NSWindow? = {
            if let key = NSApp.keyWindow,
               ...
               raw == "termloop.main" || raw.hasPrefix("termloop.main.") {
                return key
            }
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("com.termloop.appIconDidChange")
...
enum SentrySettings {
    static let infoDictionaryDSNKey = "TermLoopSentryDSN"
    static let environmentDSNKeys = ["TERMLOOP_SENTRY_DSN", "SENTRY_DSN"]

    static var enabledForCurrentLaunch: Bool {
        TelemetrySettings.enabledForCurrentLaunch && configuredDSN() != nil
    }
    ...
    static func configuredDSN(
        ...
        for key in environmentDSNKeys {
            if let dsn = normalizedDSN(environment[key]) {
                return dsn
            }
        }

        if let rawValue = infoDictionary?[infoDictionaryDSNKey] as? String,
...
enum PreferredEditorSettings {
    ...
    static func open(_ url: URL) {
        if CmuxUITestCapture.appendLineIfConfigured(
            envKey: "TERMLOOP_UI_TEST_CAPTURE_OPEN_PATH",
            line: url.path
        ) {
            return
        }
...
enum CmuxRuntimeDebugCapture {
    ...
    private static let configuration: Configuration? = {
        let env = ProcessInfo.processInfo.environment
        guard let baseURLString = env["TERMLOOP_RUNTIME_DEBUG_BASE_URL"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              let baseURL = URL(string: baseURLString),
              let token = env["TERMLOOP_RUNTIME_DEBUG_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !token.isEmpty,
              let sessionID = env["TERMLOOP_RUNTIME_DEBUG_SESSION_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
struct SettingsView: View {
    ...
    private let shortcutChordsDocsURL = URL(string: "https://termloop.ai/docs/keyboard-shortcuts#shortcut-chords")!
    ...
    private var paneFirstClickFocusSubtitle: String {
        if paneFirstClickFocusEnabled {
            return String(
                ...
                defaultValue: "When TermLoop is inactive, clicking a pane activates the window and focuses that pane in one click."
            )
        }
        return String(
            localized: "settings.app.paneFirstClickFocus.subtitleOff",
            defaultValue: "When TermLoop is inactive, the first click only activates the window. Click again to focus the pane."
        )
    }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: appLanguage != LanguageSettings.languageAtLaunch.rawValue
                                ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart TermLoop to apply")
                                : nil,
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep TermLoop in the menu bar for unread notifications and quick actions.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when TermLoop highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: "Run a shell command when a notification arrives. $TERMLOOP_NOTIFICATION_TITLE, $TERMLOOP_NOTIFICATION_SUBTITLE, $TERMLOOP_NOTIFICATION_BODY are set."
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: sendAnonymousTelemetry != telemetryValueAtLaunch
                                ...
                                : String(
                                    localized: "settings.app.telemetry.subtitle",
                                    defaultValue: "Share anonymized crash reports only. File paths, document names, query strings, cookies, headers, screenshots, and view hierarchies are stripped before upload."
                                )
                        ) {
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: TERMLOOP_SOCKET_ENABLE, TERMLOOP_SOCKET_MODE, and TERMLOOP_SOCKET_PATH (set TERMLOOP_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without TermLoop integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, TermLoop wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: cursorHooksEnabled
                                ...
                                : String(localized: "settings.automation.cursor.subtitleOff", defaultValue: "Cursor runs without TermLoop integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.cursor.note", defaultValue: "Hooks must be installed with the `cmux cursor install-hooks` command. They no-op outside TermLoop terminals."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: geminiHooksEnabled
                                ...
                                : String(localized: "settings.automation.gemini.subtitleOff", defaultValue: "Gemini runs without TermLoop integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.gemini.note", defaultValue: "TermLoop auto-installs Gemini hooks on launch. They no-op outside TermLoop terminals."))
                    }

                    SettingsCard {
                        SettingsCardRow(configurationReview: .json("automation.portBase"), String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for TERMLOOP_PORT env var."), controlWidth: pickerColumnWidth) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets TERMLOOP_PORT and TERMLOOP_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        ...
                        if openTerminalLinksInCmuxBrowser || interceptTerminalOpenCommandInCmuxBrowser {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    ...
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in TermLoop. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in TermLoop.")
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in TermLoop without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                                .font(.caption)
                                .foregroundStyle(.secondary)
    ...
    private var enableSubtitle: String {
        if isEnabled {
            return String(
                ...
                defaultValue: "Press the shortcut from any app to show or hide all TermLoop windows."
            )
        }
        return String(
            localized: "settings.globalHotkey.enable.subtitleOff",
            defaultValue: "Turn this on to show or hide all TermLoop windows from any app."
        )
    }

...
final class PostHogAnalytics {
    ...
    // This fork intentionally ships without a PostHog project key.
    private let apiKey = ""

    // No PostHog host is configured in this fork.
    private let host = ""
    ...
    private init() {
        workQueue = DispatchQueue(label: "com.termloop.posthog.analytics", qos: .utility)
    ...
    private var isEnabled: Bool {
        ...
        return ProcessInfo.processInfo.environment["TERMLOOP_POSTHOG_ENABLE"] == "1"
#else
...
#if DEBUG
        config.debug = ProcessInfo.processInfo.environment["TERMLOOP_POSTHOG_DEBUG"] == "1"
#endif

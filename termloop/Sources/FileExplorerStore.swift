...
final class FileExplorerDirectoryWatcher {
    ...
    private let watchQueue = DispatchQueue(label: "com.termloop.fileExplorerWatcher", qos: .utility)
...
struct GitStatusSummaryFile: Equatable {
...
struct GitStatusSummary: Equatable {
    let repoRoot: String
    let files: [GitStatusSummaryFile]

    func statusMap(explorerRoot: String) -> [String: GitFileStatus] {
        GitStatusProvider.statusMap(from: files, explorerRoot: explorerRoot)
    }
}
...
enum GitStatusProvider {
    ...
    static func fetchStatus(directory: String) -> [String: GitFileStatus] {
        fetchSummary(directory: directory)?.statusMap(explorerRoot: directory) ?? [:]
    }
    ...
    static func fetchStatusSSH(
        ...
        )?.statusMap(explorerRoot: directory) ?? [:]
    }

    static func fetchSummary(directory: String) -> GitStatusSummary? {
        guard let repoRoot = gitRepoRoot(for: directory) else { return nil }
    ...
    static func fetchUnifiedDiff(
        ...
        for arguments in [
            ...
            if let output = runGit(in: repoRoot, arguments: arguments)?
        ...
        case .added, .untracked:
            return synthesizeAdditionPatch(repoRoot: repoRoot, relativePath: relativePath)
        case .deleted:
    ...
    static func fetchSummarySSH(
        ...
        return parseGitStatus(output: parts[1], repoRoot: repoRoot)
    }

    private static func parseGitStatus(
        output: String?, repoRoot: String
    ) -> GitStatusSummary {
        guard let output, !output.isEmpty else {
            return GitStatusSummary(repoRoot: repoRoot, files: [])
        }
        var files: [GitStatusSummaryFile] = []
        ...
        return GitStatusSummary(repoRoot: repoRoot, files: files)
    }
    ...
    fileprivate static func statusMap(
        from files: [GitStatusSummaryFile],
        explorerRoot: String
    ) -> [String: GitFileStatus] {
        var statusMap: [String: GitFileStatus] = [:]
        for file in files {
            guard file.absolutePath.hasPrefix(explorerRoot) else { continue }
            statusMap[file.absolutePath] = file.status
    ...
    private static func runGit(in directory: String, arguments: [String]) -> String? {
        // MARK: termloop-hook
        return GitCommandRunner.runOptional(arguments, in: directory, kind: GitCommandRunner.CommandKind.classify(arguments: arguments), caller: "GitStatusProvider")
        // MARK: /termloop-hook
    }
    ...
    private static func synthesizeAdditionPatch(
        ...
        let fileURL = URL(fileURLWithPath: repoRoot).appendingPathComponent(relativePath)
        guard let data = try? Data(contentsOf: fileURL) else { return nil }
        guard let contents = String(data: data, encoding: .utf8) else {
            return "Binary or non-UTF8 file added: \(relativePath)"
        }

...
enum InternalTabDragConfigurationProvider {
    // These drags only make sense inside TermLoop. Outside the app, Finder should
...
final class FileDropOverlayView: NSView {
    ...
    override func draggingExited(_ sender: (any NSDraggingInfo)?) {
        ...
        // MARK: termloop-hook
        termLoopResetForwardingOnExit(sender: sender)
        // MARK: /termloop-hook
    ...
    override func prepareForDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        // MARK: termloop-hook
        if let accepted = termLoopTryPrepareForwardedDrop(sender: sender, shouldCapture: shouldCapture) {
            preparedDragWebView = nil
            return accepted
        }
        // MARK: /termloop-hook
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        // MARK: termloop-hook
        if let result = termLoopTryPerformForwardedDrop(sender: sender, shouldCapture: shouldCapture) {
            preparedDragWebView = nil
            activeDragWebView = nil
            return result
        }
    ...
    override func concludeDragOperation(_ sender: (any NSDraggingInfo)?) {
        ...
        // MARK: termloop-hook
        termLoopConcludeForwardedDrop(sender: sender)
        // MARK: /termloop-hook
    ...
    private func updateDragTarget(_ sender: any NSDraggingInfo, phase: String) -> NSDragOperation {
        ...
        // MARK: termloop-hook
        if let op = termLoopTryForwardDragUpdate(sender: sender, shouldCapture: shouldCapture) {
            if let prev = activeDragWebView { prev.draggingExited(sender); activeDragWebView = nil }
            return op
        }
        // MARK: /termloop-hook
...
let commandPaletteOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("termloop.commandPalette.overlay.container")
let tmuxWorkspacePaneOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("termloop.tmuxWorkspacePane.overlay.container")
...
struct ContentView: View {
    ...
    private func startSidebarResizerCursorStabilizer() {
        ...
        timer.setEventHandler {
            guard isResizerBandActive || isResizerDragging else {
                stopSidebarResizerCursorStabilizer()
                return
            }
    ...
    private var terminalContent: some View {
        ...
        return ZStack {
            ZStack {
                ForEach(mountedWorkspaces) { tab in
                    ...
                    // MARK: termloop-hook
                    .overlay(alignment: .bottom) { TermLoopHooks.workspaceFooterExtras(workspace: tab) }
                    // MARK: /termloop-hook
                }
            }
        ...
        .overlay(alignment: .top) {
            if !isMinimalMode, !TermLoopHooks.shouldHideMainAreaTitlebarOverlay() {
    ...
    private var terminalContentWithSidebarDropOverlay: some View {
        ...
        return HStack(spacing: 0) {
            // MARK: termloop-hook
            TermLoopHooks.mainAreaOverlaySwap {
                terminalContent
    ...
    private var windowIdentifier: String { "termloop.main.\(windowId.uuidString)" }
    ...
    private func setTitlebarControlsHidden(_ hidden: Bool, in window: NSWindow) {
        let controlsId = NSUserInterfaceItemIdentifier("termloop.titlebarControls")
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                title: constant(String(localized: "command.installCLI.title", defaultValue: "Shell Command: Install TermLoop CLI in PATH")),
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                title: constant(String(localized: "command.uninstallCLI.title", defaultValue: "Shell Command: Uninstall TermLoop CLI from PATH")),
        ...
        contributions.append(
            CommandPaletteCommandContribution(
                ...
                keywords: ["restart", "socket", "listener", "cli", "termloop", "control"]
            )
        )
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        registry.register(commandId: "palette.newWorkspace") {
            // MARK: termloop-hook
            TermLoopHooks.handleNewWorkspacePaletteCommand(tabManager: tabManager)
            // MARK: /termloop-hook
        }
    ...
    private func applyUITestSidebarSelectionIfNeeded(tabs: [Workspace]) {
        ...
        guard let rawValue = env["TERMLOOP_UI_TEST_SIDEBAR_SELECTED_WORKSPACE_INDICES"]?
...
struct VerticalTabsSidebar: View {
    ...
    var body: some View {
        ...
        TermLoopSidebar.Root(
            ...
            frozenTabItemPresentation: $frozenTabItemPresentation,
            modifierKeyMonitor: modifierKeyMonitor,
            dragAutoScrollController: dragAutoScrollController,
            dragFailsafeMonitor: dragFailsafeMonitor,
            tabItemSettingsStore: tabItemSettingsStore,
            ...
            hiddenTitlebarControlsLeadingInset: hiddenTitlebarControlsLeadingInset,
...
private enum FeedbackComposerSettings {
    ...
    static func supportEmail(
        ...
        configuredValue(
            environmentKey: emailEnvironmentKey,
            infoDictionaryKey: emailInfoDictionaryKey,
            environment: environment,
            infoDictionary: infoDictionary
        ) ?? defaultEmail
    }
    ...
    private static func configuredValue(
        ...
        if let override = normalizedValue(environment[environmentKey]) {
            return override
        }
        if let configuredValue = normalizedValue(infoDictionary?[infoDictionaryKey] as? String) {
...
private struct FeedbackComposerAppMetadata {
    ...
    static var current: FeedbackComposerAppMetadata {
        ...
        let env = ProcessInfo.processInfo.environment
        let commit = (infoDictionary["TERMLOOPCommit"] as? String).flatMap { value in
            value.isEmpty ? nil : value
        } ?? env["TERMLOOP_UPSTREAM_COMMIT"]
...
enum SidebarDragLifecycleNotification {
    static let stateDidChange = Notification.Name("termloop.sidebarDragStateDidChange")
    static let requestClear = Notification.Name("termloop.sidebarDragRequestClear")
...
private struct SidebarHelpMenuButton: View {
    ...
    private let changelogURL = URL(string: "https://termloop.ai/docs/changelog")
    private let githubURL = URL(string: "https://github.com/feritzcan2/termloop")
    private let githubIssuesURL = URL(string: "https://github.com/feritzcan2/termloop/issues")
    private let discordURL = URL(string: "https://github.com/feritzcan2/termloop/discussions")
    ...
    private var helpPopover: some View {
        VStack(alignment: .leading, spacing: 2) {
            helpOptionButton(
                title: String(localized: "sidebar.help.welcome", defaultValue: "Welcome to TermLoop!"),
                action: .welcome,
...
struct TabItemView: View, Equatable {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 8) {
                ...
                // MARK: termloop-hook
                TermLoopSidebar.WorktreeBadge(workspace: tab)
                // MARK: /termloop-hook
                ...
                ZStack(alignment: .trailing) {
                    // MARK: termloop-hook
                    TermLoopHooks.sidebarCloseButton(tab: tab, tabManager: tabManager, isEnabled: showCloseButton && !showsWorkspaceShortcutHint, foregroundColor: activeSecondaryColor(0.7), tooltip: closeButtonTooltip)
                    // MARK: /termloop-hook
            ...
            // MARK: termloop-hook
            TermLoopHooks.sidebarWorkspaceDescription(markdown: tab.customDescription, isActive: usesInvertedActiveForeground)
            // MARK: /termloop-hook
            ...
            // MARK: termloop-hook
            TermLoopHooks.sidebarBranchDirectoryRow(tab: tab, isActive: isActive, settings: settings)
            // MARK: /termloop-hook
                        ...
                        Button(action: {
                            ...
                            HStack(spacing: 4) {
                                ...
                                Text("\(pullRequest.label) #\(pullRequest.number)")
                                    ...
                                    .lineLimit(1)
                                    .truncationMode(.tail)
                                Text(pullRequest.displayStatus)
                                    .lineLimit(1)
                                Spacer(minLength: 0)
                            }
    ...
    private var rowHeightReader: some View {
        GeometryReader { proxy in
            Color.clear
                .onAppear { rowHeight = max(proxy.size.height, 1) }
    ...
    private var workspaceContextMenu: some View {
        ...
        // MARK: termloop-hook
        TermLoopHooks.workspaceContextMenuExtras(workspace: tab)
        // MARK: /termloop-hook

        // MARK: termloop-hook
        TermLoopHooks.workspaceForkMenu(workspace: tab, isMulti: isMulti)
        // MARK: /termloop-hook

        // MARK: termloop-hook
        TermLoopHooks.workspaceLinkToMenu(workspace: tab, isMulti: isMulti, tabManager: tabManager)
        // MARK: /termloop-hook

        // MARK: termloop-hook
        TermLoopSidebar.WorktreeMenuItems(targetIds: targetIds, tabManager: tabManager)
        // MARK: /termloop-hook

        // MARK: termloop-hook
        TermLoopSidebar.RestoreClaudeSessionMenuItem(targetIds: targetIds)
        // MARK: /termloop-hook

        // MARK: termloop-hook
        TermLoopSidebar.SelectPlanMenuItem(targetIds: targetIds)
        // MARK: /termloop-hook
        ...
        Menu(String(
            localized: "contextMenu.cmuxSubmenu",
    ...
    private func pullRequestDisplays(orderedPanelIds: [UUID]) -> [PullRequestDisplay] {
        tab.sidebarPullRequestsInDisplayOrder(orderedPanelIds: orderedPanelIds).map { pullRequest in
            PullRequestDisplay(
                ...
                url: pullRequest.url,
                status: pullRequest.status,
                displayStatus: pullRequest.displayStatus,
...
private struct SidebarMetadataEntryRow: View {
    ...
    private func rowContent(underlined: Bool) -> some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        // MARK: termloop-hook
        .modifier(TermLoopHooks.claudeRunningShimmer(entryKey: entry.key, entryValue: entry.value))
        // MARK: /termloop-hook
    }
...
private enum SidebarTabDragPayload {
    static let typeIdentifier = "com.termloop.sidebar-tab-reorder"
    static let dropContentType = UTType(exportedAs: typeIdentifier)
    static let dropContentTypes: [UTType] = [dropContentType]
    private static let prefix = "termloop.sidebar-tab."

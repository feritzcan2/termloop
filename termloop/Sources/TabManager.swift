...
class TabManager: ObservableObject {
    ...
    typealias GitHubPullRequestProbeItem = GitHostPullRequestProbeItem
    ...
    // scheduler tick burns main-thread cycles scanning every tracked workspace.
    private nonisolated static let workspacePullRequestPollTickInterval: TimeInterval = 20
    private nonisolated static let workspacePullRequestRepoCacheLifetime: TimeInterval = 90
    private nonisolated static let workspacePullRequestRepoCachePruneLifetime: TimeInterval = 5 * 60
    ...
    private nonisolated static let workspacePullRequestRepoFetchConcurrency = 2
    private nonisolated static let workspacePullRequestBranchFetchConcurrency = 3
    ...
    @Published var selectedTabId: UUID? {
        ...
        didSet {
            ...
            // MARK: termloop-hook
            TermLoopHooks.workspaceSelectionChanged(tabManager: self, selectedTabId: selectedTabId)
            // MARK: /termloop-hook
    ...
    private let initialWorkspaceGitProbeQueue = DispatchQueue(
        label: "com.termloop.initial-workspace-git-probe",
        qos: .utility
    )
    ...
    private func startWorkspaceGitMetadataPollTimer() {
        let timer = DispatchSource.makeTimerSource(queue: .global(qos: .utility))
        let interval = Self.backgroundPollInterval * 5
    ...
    private func refreshTrackedWorkspaceGitMetadata() {
        ...
        for workspace in tabs {
            ...
            guard !candidatePanelIds.isEmpty else { continue }

            let panelId = workspace.focusedPanelId.flatMap { focusedPanelId in
                candidatePanelIds.contains(focusedPanelId) ? focusedPanelId : nil
            } ?? candidatePanelIds.sorted { $0.uuidString < $1.uuidString }.first!
            ...
            for panelId in Set(workspace.panelGitBranches.keys).union(workspace.panelPullRequests.keys) {
                ...
                let branch = Self.normalizedBranchName(
                    workspace.panelGitBranches[panelId]?.branch
                        ?? workspace.primaryPanelPullRequest(panelId: panelId)?.branch
                )
                ...
                guard shouldRefreshWorkspacePullRequest(
                    key: key,
                    now: now,
                    currentPullRequest: workspace.primaryPanelPullRequest(panelId: panelId)
                ) else {
                    continue
                }
                ...
                if candidate.repoSlugs.isEmpty,
                   let directory,
                   PullRequestRepositoryIdentityCache.shared.isResolvingOrHasNonEmptyCache(directory: directory) {
                    workspacePullRequestNextPollAtByKey[key] = now.addingTimeInterval(Self.workspacePullRequestPollTickInterval)
#if DEBUG
                    dlog(
                        "workspace.prRefresh.repo.pending workspace=\(workspace.id.uuidString.prefix(5)) " +
    ...
    private func workspacePullRequestCandidate(
        ...
        // MARK: termloop-hook
        return WorkspacePullRequestCandidate(workspaceId: workspace.id, panelId: panelId, branch: branch, repoSlugs: PullRequestRepositoryIdentityCache.shared.resolveOrPopulate(directory: gitProbeDirectory(for: workspace, panelId: panelId)))
        // MARK: /termloop-hook
    }
    ...
    private func applyWorkspacePullRequestRefreshResults(
        ...
        defer {
            // Cap follow-up recursion: dirty repo + in-flight refresh can
            // keep rerunPending set forever, growing this chain depth-first
            // and freezing the main thread. Periodic timer (~10s) re-arms.
            if needsFollowUpPass,
               reason.components(separatedBy: ".followUp").count - 1 < 3 {
        ...
        for key in requestedKeys {
            ...
            let priorPullRequest = workspace.primaryPanelPullRequest(panelId: result.panelId)
            let countsAsTerminalSweep = priorPullRequest.map { $0.status != .open } ?? false

            switch result.resolution {
            case .resolved(let resolvedPullRequests):
                workspacePullRequestTransientFailureCountByKey[key] = 0
                let states = resolvedPullRequests.compactMap { resolved -> SidebarPullRequestState? in
                    guard let status = SidebarPullRequestStatus(rawValue: resolved.statusRawValue),
                ...
                if states.isEmpty {
                    ...
                    workspace.setPanelPullRequests(panelId: result.panelId, states)
                }
            case .notFound:
                workspacePullRequestTransientFailureCountByKey[key] = 0
            ...
            case .transientFailure:
                ...
                if nextFailureCount >= 3,
                   let existing = workspace.panelPullRequests[result.panelId],
                   !existing.isEmpty {
                    let stamped = existing.map { state in
                    ...
                    workspace.setPanelPullRequests(panelId: result.panelId, stamped)
                }
            }
            ...
            let label: String = {
                ...
                case .resolved(let resolvedPullRequests):
                    if resolvedPullRequests.count == 1, let one = resolvedPullRequests.first {
                        return "#\(one.number):\(one.statusRawValue)"
                    }
                    return "\(resolvedPullRequests.count)PRs:" + resolvedPullRequests
                        .map { "#\($0.number):\($0.statusRawValue)" }
                        .joined(separator: ",")
                }
            }()
            dlog(
    ...
    private func scheduleNextWorkspacePullRequestPoll(
        ...
        if case .resolved(let resolvedPullRequests) = resolution,
           !resolvedPullRequests.isEmpty,
           resolvedPullRequests.allSatisfy({
               (SidebarPullRequestStatus(rawValue: $0.statusRawValue) ?? .open) != .open
           }) {
            workspacePullRequestLastTerminalStateRefreshAtByKey[key] = now
    ...
    private func maybeMutateSelectionDuringWorkspaceCreationForDev(
        ...
        let isEnabled: Bool = {
            if let raw = env["TERMLOOP_DEV_MUTATE_WORKSPACE_SELECTION_DURING_CREATION"] {
    ...
    func addWorkspace(
        ...
        return withExtendedLifetime((capturedTabs, sourceWorkspace)) {
            ...
            // MARK: termloop-hook
            WorkspaceMetadataStore.shared.setProjectId(
...
#endif
            // Agent-bound workspaces should launch directly into the agent input flow.
            // The first-run welcome screen is useful for plain terminals, but it is noisy
            // and can interfere when creating terminal agents.
            let shouldAutoShowWelcome = terminalAgentId == nil
    ...
    private func applyWorkspaceGitMetadataSnapshot(
        ...
        let previousPullRequestBranch = workspace.primaryPanelPullRequest(panelId: probeKey.panelId)?.branch
        ...
        if let nextBranch {
            ...
            workspace.updatePanelGitChanges(
                panelId: probeKey.panelId,
                files: snapshot.changes
            )
        } else {
            workspace.clearPanelGitBranch(panelId: probeKey.panelId)
            workspace.clearPanelGitChanges(panelId: probeKey.panelId)
        }
        ...
        case .resolved(let pullRequest):
            workspace.updatePanelPullRequest(
                ...
                statusDetail: pullRequest.statusDetail,
        ...
        let normalizedPreviousBranch = previousBranch.flatMap(Self.normalizedBranchName)
        let normalizedPreviousPullRequestBranch = previousPullRequestBranch.flatMap(Self.normalizedBranchName)
        let hasPullRequestState = workspace.panelPullRequests[probeKey.panelId] != nil
        let hasPullRequestTracking =
            workspacePullRequestNextPollAtByKey[probeKey] != nil
                || workspacePullRequestProbeStateByKey[probeKey] != nil
        let shouldSkipNextPullRequest = normalizedNextBranch.map(Self.shouldSkipWorkspacePullRequestLookup(branch:)) ?? true
        let shouldRefreshPullRequest =
            !shouldSkipNextPullRequest
            && (
                normalizedPreviousBranch != normalizedNextBranch
                    || (
                        normalizedPreviousPullRequestBranch != nil
                            && normalizedPreviousPullRequestBranch != normalizedNextBranch
                    )
                    || (!hasPullRequestState && !hasPullRequestTracking)
            )

        if shouldSkipNextPullRequest {
            if hasPullRequestState {
                workspace.clearPanelPullRequest(panelId: probeKey.panelId)
            }
            if hasPullRequestTracking {
                clearWorkspacePullRequestTracking(for: probeKey)
            }
        } else if shouldRefreshPullRequest {
        ...
        dlog(
            ...
            "panel=\(probeKey.panelId.uuidString.prefix(5)) branch=\(branchLabel) dirty=\(snapshot.isDirty ? 1 : 0) changes=\(snapshot.changes.count) " +
            "pr=\(prLabel)"
        )
#endif
    }
    ...
    private nonisolated static func initialWorkspaceGitMetadataSnapshot(
        ...
        // MARK: termloop-hook
        if let snapshot = TermLoopHooks.initialWorkspaceGitMetadataSnapshot(directory: directory) { return InitialWorkspaceGitMetadataSnapshot(branch: snapshot.branch, isDirty: snapshot.isDirty, changes: snapshot.changes, pullRequest: snapshot.branch == nil ? .notFound : .deferred) }
        // MARK: /termloop-hook
        ...
        let changes = GitStatusProvider.fetchSummary(directory: directory)?.files.map { file in
            SidebarGitChangeItem(path: file.relativePath, status: file.status)
        } ?? []
        let isDirty = !changes.isEmpty
        return InitialWorkspaceGitMetadataSnapshot(
            branch: branch,
            isDirty: isDirty,
            changes: changes,
    ...
    private nonisolated static func runGitCommand(directory: String, arguments: [String]) -> String? {
        // MARK: termloop-hook
        return GitCommandRunner.runOptional(arguments, in: directory, kind: GitCommandRunner.CommandKind.classify(arguments: arguments), caller: "TabManager.gitMetadata")
        // MARK: /termloop-hook
    }
    ...
    private nonisolated static func fetchWorkspacePullRequestRepoResults(
        ...
        let authHeadersByRepoSlug: [String: String?] = Dictionary(
            uniqueKeysWithValues: repoDirectoriesBySlug.map { repoSlug, directory in
                (
                    repoSlug,
                    GitHostPullRequestProbe.authHeader(
                        repoSlug: repoSlug,
                        directory: directory
                    )
                )
        ...
        let fetchedResults = await limitedConcurrentMap(
            repoDirectoriesBySlug.keys.sorted(),
            maxConcurrentTasks: Self.workspacePullRequestRepoFetchConcurrency
        ) { repoSlug in
            let result = await Self.workspacePullRequestRepoFetchResult(
                ...
                session: session,
                authHeader: authHeadersByRepoSlug[repoSlug] ?? nil
            )
    ...
    private nonisolated static func resolveWorkspacePullRequestRefreshResults(
        ...
        candidates.map { candidate in
            ...
            var matchedPullRequests: [GitHubPullRequestProbeItem] = []
            ...
            for repoSlug in candidate.repoSlugs {
                ...
                case .success(let cacheEntry, let usedCache, let transientBranches):
                    ...
                    if let candidateMatches = cacheEntry.pullRequestsByBranch[candidate.branch],
                       !candidateMatches.isEmpty {
                        matchedPullRequests = candidateMatches
            ...
            let usedCachedRepoData: Bool
            let resolvedItems: [WorkspacePullRequestResolvedItem] = matchedPullRequests.compactMap { item in
                guard let status = PullRequestOrdering.pullRequestStatus(from: item.state) else { return nil }
                return WorkspacePullRequestResolvedItem(
                    number: item.number,
                    urlString: item.url,
            ...
            if !resolvedItems.isEmpty {
                resolution = .resolved(resolvedItems)
    ...
    private nonisolated static func workspacePullRequestRepoFetchResult(
        ...
        guard GitHostPullRequestProbe.isSupported(repoSlug: repoSlug) else {
            return .transientFailure
        }
        ...
        guard let allPullRequests = await GitHostPullRequestProbe.fetchRecent(
            repoSlug: repoSlug,
            pageLimit: Self.workspacePullRequestRepoPageLimit,
            pageSize: Self.workspacePullRequestRepoPageSize,
            session: session,
            authHeader: authHeader
        ) else {
#if DEBUG
            dlog("workspace.prRefresh.repo.fail repo=\(repoSlug)")
#endif
            return .transientFailure
        }
...
#if DEBUG
        dlog(
            "workspace.prRefresh.repo.success repo=\(repoSlug) " +
    ...
    private nonisolated static func workspacePullRequestBranchLookupOutcome(
        ...
        let branchResults = await limitedConcurrentMap(
            candidateBranches,
            maxConcurrentTasks: Self.workspacePullRequestBranchFetchConcurrency
        ) { branch in
        ...
        for (branch, result) in branchResults {
            switch result {
            case .found(let pullRequests):
                pullRequestsByBranch[branch] = pullRequests
    ...
    private nonisolated static func workspacePullRequestBranchFetchResult(
        ...
        guard let pullRequests = await GitHostPullRequestProbe.fetchBranch(
            repoSlug: repoSlug,
            branch: branch,
            pageSize: Self.workspacePullRequestRepoPageSize,
            session: session,
...
#if DEBUG
            dlog("workspace.prRefresh.branch.fail repo=\(repoSlug) branch=\(branch)")
#endif
            return .transientFailure
        }
        ...
        let sorted = PullRequestOrdering.sortedPullRequests(from: matchingPullRequests)
        if sorted.isEmpty {
            return .notFound
        }
        return .found(sorted)
    ...
    private nonisolated static func limitedConcurrentMap<Input: Sendable, Output: Sendable>(
        _ inputs: [Input],
        ...
        return await withTaskGroup(of: Output.self, returning: [Output].self) { group in
            ...
            var started = 0
            while started < limit, let input = iterator.next() {
                started += 1
                group.addTask {
    ...
    private nonisolated static func pullRequestMapByNormalizedBranch(
        ...
        var grouped: [String: [GitHubPullRequestProbeItem]] = [:]

        for pullRequest in pullRequests {
            guard let branch = normalizedBranchName(pullRequest.headRefName),
                  PullRequestOrdering.pullRequestStatus(from: pullRequest.state) != nil,
                  URL(string: pullRequest.url) != nil else {
                continue
            }
            grouped[branch, default: []].append(pullRequest)
        }

        return grouped.mapValues { PullRequestOrdering.sortedPullRequests(from: $0) }
    }

    /// Picks a single "lead" PR from a candidate list using status priority
    /// (open > merged > closed), then `updatedAt` desc, then PR number desc.
    /// Used for single-PR surfaces (focused workspace pill, CLI `pr=...`,
    /// command-hint reconciliation) and as the first entry of producer-sorted
    /// PR lists so `primaryPanelPullRequest` matches.
    nonisolated static func leadPullRequest(
        from pullRequests: [GitHubPullRequestProbeItem]
        ...
        func isPreferred(
            ...
            guard let candidateStatus = PullRequestOrdering.pullRequestStatus(from: candidate.state),
                  let currentStatus = PullRequestOrdering.pullRequestStatus(from: current.state) else {
                return false
            }
        ...
        for pullRequest in pullRequests {
            guard PullRequestOrdering.pullRequestStatus(from: pullRequest.state) != nil,
                  URL(string: pullRequest.url) != nil else {
                continue
            }
    ...
    nonisolated static func githubRepositorySlugs(fromGitRemoteVOutput output: String) -> [String] {
        // MARK: termloop-hook
        return GitRemoteParser.githubRepositorySlugs(fromRemoteVOutput: output)
        // MARK: /termloop-hook
    }
    ...
    nonisolated static func shouldSkipWorkspacePullRequestLookup(branch: String) -> Bool {
        ...
        case "main", "master", "dev", "development":
            return true
        default:
            return false
        }
    ...
    private func reconcileLocalPullRequestActionIfPossible(
        ...
        guard let currentPullRequest = workspace.primaryPanelPullRequest(panelId: panelId),
        ...
        workspace.updatePanelPullRequest(
            ...
            status: nextStatus,
            statusDetail: nextStatus.rawValue,
    ...
    func closeWorkspace(_ workspace: Workspace) {
        ...
        if let index = tabs.firstIndex(where: { $0.id == workspace.id }) {
            ...
            // MARK: termloop-hook
            TermLoopHooks.workspaceDidClose(workspaceId: workspace.id)
            // MARK: /termloop-hook
    ...
    /// Closes a workspace without prompting for confirmation.
    /// Used by destructive bulk flows that already confirmed intent.
    func closeWorkspaceWithoutConfirmation(_ workspace: Workspace) {
        closeWorkspaceIfRunningProcess(workspace, requiresConfirmation: false)
    }
    ...
    private func closeWorkspaceIfRunningProcess(_ workspace: Workspace, requiresConfirmation: Bool = true) {
        // MARK: termloop-hook
        if TermLoopHooks.workspaceWillClose(workspace: workspace, completion: { [weak self] shouldClose in if shouldClose { self?.closeWorkspaceIfRunningProcess(workspace, requiresConfirmation: requiresConfirmation) } }) { return }
        // MARK: /termloop-hook
    ...
    func closePanelAfterChildExited(tabId: UUID, surfaceId: UUID) {
        ...
        // MARK: termloop-hook
        if confirmCloseExitedLastLocalSurface(tab: tab, surfaceId: surfaceId) { return }
        // MARK: /termloop-hook
...
#if DEBUG
        if ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_FORCE_CONFIRM_CLOSE_WORKSPACE"] == "1" {
            return true
        }
#endif
    ...
    private func setupUITestFocusShortcutsIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_FOCUS_SHORTCUTS"] == "1" else { return }
    ...
    private func setupSplitCloseRightUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_SETUP"] == "1" else { return }
        guard let path = env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_PATH"], !path.isEmpty else { return }
        let visualMode = env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1"
        let shotsDir = (env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_SHOTS_DIR"] ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        let visualIterations = Int((env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_ITERATIONS"] ?? "20").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 20
        let burstFrames = Int((env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_BURST_FRAMES"] ?? "6").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 6
        let closeDelayMs = Int((env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_CLOSE_DELAY_MS"] ?? "70").trimmingCharacters(in: .whitespacesAndNewlines)) ?? 70
        let pattern = (env["TERMLOOP_UI_TEST_SPLIT_CLOSE_RIGHT_PATTERN"] ?? "close_right")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        for i in 1...iterations {
            ...
            sendText(topLeftId, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo TERMLOOP_SPLIT_TOPLEFT_\(i); done; printf '\\033[HTERMLOOP_MARKER_TOPLEFT\\n'\r")
            sendText(topRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo TERMLOOP_SPLIT_TOPRIGHT_\(i); done; printf '\\033[HTERMLOOP_MARKER_TOPRIGHT\\n'\r")
            if let bottomLeft {
                sendText(bottomLeft.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo TERMLOOP_SPLIT_BOTTOMLEFT_\(i); done; printf '\\033[HTERMLOOP_MARKER_BOTTOMLEFT\\n'\r")
            }
            if let bottomRight {
                sendText(bottomRight.id, "printf '\\033[2J\\033[H'; for i in {1..200}; do echo TERMLOOP_SPLIT_BOTTOMRIGHT_\(i); done; printf '\\033[HTERMLOOP_MARKER_BOTTOMRIGHT\\n'\r")
            }
    ...
    private func setupChildExitSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_CHILD_EXIT_SPLIT_SETUP"] == "1" else { return }
        guard let path = env["TERMLOOP_UI_TEST_CHILD_EXIT_SPLIT_PATH"], !path.isEmpty else { return }
        let requestedIterations = Int(env["TERMLOOP_UI_TEST_CHILD_EXIT_SPLIT_ITERATIONS"] ?? "1") ?? 1
    ...
    private func setupChildExitKeyboardUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1" else { return }
        guard let path = env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"], !path.isEmpty else { return }
        let autoTrigger = env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] == "1"
        let strictKeyOnly = env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] == "1"
        let triggerMode = (env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_TRIGGER_MODE"] ?? "shell_input")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        let layout = (env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] ?? "lr")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let expectedPanelsAfter = max(
            1,
            Int((env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] ?? "1")
                .trimmingCharacters(in: .whitespacesAndNewlines)
            ) ?? 1
        )
...
extension TabManager {
    ...
    func sessionSnapshot(includeScrollback: Bool) -> SessionTabManagerSnapshot {
        ...
        let workspaceSnapshots = restorableTabs
            .map {
                // MARK: termloop-hook
                TermLoopHooks.sessionSnapshot(for: $0, includeScrollback: includeScrollback)
                // MARK: /termloop-hook
            }
    ...
    func restoreSessionSnapshot(_ snapshot: SessionTabManagerSnapshot) {
        ...
        let workspaceSnapshots = snapshot.workspaces
            .prefix(SessionPersistencePolicy.maxWorkspacesPerWindow)
        // MARK: termloop-hook
        TermLoopHooks.beginTabManagerSessionRestore(tabManager: self, selectedWorkspaceIndex: snapshot.selectedWorkspaceIndex, totalWorkspaceCount: snapshot.workspaces.count)
        // MARK: /termloop-hook
        for workspaceSnapshot in workspaceSnapshots {
            ...
            workspace.owningTabManager = self
            // MARK: termloop-hook
            TermLoopHooks.restoreWorkspaceSessionSnapshot(workspace: workspace, snapshot: workspaceSnapshot, tabManager: self)
            // MARK: /termloop-hook
            wireClosedBrowserTracking(for: workspace)
        ...
        // MARK: termloop-hook
        TermLoopHooks.didRestoreWorkspaces(workspaces: newTabs)
        // MARK: /termloop-hook

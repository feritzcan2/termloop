...
private enum CmuxThemeNotifications {
    static let reloadConfig = Notification.Name("com.termloop.themes.reload-config")
}
...
enum CmuxTypingTiming {
    static let isEnabled: Bool = {
        ...
        if environment["TERMLOOP_TYPING_TIMING_LOGS"] == "1" || environment["TERMLOOP_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
    ...
    static let isVerboseProbeEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        if environment["TERMLOOP_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
...
final class VSCodeServeWebController {
    ...
    private let queue = DispatchQueue(label: "termloop.vscode.serveWeb")
    private let launchQueue = DispatchQueue(label: "termloop.vscode.serveWeb.launch")
    ...
    private static func makeConnectionTokenFile() -> URL? {
        ...
        let tokenFileName = "termloop-vscode-token-\(UUID().uuidString)"
...
struct CmuxCLIPathInstaller {
    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bundledCLIMissing(let expectedPath):
                return "Bundled termloop CLI was not found at \(expectedPath)."
            ...
            case .installVerificationFailed(let path):
                return "Installed symlink at \(path) did not point to the bundled termloop CLI."
    ...
    init(
        ...
        destinationURL: URL = URL(fileURLWithPath: "/usr/local/bin/termloop"),
    ...
    private static func defaultBundledCLIURL(bundle: Bundle = .main) -> URL? {
        bundle.resourceURL?.appendingPathComponent("bin/termloop", isDirectory: false)
    }
...
/// For browser content, let the page try the Find command family before TermLoop's menu fallback.
/// This preserves native web-app shortcuts like VS Code's Cmd+F while still allowing TermLoop's
...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    private static func detectRunningUnderXCTest(_ env: [String: String]) -> Bool {
        ...
        if env.keys.contains(where: { $0.hasPrefix("TERMLOOP_UI_TEST_") }) { return true }
        return false
    }
    ...
    private nonisolated static let persistedWindowGeometryDefaultsKey = "termloop.session.lastWindowGeometry.v2"
    private nonisolated static let legacyPersistedWindowGeometryDefaultsKeys = [
        "termloop.session.lastWindowGeometry.v1"
    ]
    ...
    private func childExitKeyboardProbePath() -> String? {
        ...
        guard env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1",
              let path = env["TERMLOOP_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    private nonisolated static let launchServicesRegistrationQueue = DispatchQueue(
        label: "com.termloop.app.launchServicesRegistration",
        qos: .utility
    )
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        // MARK: termloop-hook
        TermLoopHooks.bootstrapBlockedBin()
        // MARK: /termloop-hook
        // MARK: termloop-hook
        TermLoopHooks.installQuickActionHotkey()
        // MARK: /termloop-hook
        let env = ProcessInfo.processInfo.environment
        ...
        if telemetryEnabled, let sentryDSN = SentrySettings.configuredDSN(environment: env) {
            ...
            SentrySDK.start { options in
                ...
                options.maxBreadcrumbs = SentryPrivacy.maxBreadcrumbs
                ...
                options.beforeBreadcrumb = { breadcrumb in
                    SentryPrivacy.sanitizeBreadcrumb(breadcrumb)
                }
                options.beforeSend = { event in
                    SentryPrivacy.sanitizeEvent(event)
                }
            }
            SentryRuntime.markStarted()
        }

        let forceDuplicateLaunchObserver = env["TERMLOOP_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] == "1"
        ...
        if env["TERMLOOP_UI_TEST_MODE"] == "1" {
            let trigger = env["TERMLOOP_UI_TEST_TRIGGER_UPDATE_CHECK"] ?? "<nil>"
            let feed = env["TERMLOOP_UI_TEST_FEED_URL"] ?? "<nil>"
            UpdateLogStore.shared.append("ui test env: trigger=\(trigger) feed=\(feed)")
        }
        if env["TERMLOOP_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" {
        ...
        if isRunningUnderXCTest {
            if let rawVariant = env["TERMLOOP_UI_TEST_BROWSER_IMPORT_HINT_VARIANT"] {
                UserDefaults.standard.set(
                    BrowserImportHintSettings.variant(for: rawVariant).rawValue,
                    forKey: BrowserImportHintSettings.variantKey
                )
            }
            if let rawShow = env["TERMLOOP_UI_TEST_BROWSER_IMPORT_HINT_SHOW"] {
                UserDefaults.standard.set(
                    rawShow == "1",
                    forKey: BrowserImportHintSettings.showOnBlankTabsKey
                )
            }
            if let rawDismissed = env["TERMLOOP_UI_TEST_BROWSER_IMPORT_HINT_DISMISSED"] {
                UserDefaults.standard.set(
                    rawDismissed == "1",
            ...
            if env["TERMLOOP_UI_TEST_BROWSER_IMPORT_HINT_OPEN_BLANK_BROWSER"] == "1" {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { [weak self] in
                    guard let self else { return }
                    _ = self.openBrowserAndFocusAddressBar(insertAtEnd: true)
                }
            }
            if env["TERMLOOP_UI_TEST_BROWSER_IMPORT_HINT_OPEN_SETTINGS"] == "1" {
            ...
            if env["TERMLOOP_UI_TEST_BROWSER_IMPORT_AUTO_OPEN"] == "1" {
    ...
    private func writeUITestDiagnosticsIfNeeded(stage: String) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["TERMLOOP_UI_TEST_DIAGNOSTICS_PATH"], !path.isEmpty else { return }
        ...
        let targetDisplayID = env["TERMLOOP_UI_TEST_TARGET_DISPLAY_ID"] ?? ""
    ...
    private func appendUITestSocketDiagnosticsIfNeeded(
        ...
        guard env["TERMLOOP_UI_TEST_SOCKET_SANITY"] == "1" else { return }

        guard let config = socketListenerConfigurationIfEnabled() else {
            payload["socketExpectedPath"] = env["TERMLOOP_SOCKET_PATH"] ?? ""
    ...
    private func appendUITestRenderDiagnosticsIfNeeded(
        ...
        guard env["TERMLOOP_UI_TEST_DISPLAY_RENDER_STATS"] == "1" else { return }
    ...
    private func moveUITestWindowToTargetDisplayIfNeeded(attempt: Int = 0) {
        let env = ProcessInfo.processInfo.environment
        guard let rawDisplayID = env["TERMLOOP_UI_TEST_TARGET_DISPLAY_ID"],
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        DispatchQueue.main.async {
            ...
            alert.messageText = String(localized: "dialog.quitCmux.title", defaultValue: "Quit TermLoop?")
    ...
    func applicationWillTerminate(_ notification: Notification) {
        isTerminatingApp = true
        // MARK: termloop-hook
        TermLoopHooks.applicationWillTerminate()
        // MARK: /termloop-hook
        _ = saveSessionSnapshot(includeScrollback: true, removeWhenEmpty: false)
    ...
    private func setupTerminalCmdClickUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_SETUP"] == "1" else {
            dlog("cmdclick.ui.setup skip reason=env_missing tag=\(env["TERMLOOP_TAG"] ?? "nil")")
            return
        }
        guard let manifestPath = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
        ...
        guard let fixtureDirectory = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_FIXTURE_DIR"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
        ...
        let commandPath = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_COMMAND_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let displayMode = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_MODE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let lineFormat = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_LINE_FORMAT"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let linePrefix = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_LINE_PREFIX"] ?? ""
        let extraFileNamesJSON = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_EXTRA_FILE_NAMES_JSON"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)

        let fileName = env["TERMLOOP_UI_TEST_TERMINAL_CMD_CLICK_FILE_NAME"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func prepareStartupSessionSnapshotIfNeeded() {
        ...
        // MARK: termloop-hook
        // Bootstrap TermLoop stores from the sidecar (or create a Default
        // project if no sidecar exists yet) regardless of restore policy.
        defer {
            if let sessionURL = SessionPersistenceStore.defaultSnapshotFileURL() {
                TermLoopHooks.loadSidecarSnapshot(alongside: sessionURL) { [weak self] in
                    guard let self else { return }
    ...
    private nonisolated static func removeLegacyPersistedWindowGeometry(
        ...
        // MARK: termloop-hook
        TermLoopHooks.removeLegacyPersistedWindowGeometryIfPresent(defaults: defaults, keys: legacyPersistedWindowGeometryDefaultsKeys)
        // MARK: /termloop-hook
    }
    ...
    func saveSessionSnapshot(
        ...
        let writeSynchronously = forceSync
            || Self.shouldWriteSessionSnapshotSynchronously(
    ...
    nonisolated static func shouldSkipSessionSaveDuringStartupRestore(
        ...
        // MARK: termloop-hook
        return TermLoopHooks.shouldSkipSessionSaveDuringStartupRestore(isApplyingStartupSessionRestore: isApplyingStartupSessionRestore, includeScrollback: includeScrollback)
        // MARK: /termloop-hook
    }
    ...
    private func persistSessionSnapshot(
        ...
        let writeBlock = {
            ...
            if let snapshot {
                ...
                // MARK: termloop-hook
                TermLoopHooks.removeMirroredSessionSnapshotIfNeeded()
                // MARK: /termloop-hook
            }
        }
    ...
    // MARK: termloop-hook
    /// Walks window contexts in the same sorted order that
    /// `buildSessionSnapshot` uses and returns each workspace's metadata by
    /// position. The sidecar writer persists this alongside the session so
    /// restore can rehydrate metadata without relying on workspace UUIDs
    /// (which upstream's `SessionWorkspaceSnapshot` does not round-trip).
    func termLoopWorkspaceMetadataByPosition() -> [[WorkspaceMetadataStore.Metadata]] {
        termLoopWorkspaceRestoreStampsByPosition().map { $0.map(\.metadata) }
    }

    func termLoopWorkspaceRestoreStampsByPosition() -> [[WorkspaceRestoreStamp]] {
        let contexts = mainWindowContexts.values.sorted { lhs, rhs in
            let lhsWindow = lhs.window ?? windowForMainWindowId(lhs.windowId)
            let rhsWindow = rhs.window ?? windowForMainWindowId(rhs.windowId)
            let lhsIsKey = lhsWindow?.isKeyWindow ?? false
                ...
                context.tabManager.tabs
                    ...
                    .map { workspace in
                        WorkspaceRestoreStamp(
                            metadata: TermLoopHooks.persistenceMetadata(for: workspace),
                            processTitle: workspace.title,
                            customTitle: workspace.customTitle,
                            currentDirectory: workspace.currentDirectory,
                            termLoopRestoreId: workspace.termLoopRestoreId
                        )
                    }
            }
    }

    func termLoopOpenWorkspacesInPersistenceOrder() -> [Workspace] {
        let contexts = mainWindowContexts.values.sorted { lhs, rhs in
            let lhsWindow = lhs.window ?? windowForMainWindowId(lhs.windowId)
            let rhsWindow = rhs.window ?? windowForMainWindowId(rhs.windowId)
            let lhsIsKey = lhsWindow?.isKeyWindow ?? false
        ...
        return contexts
            .prefix(SessionPersistencePolicy.maxWindowsPerSnapshot)
            .flatMap { context in
                context.tabManager.tabs.prefix(SessionPersistencePolicy.maxWorkspacesPerWindow)
            }
    }
    // MARK: /termloop-hook
    ...
    private func windowForMainWindowId(_ windowId: UUID) -> NSWindow? {
        ...
        let expectedIdentifier = "termloop.main.\(windowId.uuidString)"
    ...
    private func mainWindowId(for window: NSWindow) -> UUID? {
        ...
        guard let rawIdentifier = window.identifier?.rawValue,
              rawIdentifier.hasPrefix("termloop.main.") else { return nil }
        let idPart = String(rawIdentifier.dropFirst("termloop.main.".count))
    ...
    func addWorkspaceInPreferredMainWindow(
        ...
        if let workingDirectory {
            ...
            workspace = context.tabManager.addWorkspace(select: true)
        }
        #if DEBUG
        logWorkspaceCreationRouting(
            phase: "created",
        ...
        if shortcutEventHasAddressableWindow(event) {
            if let eventWindow = resolvedShortcutEventWindow(event),
               cmuxWindowShouldOwnCloseShortcut(eventWindow) {
                // Auxiliary termloop windows do not own a terminal tab manager. Let them fall back
    ...
    func sendWelcomeCommandWhenReady(to workspace: Workspace, markShownOnSend: Bool = false) {
        sendTextWhenReady("termloop welcome\n", to: workspace) {
            if markShownOnSend {
    ...
    @objc func installCmuxCLIInPath(_ sender: Any?) {
        ...
        do {
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.installed", defaultValue: "TermLoop CLI Installed"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.installFailed", defaultValue: "Couldn't Install TermLoop CLI"),
    ...
    @objc func uninstallCmuxCLIInPath(_ sender: Any?) {
        ...
        do {
            ...
            let prefix = outcome.removedExistingEntry
                ...
                : String(localized: "cli.uninstall.notFound", defaultValue: "No TermLoop CLI symlink was found at \(outcome.destinationURL.path).")
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.uninstalled", defaultValue: "TermLoop CLI Uninstalled"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.uninstallFailed", defaultValue: "Couldn't Uninstall TermLoop CLI"),
                informativeText: error.localizedDescription,
                style: .warning
            )
        }
    ...
    private func setupJumpUnreadUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_JUMP_UNREAD_SETUP"] == "1" else { return }
        guard let notificationStore else { return }
    ...
    func armJumpUnreadFocusRecord(tabId: UUID, surfaceId: UUID) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["TERMLOOP_UI_TEST_JUMP_UNREAD_PATH"], !path.isEmpty else { return }
    ...
    private func setupGotoSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_GOTO_SPLIT_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }

        let useGhosttyConfig = env["TERMLOOP_UI_TEST_GOTO_SPLIT_USE_GHOSTTY_CONFIG"] == "1"
        ...
        func hasMainTerminalWindow() -> Bool {
            NSApp.windows.contains { window in
                ...
                return raw == "termloop.main" || raw.hasPrefix("termloop.main.")
            }
        }
        ...
        func runSetupWhenWindowReady() {
            ...
            let requestedBrowserURL = env["TERMLOOP_UI_TEST_GOTO_SPLIT_BROWSER_URL"]?
                .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func setupBonsplitTabDragUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }
        let startWithHiddenSidebar = env["TERMLOOP_UI_TEST_BONSPLIT_START_WITH_HIDDEN_SIDEBAR"] == "1"
    ...
    private func bonsplitTabDragUITestDataPath() -> String? {
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1",
              let path = env["TERMLOOP_UI_TEST_BONSPLIT_TAB_DRAG_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    private func isGotoSplitUITestRecordingEnabled() -> Bool {
        let env = ProcessInfo.processInfo.environment
        return env["TERMLOOP_UI_TEST_GOTO_SPLIT_SETUP"] == "1" || env["TERMLOOP_UI_TEST_GOTO_SPLIT_RECORD_ONLY"] == "1"
    }

    private func gotoSplitUITestDataPath() -> String? {
        guard isGotoSplitUITestRecordingEnabled() else { return nil }
        let env = ProcessInfo.processInfo.environment
        guard let path = env["TERMLOOP_UI_TEST_GOTO_SPLIT_PATH"], !path.isEmpty else { return nil }
        return path
    }
    ...
    private func focusWebViewForGotoSplitUITest(tab: Workspace, browserPanelId: UUID) {
        ...
        func recordFocusedState() {
            ...
            if ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_GOTO_SPLIT_INPUT_SETUP"] == "1" {
    ...
    private func setupFocusedInputForGotoSplitUITest(panel: BrowserPanel) {
        ...
        (() => {
          ...
          const seed = () => {
            ...
            let container = document.getElementById("termloop-ui-test-focus-container");
            if (!container || !container.tagName || container.tagName.toLowerCase() !== "div") {
              container = document.createElement("div");
              container.id = "termloop-ui-test-focus-container";
            ...
            const input = ensureInput("termloop-ui-test-focus-input", "termloop-ui-focus-primary");
            const secondaryInput = ensureInput("termloop-ui-test-focus-input-secondary", "termloop-ui-focus-secondary");
            ...
            let trackedFocusId = input.getAttribute("data-termloop-addressbar-focus-id");
            if (!trackedFocusId) {
              trackedFocusId = "termloop-ui-test-focus-input-tracked";
              input.setAttribute("data-termloop-addressbar-focus-id", trackedFocusId);
            }
    ...
    private func setupMultiWindowNotificationsUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] == "1" else { return }
        guard let path = env["TERMLOOP_UI_TEST_MULTI_WINDOW_NOTIF_PATH"], !path.isEmpty else { return }
    ...
    private func prepareMultiWindowNotificationSourceTerminalIfNeeded(
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["TERMLOOP_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] == "1" else { return }
    ...
    private func publishMultiWindowNotificationSocketStateIfNeeded(at path: String) {
        ...
        guard let config = socketListenerConfigurationIfEnabled() else {
            writeMultiWindowNotificationTestData([
                "socketExpectedPath": env["TERMLOOP_SOCKET_PATH"] ?? "",
                "socketMode": "off",
                "socketReady": "0",
                "socketPingResponse": "",
                "socketIsRunning": "0",
    ...
    func jumpToLatestUnread() {
        guard let notificationStore else { return }
#if DEBUG
        if ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_JUMP_UNREAD_SETUP"] == "1" {
            writeJumpUnreadTestData([
                "jumpUnreadInvoked": "1",
        ...
        shortcutMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .keyUp, .flagsChanged]) { [weak self] event in
            ...
            if event.type == .keyDown {
                ...
                let shortcutMonitorTraceEnabled =
                    ProcessInfo.processInfo.environment["TERMLOOP_SHORTCUT_MONITOR_TRACE"] == "1"
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
        ...
        if matchConfiguredShortcut(event: event, action: .closeOtherTabsInPane) {
            if let targetWindow = event.window ?? NSApp.keyWindow ?? NSApp.mainWindow,
               targetWindow.identifier?.rawValue == "termloop.settings" {
        ...
        if matchConfiguredShortcut(event: event, action: .closeTab) {
            ...
            if let targetWindow = [targetWindow, NSApp.keyWindow]
                .compactMap({ $0 })
                .first(where: { $0.identifier?.rawValue == "termloop.browser-popup" }) {
#if DEBUG
    ...
    func closeMainWindowContainingTabId(_ tabId: UUID) {
        ...
        let expectedIdentifier = "termloop.main.\(context.windowId.uuidString)"
...
#if DEBUG
        let isJumpUnreadUITest = ProcessInfo.processInfo.environment["TERMLOOP_UI_TEST_JUMP_UNREAD_SETUP"] == "1"
...
final class MenuBarExtraController: NSObject, NSMenuDelegate {
    ...
    private let menu = NSMenu(title: "TermLoop")
    ...
    private let quitItem = NSMenuItem(title: String(localized: "menu.quitCmux", defaultValue: "Quit TermLoop"), action: nil, keyEquivalent: "")
        ...
        if let button = statusItem.button {
            ...
            button.toolTip = displayedUnreadCount == 0
                ...
                : displayedUnreadCount == 1
                    ? "TermLoop: " + String(localized: "statusMenu.tooltip.unread.one", defaultValue: "1 unread notification")
                    : "TermLoop: " + String(localized: "statusMenu.tooltip.unread.other", defaultValue: "\(displayedUnreadCount) unread notifications")
        }
    }
...
enum MenuBarIconRenderer {
    ...
    private static func drawGlyph(in rect: NSRect) {
        // Match the canonical termloop center-mark path from Icon Center Image Artwork.svg.
...
private extension AppDelegate {
    @objc func handleThemesReloadNotification(_ notification: Notification) {
        DispatchQueue.main.async {
            GhosttyApp.shared.reloadConfiguration(source: "distributed.termloop.themes")
        }
    }
}
...
private extension NSWindow {
    ...
    @objc func cmux_sendEvent(_ event: NSEvent) {
        ...
        // Hit-test context cache is only consumed by `cmuxPointerHitWebView`,
        // which short-circuits to nil for non-pointer-down events. Running
        // `themeFrame.hitTest()` + `contentView.hitTest()` for every mouseMoved
        // / mouseDragged / scrollWheel / key event walks the entire NSView
        // tree at 60-120 Hz with a result that's never read — burns ~30-90%
        // CPU during mouse motion. Gate the precompute to the same predicate
        // as the consumer so the cache only fills when it'll actually be used.
        cmuxFirstResponderGuardHitViewContext = Self.cmuxIsPointerDownEvent(event)

...
const claudeNodeOptionsRestoreModuleScript = `const hadOriginalNodeOptions = process.env.TERMLOOP_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
...
func runClaudeTeamsRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	configureAgentEnvironment(agentConfig{
		...
		termEnvVar:     "TERMLOOP_CLAUDE_TEAMS_TERM",
...
func createTmuxShimDir(dirName string, tmuxScript string) (string, error) {
	...
	dir := filepath.Join(home, ".termloop", dirName)
...
func ensureClaudeNodeOptionsRestoreModule() (string, error) {
	dir := filepath.Join(os.TempDir(), "termloop-claude-node-options")
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
...
func configureClaudeNodeOptions(restoreModulePath string) {
	...
	if hadExisting {
		os.Setenv("TERMLOOP_ORIGINAL_NODE_OPTIONS_PRESENT", "1")
		os.Setenv("TERMLOOP_ORIGINAL_NODE_OPTIONS", existing)
	} else {
		os.Setenv("TERMLOOP_ORIGINAL_NODE_OPTIONS_PRESENT", "0")
		os.Unsetenv("TERMLOOP_ORIGINAL_NODE_OPTIONS")
	}
...
func configureAgentEnvironment(cfg agentConfig) {
	...
	// Socket path
	os.Setenv("TERMLOOP_SOCKET_PATH", cfg.socketPath)
	os.Setenv("TERMLOOP_SOCKET", cfg.socketPath)
	...
	if cfg.focused != nil {
		os.Setenv("TERMLOOP_WORKSPACE_ID", cfg.focused.workspaceId)
		if cfg.focused.surfaceId != "" {
			os.Setenv("TERMLOOP_SURFACE_ID", cfg.focused.surfaceId)
		}
	}
...
func omoShadowConfigDir() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".termloop", "omo-config")
}

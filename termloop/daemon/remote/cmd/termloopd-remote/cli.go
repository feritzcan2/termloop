...
// runCLI is the entry point for the "cli" subcommand (or busybox "termloop" invocation).
func runCLI(args []string) int {
	socketPath := os.Getenv("TERMLOOP_SOCKET_PATH")
	...
	for i := 0; i < len(args); i++ {
		...
		case "--socket":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "termloop: --socket requires a path")
				return 2
			}
			socketPath = args[i+1]
			i++
...
doneFlags:
	...
	if socketPath == "" {
		fmt.Fprintln(os.Stderr, "termloop: TERMLOOP_SOCKET_PATH not set and --socket not provided")
		return 1
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "termloop: unknown command %q\n", cmdName)
		return 2
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "termloop: internal error: unknown protocol for %q\n", cmdName)
		return 1
	}
}
...
func execV1(socketPath string, spec *commandSpec, args []string, refreshAddr func() string) int {
	...
	if !spec.noParams {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "termloop: %v\n", err)
			return 2
		}
...
func runRPC(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "termloop rpc: requires a method name")
		return 2
	}
	method := args[0]
	...
	if len(args) > 1 {
		if err := json.Unmarshal([]byte(args[1]), &params); err != nil {
			fmt.Fprintf(os.Stderr, "termloop rpc: invalid JSON params: %v\n", err)
			return 2
		}
	}
...
// runBrowserRelay handles "termloop browser <subcommand>" by mapping to browser.* v2 methods.
func runBrowserRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "termloop browser: requires a subcommand (open, navigate, back, forward, reload, get-url)")
		return 2
	}
	...
	case "get-url":
		...
		useSurfaceEnv = true
	default:
		fmt.Fprintf(os.Stderr, "termloop browser: unknown subcommand %q\n", sub)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "termloop browser: %v\n", err)
		return 2
	}
...
func applyWorkspaceEnvFallback(params map[string]any) {
	...
	if envWs := os.Getenv("TERMLOOP_WORKSPACE_ID"); envWs != "" {
...
func applySurfaceEnvFallback(params map[string]any) {
	...
	if envSf := os.Getenv("TERMLOOP_SURFACE_ID"); envSf != "" {
...
// readSocketAddrFile reads the socket address from ~/.termloop/socket_addr as a fallback
// when TERMLOOP_SOCKET_PATH is not set. Written by the TermLoop app after the relay establishes.
func readSocketAddrFile() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	data, err := os.ReadFile(filepath.Join(home, ".termloop", "socket_addr"))
	if err != nil {
		return ""
	}
...
func readRelayAuthFile(socketPath string) *relayAuthState {
	if strings.Contains(socketPath, ":") && !strings.HasPrefix(socketPath, "/") {
		...
		data, err := os.ReadFile(filepath.Join(home, ".termloop", "relay", port+".auth"))
		if err != nil {
			return nil
		}
...
func currentRelayAuth(socketPath string) *relayAuthState {
	relayID := strings.TrimSpace(os.Getenv("TERMLOOP_RELAY_ID"))
	relayToken := strings.TrimSpace(os.Getenv("TERMLOOP_RELAY_TOKEN"))
...
// dialSocket connects to the TermLoop socket. If addr contains a colon and doesn't
...
func authenticateRelayConn(conn net.Conn, auth *relayAuthState) error {
	...
	if challenge.Protocol != "termloop-relay-auth" || challenge.Version != 1 || challenge.RelayID != auth.RelayID || challenge.Nonce == "" {
...
func cliUsage() {
	fmt.Fprintln(os.Stderr, "Usage: termloop [--socket <path>] [--json] <command> [args...]")
	...
	fmt.Fprintln(os.Stderr, "  omo [args...]              Launch OpenCode with TermLoop integration")
	fmt.Fprintln(os.Stderr, "  omx [args...]              Launch Oh My Codex with TermLoop integration")
	fmt.Fprintln(os.Stderr, "  omc [args...]              Launch Oh My Claude Code with TermLoop integration")

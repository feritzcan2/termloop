...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/termloop-wait-for-") {
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"TERMLOOP_CLAUDE_TEAMS_TERMLOOP_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "TERMLOOP_SOCKET_PATH", "TERMLOOP_SOCKET", "TERM_PROGRAM",
		"TERMLOOP_WORKSPACE_ID", "TERMLOOP_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("TERMLOOP_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("TERMLOOP_SOCKET_PATH = %q", os.Getenv("TERMLOOP_SOCKET_PATH"))
	}
	...
	if os.Getenv("TERMLOOP_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("TERMLOOP_WORKSPACE_ID = %q", os.Getenv("TERMLOOP_WORKSPACE_ID"))
	}
	if os.Getenv("TERMLOOP_SURFACE_ID") != "surf-789" {
		t.Errorf("TERMLOOP_SURFACE_ID = %q", os.Getenv("TERMLOOP_SURFACE_ID"))
	}
	// Verify extra env

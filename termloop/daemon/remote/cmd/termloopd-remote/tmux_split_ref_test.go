...
func TestTmuxSplitWindowCanonicalizesCallerSurfaceRefs(t *testing.T) {
	...
	origWorkspace := os.Getenv("TERMLOOP_WORKSPACE_ID")
	origSurface := os.Getenv("TERMLOOP_SURFACE_ID")
	origPane := os.Getenv("TMUX_PANE")
	os.Setenv("HOME", t.TempDir())
	os.Setenv("TERMLOOP_WORKSPACE_ID", "workspace:1")
	os.Setenv("TERMLOOP_SURFACE_ID", "surface:1")
	os.Setenv("TMUX_PANE", "%pane:1")
	defer func() {
		os.Setenv("HOME", origHome)
		if origWorkspace != "" {
			os.Setenv("TERMLOOP_WORKSPACE_ID", origWorkspace)
		} else {
			os.Unsetenv("TERMLOOP_WORKSPACE_ID")
		}
		if origSurface != "" {
			os.Setenv("TERMLOOP_SURFACE_ID", origSurface)
		} else {
			os.Unsetenv("TERMLOOP_SURFACE_ID")
		}
		if origPane != "" {
...
func TestTmuxSplitWindowIgnoresStaleUUIDColumnSurface(t *testing.T) {
	...
	storePath := filepath.Join(home, ".termloop", "tmux-compat-store.json")

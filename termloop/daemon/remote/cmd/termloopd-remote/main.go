...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "termloopd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  termloopd-remote version")
	_, _ = fmt.Fprintln(w, "  termloopd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  termloopd-remote cli <command> [args...]")
}

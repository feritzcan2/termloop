...
# @raycast.title Open TermLoop Next
...
TERMLOOP_NEXT_DIR="$(cd "$SCRIPT_DIR/.." && pwd -P)"
...
if [ ! -d "$TERMLOOP_NEXT_DIR" ]; then
  echo "TermLoop Next directory not found: $TERMLOOP_NEXT_DIR"
  exit 1
fi

tag="${1:-main}"
checkout_selector="${2:-main}"
launcher="$TERMLOOP_NEXT_DIR/tools/dev/termloop-dev"
checkout=""

if [ ! -x "$launcher" ]; then
  echo "TermLoop Next launcher not found or not executable: $launcher"
  exit 1
fi

if [ "$checkout_selector" = "main" ]; then
  checkout="$TERMLOOP_NEXT_DIR"
elif [ -d "$checkout_selector" ]; then
  checkout="$checkout_selector"
elif [ -d "$TERMLOOP_NEXT_DIR/$checkout_selector" ]; then
  checkout="$TERMLOOP_NEXT_DIR/$checkout_selector"
elif [ -d "$TERMLOOP_NEXT_DIR/.termloop-worktrees/$checkout_selector" ]; then
  checkout="$TERMLOOP_NEXT_DIR/.termloop-worktrees/$checkout_selector"
else
  ...
  done < <(git -C "$TERMLOOP_NEXT_DIR" worktree list --porcelain | sed -n 's/^worktree //p')
fi
...
if [ "$tag" = "main" ]; then
  if [ "$checkout" != "$TERMLOOP_NEXT_DIR" ]; then

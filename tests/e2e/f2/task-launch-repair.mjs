...
git(repository,["config","user.name","TermLoop Fixture"]); git(repository,["config","user.email","fixture@termloop.invalid"]); git(repository,["commit","--allow-empty","-m","fixture"]); git(repository,["update-ref","refs/remotes/origin/main","HEAD"]);
...
try {
 ...
 const emptyTask=await call(record,"task.create",{projectId:project.id,title:"Needs worktree",brief:null,worktreeIntent:"none",worktreePrefix:null,baseRef:null,agentId:null,model:null,permission:null,reasoning:null,kickoffMessage:null});
 ...
 const task=await call(record,"task.create",{projectId:project.id,title:"Managed Task",brief:"visible brief",worktreeIntent:"none",worktreePrefix:null,baseRef:null,agentId:null,model:null,permission:null,reasoning:null,kickoffMessage:null});
 const provision=await call(record,"task.provisionWorktree",{operationId:randomUUID(),taskId:task.id,repositoryPath:repository,destinationPath:worktree,branchName:"feature/task-launch",branchMode:"create",baseRef:"refs/remotes/origin/main"});

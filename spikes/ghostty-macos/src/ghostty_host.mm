// Spike: host native Ghostty surfaces inside an Electron BrowserWindow.
//
...
static bool actionCallback(ghostty_app_t, ghostty_target_s, ghostty_action_s) {
  // The spike ignores all app actions (new window, bell, titles, ...).
  return false;
}
...
- (void)dispatchKeyEvent:(NSEvent *)event
  ...
  // Electron can leave `characters` empty for a physical event delivered to
  // an embedded child NSView. Explicit AppKit translation is required by
  // Kitty keyboard mode; legacy mode can otherwise mask this by keycode.
  if (chars.length == 0) {
...
static Napi::Value SetSurfaceVisible(const Napi::CallbackInfo &info) {
  ...
  // Ghostty's argument is visibility, not an `occluded` boolean. This mirrors
  // the official macOS host's NSWindow occlusion-state forwarding.
...
// Diagnostics: synthesize the same NSEvent path used by physical keyboard
// input so the harness can validate legacy and Kitty keyboard encodings.
static Napi::Value SendKey(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  ...
  const unsigned short keyCode =
      (unsigned short)info[2].As<Napi::Number>().Uint32Value();
  const bool blankCharacters = info.Length() > 3 &&
      info[3].IsBoolean() && info[3].As<Napi::Boolean>().Value();
  NSString *eventCharacters = blankCharacters ? @"" : characters;
  NSEvent *down = [NSEvent keyEventWithType:NSEventTypeKeyDown
                                   location:NSZeroPoint
                              modifierFlags:0
                                  timestamp:NSProcessInfo.processInfo.systemUptime
                               windowNumber:e->view.window.windowNumber
                                    context:nil
...
// Regression diagnostics for modifier-only AppKit events. FlagsChanged events
// intentionally have no character payload; asking them for `characters`
// raises an Objective-C exception and terminates the Electron main process.
static Napi::Value SendModifierKey(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  SurfaceEntry *e = entryForId(info[0].As<Napi::Number>().Uint32Value());
  if (e == nullptr) return env.Undefined();
  const unsigned short keyCode =
      (unsigned short)info[1].As<Napi::Number>().Uint32Value();
  NSEventModifierFlags flags = 0;
  switch (keyCode) {
    case 0x38:
      flags = NSEventModifierFlagShift | NX_DEVICELSHIFTKEYMASK;
      break;
    case 0x3B:
      flags = NSEventModifierFlagControl | NX_DEVICELCTLKEYMASK;
      break;
    case 0x37:
      flags = NSEventModifierFlagCommand | NX_DEVICELCMDKEYMASK;
      break;
    default:
      Napi::RangeError::New(env, "unsupported modifier keycode")
          .ThrowAsJavaScriptException();
      return env.Undefined();
  }
  const auto send = [&](NSEventModifierFlags eventFlags) {
...
// Diagnostics: exercise AppKit's command-key-equivalent route used by Cmd+V.
static Napi::Value SendPasteShortcut(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  SurfaceEntry *e = entryForId(info[0].As<Napi::Number>().Uint32Value());
  if (e == nullptr) return Napi::Boolean::New(env, false);
  std::string text = info[1].As<Napi::String>();
  NSString *value = [[NSString alloc] initWithBytes:text.data()
                                             length:text.size()
                                           encoding:NSUTF8StringEncoding];
  ...
  NSEvent *event = [NSEvent keyEventWithType:NSEventTypeKeyDown
                 ...
                 charactersIgnoringModifiers:@"v"
                                   isARepeat:NO
                                     keyCode:0x09];
  return Napi::Boolean::New(env, [e->view performKeyEquivalent:event]);
}
...
static Napi::Object Init(Napi::Env env, Napi::Object exports) {
  ...
  exports.Set("sendModifierKey", Napi::Function::New(env, SendModifierKey));

...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GeneratedInputSettlement {
    OutputActivity,
    ComposerRender,
    CodexComposerRender,
    ProviderQueue,
...
impl GeneratedInputDeliveryRuntime {
    ...
    pub fn provenance(
        ...
        session_id: &str,
        runtime_epoch: u64,
    ) -> Option<&termloop_launch::Provenance> {
        self.deliveries
            .get(session_id)
            .filter(|delivery| delivery.runtime_epoch == runtime_epoch)
    ...
    pub fn contains_submission(
        ...
        self.deliveries.get(session_id).is_some_and(|delivery| {
            delivery.runtime_epoch == runtime_epoch && &delivery.submission == submission
        })
    }
    ...
    pub fn can_begin_pending_submission(&self, session_id: &str, runtime_epoch: u64) -> bool {
        self.deliveries.get(session_id).is_none_or(|delivery| {

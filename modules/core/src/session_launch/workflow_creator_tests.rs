...
impl Fixture {
    ...
    fn session(&mut self, workflow_id: Option<String>) -> ImproverSessionTarget {
        ...
        self.runtime
            .store
            .insert_session(
                &self.runtime.write_authority,
                SessionRecord {
                    id: "creator".into(),
                    project_id: self.project.clone(),
                    name: Some("Workflow Creator".into()),
                    kind: SessionKind::Agent,
                    process: ProcessDescriptor {
                        program: "claude".into(),
                        args: vec![],
                        cwd: self.root.to_string_lossy().into(),
...
fn preview_is_read_only_pins_context_and_consumes_exact_ticket_once() {
    ...
    assert!(
        preview["delivered_preview"]
            .as_str()
            .unwrap()
            .contains("İsteği uygula. Preserve {{context}}.")
    );
    let mut launch = params.clone();
    launch["launchTicket"] = preview["launch_ticket"].clone();
    let plan = f
        .runtime
        .take_workflow_creator_launch(launch.clone())
        .unwrap();
    assert_eq!(
        plan.improver_session_name.as_deref(),
        Some("Workflow Creator")
    );
    assert_eq!(
        super::super::improver_session_target(&plan)
            .unwrap()
            .target_kind,
    ...
    for field in [
        ...
        assert!(
            f.runtime.take_workflow_creator_launch(changed).is_err(),
            "{field}"
        );
    }
    let mut invalid = params;
    invalid.as_object_mut().unwrap().remove("workflowId");
...
fn draft_write_is_separate_durable_cas_state_never_a_template_or_execution() {
    ...
    let plan = f
        ...
        .prepare_improver_configuration_write(
            ...
            None,
            content.clone(),
            "Initial proposal".into(),
        )
        .unwrap();
    let result = f
        .runtime
        .apply_owned_configuration_application(plan, crate::AssistantAvailability::Proven, 1)
        .unwrap()
        .result
        .unwrap();
    ...
    assert!(matches!(
        f.runtime.prepare_improver_configuration_write(
            "creator",
            &target,
            None,
            content.clone(),
    ...
    let mut next: Value = serde_json::from_str(&content).unwrap();
    next["workflow"]["name"] = json!("Revised proposal");
    let plan = f
        .runtime
        .prepare_improver_configuration_write(
            "creator",
        ...
        .unwrap();
    f.runtime
        .apply_owned_configuration_application(plan, crate::AssistantAvailability::Proven, 2)
        .unwrap()
        .result
        .unwrap();
    let read = f
        .runtime
        .get_workflow_creator_draft(json!({ "projectId": f.project, "workflowId": null }))
        .unwrap();
    ...
    let stopped = f
        .runtime
        .store
        .mark_session_exited(&f.runtime.write_authority, "creator");
    stopped.unwrap();
    assert!(matches!(
        f.runtime
            .read_improver_configuration_version("creator", &target),
...
fn proposal_rejects_bad_graphs_settings_fields_and_stale_source_without_changing_template() {
    let mut f = Fixture::new();
    let mut create = draft();
    create["projectId"] = json!(f.project);
    create["expectedRevision"] = json!(f.runtime.store.revision());
    ...
    for (pointer, value) in [
        ("/sourceGeneration", json!(2)),
        ("/workflow/model", json!("made-up")),
        ("/workflow/maxReviewCycles", json!(4)),
        ("/workflow/steps", json!([])),
        ("/workflow/steps/0/agentId", json!("claude")),
        ("/workflow/steps/0/instructions", json!("x".repeat(4097))),
    ] {
    ...
    f.runtime
        ...
        .unwrap();
    assert_eq!(
        crate::workflow::workflow_configuration_json(
            &f.runtime.workflow_configuration(&id).unwrap()
        ),
        saved
    );
    let mut update = draft();
    update["name"] = json!("Changed elsewhere");
    update["workflowId"] = json!(id);
    update["expectedRevision"] = json!(f.runtime.store.revision());
    f.runtime.update_workflow_configuration(update).unwrap();
    let read = f
        .runtime
        .get_workflow_creator_draft(json!({ "projectId": f.project, "workflowId": id }))
        .unwrap();
    ...
    assert!(matches!(
        f.runtime.prepare_improver_configuration_write(
            "creator",
            &target,
            Some(read["versionId"].as_str().unwrap().into()),
            content.to_string(),
            "".into()
        ),
        Err(CoreError::RevisionConflict)
    ...
    f.runtime
        .delete_workflow_configuration(
            json!({ "workflowId": id, "expectedRevision": f.runtime.store.revision() }),
        )
        .unwrap();
...
fn cross_project_context_and_changed_preview_fail_closed() {
    ...
    let mut params = f.params();
    let preview = f.runtime.preview_workflow_creator(params.clone()).unwrap();
    let mut new_template = draft();
    new_template["projectId"] = json!(f.project);
    new_template["expectedRevision"] = json!(f.runtime.store.revision());
    f.runtime
        .create_workflow_configuration(new_template)
        .unwrap();
    ...
    let other = f
        ...
        .handle(
            "project.create",
            json!({ "name": "Other", "folderPath": f.root.join("other") }),
        )
        .unwrap();
    let mut create = draft();
    create["projectId"] = other["id"].clone();
    create["expectedRevision"] = json!(f.runtime.store.revision());
    let saved = f.runtime.create_workflow_configuration(create).unwrap();
    let mut params = f.params();
    params["workflowId"] = saved["configuration"]["id"].clone();

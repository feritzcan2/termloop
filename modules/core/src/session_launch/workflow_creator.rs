...
use super::{
    AgentLaunchPlan, AgentMcpRole, interactive_agent_options, invocation_error,
...
impl CoreRuntime {
    pub(crate) fn require_workflow_creator_session(
        &self,
        session_id: &str,
    ...
    pub fn get_workflow_creator_draft(&self, params: Value) -> Result<Value, CoreError> {
        ...
        if let Some(id) = &target.target_id
            && self.workflow_configuration(id)?.project_id != project_id
        {
            return Err(CoreError::NotFound);
        }
        let version = self
        ...
        let proposal: Option<Value> = version
            .map(|version| {
            ...
            .transpose()?;
        Ok(
            json!({ "proposal": proposal, "versionId": version.map(|version| &version.id),
    ...
    fn workflow_creator_context(&self, params: &Value) -> Result<String, CoreError> {
        ...
        let configuration = workflow_id
            .map(|id| self.workflow_configuration(id))
            .transpose()?;
        if configuration
            .as_ref()
            .is_some_and(|value| value.project_id != project_id)
        {
            return Err(CoreError::NotFound);
        }
        let task = nullable_id(params, "taskId")?
            .map(|id| {
                self.store
                    .tasks()
                    .iter()
                    .find(|task| task.id == id && task.project_id == project_id)
        ...
        let draft = params
            .get("draft")
            .ok_or_else(|| CoreError::InvalidParams("draft".into()))?;
        if !draft.is_null() {
            self.validate_workflow_draft(draft)?;
        }
    ...
    pub fn preview_workflow_creator(&mut self, params: Value) -> Result<Value, CoreError> {
        ...
        let mut plan = self.plan_agent_launch(
            json!({ "projectId": project_id, "cwd": cwd, "agentId": agent_id }),
        )?;
        if !plan
    ...
    pub fn take_workflow_creator_launch(
        &mut self,
        mut params: Value,
        ...
        if plan
            .workflow_creator
            .as_ref()
            .is_none_or(|creator| creator.params != params)
        {
    ...
    pub(super) fn revalidate_workflow_creator(
        &self,
        plan: &AgentLaunchPlan,
    ) -> Result<(), CoreError> {
        let Some(creator) = &plan.workflow_creator else {
            return Ok(());
        };
        let project = self
            .store
            .projects()
            .iter()
            .find(|p| p.id == plan.project_id)
            .ok_or(CoreError::NotFound)?;
        if project.folder_path != plan.cwd
            || self.workflow_creator_context(&creator.params)? != creator.context
        {

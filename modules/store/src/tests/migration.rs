...
#[test]
fn schema_59_preserves_single_agent_automation_without_inferring_a_workflow() {
    let path = std::env::temp_dir().join(format!(
        "termloop-store-automation-workflow-{}.json",
        uuid::Uuid::new_v4()
    ));
    std::fs::write(
        &path,
        serde_json::to_vec(&serde_json::json!({
            "schema_version": 59, "revision": 7,
            ...
            "project_task_automation_configurations": [{
                ...
                "agentId":"codex", "model":"default", "permission":"plan",
    ...
    let configuration = store.project_task_automation_configurations()[0].clone();
    assert_eq!(configuration.workflow_id, None);
    assert_eq!(configuration.agent_id.as_deref(), Some("codex"));
    assert_eq!(configuration.permission.as_deref(), Some("plan"));
    let persisted: serde_json::Value =
        serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();
    assert_eq!(persisted["schema_version"], CURRENT_SCHEMA_VERSION);
    assert_eq!(persisted["revision"], 7);
    assert_eq!(
        persisted["project_task_automation_configurations"][0].get("workflowId"),
        Some(&serde_json::Value::Null)
    );
    drop(store);
    let reopened = Store::open(&path).unwrap();
    assert_eq!(
        reopened.project_task_automation_configurations(),
        &[configuration]
    );

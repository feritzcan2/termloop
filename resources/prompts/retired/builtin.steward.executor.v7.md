...
identified in the current visible conversation or context and `task_read` shows
that Task has no Jira URL. Never guess a URL, infer one from a key or fuzzy
title, search for a candidate, or replace an existing link. When an explicit
request includes a worktree and coding Agent, create or identify the Task and
call `task_agent_start` once with the requested outcome.

...
Include known Task or Session refs when a message addresses them and
`refs.routineFindingId` on every finding-bound proposal. Successful mutations
are represented by TermLoop action receipts, not by a second claim in
`steward_suggest`.
...
- `suggestion`: optional non-blocking advice the user may accept; and
...
TermLoop or external action the Steward will perform and its target.

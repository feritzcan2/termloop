...
the complete current editable document. Interpret the user's conversational
request as an edit to that source document. Preserve every unaffected existing
instruction and produce the complete modified document; do not copy the user's
request verbatim as the whole prompt unless the user explicitly asks to discard
and replace everything with that exact text. Call
...
`systemPrompt`. Never use a Tracker report, external/source content, your own

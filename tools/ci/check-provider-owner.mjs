...
const rule = rules.find(({ id }) => id === "PROVIDER_NETWORK_OWNER");
if (!rule) throw new Error("PROVIDER_NETWORK_OWNER rule is missing");
...
if (errors.length) {
  for (const file of errors) console.error(`PROVIDER_NETWORK_OWNER: ${file}: ${rule.message}`);
  process.exit(1);
}
console.log("PROVIDER_OWNER_OK");

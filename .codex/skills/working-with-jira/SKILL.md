...
## TermLoop workspace context

When resuming an in-progress workspace, call `mcp__termloop__get_jira_ticket`
before guessing the ticket from the branch name.

As soon as you parse a Jira key, call `mcp__termloop__set_jira_ticket` with
`{ key, status, url }` so the TermLoop sidebar chip reflects the active ticket.
Call it again whenever the key, status, or URL changes, including after
transitions. Skip duplicate calls when nothing changed.

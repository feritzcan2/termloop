import type { GhosttyHostAddon, NativeInputPolicy } from "./platform/addon.js";
import { socketFromFd, type FdSocket } from "./platform/fd-socket.js";

export type GhosttyWindow = {
  getNativeWindowHandle(): Buffer;
  isDestroyed(): boolean;
};
export type GhosttyHostEvents = {
  input(surfaceId: number, data: ArrayBuffer): void;
  closed(surfaceId: number): void;
...
/// Owns native Ghostty surfaces for one GhosttyWindow. Bytes written by
...
export class GhosttySurfaceManager {
  ...
  readonly #window: GhosttyWindow;
  readonly #configFile: string;
  readonly #lightConfigFile: string;
  readonly #surfaces = new Map<number, SurfaceEntry>();
  #appInitialized = false;

  constructor(addon: GhosttyHostAddon, window: GhosttyWindow, configFile: string, lightConfigFile: string, private readonly events: GhosttyHostEvents, private readonly inputPolicy: NativeInputPolicy = {}) {
    this.#addon = addon;
    this.#window = window;
  ...
  create(frame?: SurfaceFrame): { surfaceId: number; rows: number; cols: number } {
    ...
    socket.on("data", (chunk: Buffer) => {
      ...
      this.events.input(created.id, Uint8Array.from(chunk).buffer);
    });
  ...
  #shellShortcut(shortcut: string): void {
    if (this.#window.isDestroyed()) return;
    this.events.shortcut(shortcut);
  }
}

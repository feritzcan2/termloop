...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
  ...
  // The embedder owns shell navigation. Deliver its configured chords to
  ...
  if (const char *shortcut = embedderShortcutForEvent(event)) {
  ...
  if (!g_inputPolicy->imagePasteAction.empty() && isImageOnlyPasteKeyEquivalent(event)) {
    notifyShellShortcut(g_inputPolicy->imagePasteAction.c_str());
    return YES;
  }
...
- (void)flagsChanged:(NSEvent *)event {
  ...
  if (g_inputPolicy->doubleShiftAction.empty() || !isShift || conflictingModifiers != 0) {
    self.firstShiftDownAt = 0;
    self.shiftReleased = NO;
  } else if (action == GHOSTTY_ACTION_PRESS) {
    if (self.firstShiftDownAt > 0 && self.shiftReleased &&
        event.timestamp - self.firstShiftDownAt <= g_inputPolicy->doubleShiftWindow) {
      self.firstShiftDownAt = 0;
      self.shiftReleased = NO;
      notifyShellShortcut(g_inputPolicy->doubleShiftAction.c_str());
    } else {
      self.firstShiftDownAt = event.timestamp;
      self.shiftReleased = NO;
    }
  } else if (self.firstShiftDownAt > 0) {
    if (event.timestamp - self.firstShiftDownAt <= g_inputPolicy->doubleShiftWindow) {
...
static Napi::Value InitApp(const Napi::CallbackInfo &info) {
  ...
  if (info.Length() > 0 && info[0].IsObject()) {
    ...
    if (!configureInputPolicy(opts)) return env.Undefined();

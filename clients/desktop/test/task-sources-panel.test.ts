...
function actions(state: PanelState): TaskSourceActions & { calls: string[] } {
  ...
  return {
    ...
    setProjectAutomation: async (params) => {
      calls.push(`automationSet:${params.projectId}:${params.createWorktree}:${params.agentId ?? "-"}:${params.model ?? "-"}:${params.reasoning ?? "-"}:${params.kickoffMessage ? "message" : "-"}:${params.expectedRevision}`);
      state.automation = {
    ...
    importCandidate: async (params) => {
      calls.push(`import:${params.externalId}:${params.expectedGeneration}:${params.expectedObservationSequence}:${params.expectedRevision}:${params.worktreeIntent}:${params.agentId ?? "-"}:${params.model ?? "-"}:${params.reasoning ?? "-"}:${params.kickoffMessage ? "message" : "-"}`);
...
describe("Task Sources panel", () => {
  ...
  it("states the Task rule once above the sources and saves the edited launch profile", async () => {
    const api = actions({ sources: [source({ name: "Apcoadigital Jira" })], candidates: [] });
  ...
  it("confirms the worktree and agent for one import, prefilled from the Project default, and blocks a second mutation while busy", async () => {
    ...
    expect(api.calls.filter((call) => call.startsWith("import:"))).toEqual([
      "import:10001:3:7:10:provision:claude:default:default:message",
    ]);
  ...
  it("saves board 310 with multiple discovered statuses from an existing source", async () => {
    ...
    const form = host.querySelector('form[aria-label="Edit Apcoadigital Jira"]') as HTMLFormElement;

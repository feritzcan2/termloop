...
function LaunchManifestView({ manifest, tab }: { manifest: InspectableLaunchManifest; tab: "preview" | "raw" }) {
  ...
  return <div className="launch-inspector-grid">
    <Block title="Launch settings"><dl><dt>Agent</dt><dd>{manifest.target.agent_id}</dd>{manifest.target.account_name ? <><dt>Account</dt><dd>{manifest.target.account_name}</dd></> : null}<dt>Model</dt><dd>{manifest.target.model}</dd><dt>Permission</dt><dd>{manifest.target.permission}</dd><dt>Reasoning</dt><dd>{manifest.target.reasoning}</dd><dt>Directory</dt><dd>{manifest.target.cwd}</dd><dt>Conversation</dt><dd>{manifest.target.conversation}</dd></dl></Block>

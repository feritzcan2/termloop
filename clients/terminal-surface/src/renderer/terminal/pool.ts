import type { TerminalPresentation, TerminalPresentationPort } from "./presentation.js";
...
export type TerminalSession = { id: string; runtime_epoch: number; lifecycle_state: string };
export type TerminalRetentionPolicy<S> = {
  retain(session: S): boolean;
  canAttach(session: S): boolean;
  connectionScope(session: S): string;
};
...
export type AttachmentFactory<S> = (session: S) => Promise<TerminalAttachmentLike>;
type Entry<S> = {
  session: S;
...
export class TerminalPool<S extends TerminalSession> {
  readonly #entries = new Map<string, Entry<S>>();
  ...
  #present(entry: Entry<S>, patch: Partial<TerminalPresentation>): void {
    if (entry.rendererFailed) patch = { ...patch, phase: "failed", notice: "Terminal display stopped. Reopen the view to recover." };
  ...
  async #read(id: string, enabled: boolean): Promise<void> {
    ...
    try {
      ...
      if (text === undefined) throw new Error("terminal snapshot unavailable");
  ...
  async #recover(id: string): Promise<void> {
    ...
    try { await this.mount(id, container); }
    catch { this.#present(entry, { phase: "failed", notice: "The terminal view could not be reopened." }); }
  }
  ...
  constructor(
    ...
    private readonly attachmentFactory: AttachmentFactory<S>,
    private readonly diagnosticsEnabled = false,
    private readonly onImagePaste: (sessionId: string) => void,
    private readonly policy: TerminalRetentionPolicy<S>,
  ) {}

  reconcile(sessions: readonly S[]): void {
    const retained = new Map(sessions
      .filter(this.policy.retain)
    ...
    for (const [id, entry] of this.#entries) {
      ...
      if (!next) {
        ...
        if (runtimeChanged && entry.mounted) {
          ...
          entry.mounted = false;
        } else if (this.policy.canAttach(next) && entry.mounted) {
    ...
    for (const session of retained.values()) {
      if (!this.#entries.has(session.id)) {
        this.#entries.set(session.id, {
          session,
          presentation: { phase: this.policy.canAttach(session) ? "connecting" : "exited" },
  ...
  async mount(sessionId: string, container: HTMLElement): Promise<void> {
    ...
    if (!entry.surface) {
      ...
      entry.replay = new TerminalReplayBuffer((bytes, complete) => {
        ...
        const ready = () => {
          if (entry.surface === surface && entry.attachment === attachment
            && ["connecting", "replaying", "live"].includes(entry.presentation.phase)) {
  ...
  reconnectAttachments(profileId?: string): void {
    for (const entry of this.#entries.values()) {
      if (profileId && this.policy.connectionScope(entry.session) !== profileId) continue;
      this.#detachAttachment(entry);
      if (entry.surface && this.policy.canAttach(entry.session)) {
        void this.#ensureAttachment(entry);
      }
    }
  }

  async #ensureAttachment(entry: Entry<S>): Promise<void> {
    ...
    if (!this.policy.canAttach(entry.session)) return;
    ...
    const pending = this.attachmentFactory(entry.session)
      .then((attachment) => {
        if (
          ...
          || !this.policy.canAttach(entry.session)
        ) {
          attachment.dispose();
          return;
        }
      ...
      .catch((error) => {
        ...
        if (entry.attaching !== pending) return;
        if (sessionStoppedDuringAttach(error)) {
  ...
  #handleEvent(entry: Entry<S>, attachment: TerminalAttachmentLike, event: AttachmentEvent): void {
  ...
  #write(entry: Entry<S>, bytes: Uint8Array, done: () => void): void {
  ...
  #disposeRuntime(entry: Entry<S>): void {
  ...
  #detachAttachment(entry: Entry<S>): void {
    entry.batcher?.flush();
    entry.replay?.cancel();
  ...
  #disposeEntry(entry: Entry<S>): void {
    this.#disposeRuntime(entry);
  }
}
